...
public:
  ...
  /// `A.AI() + 1` as `ai_end`.
  bool apply(const COLTYPE nnodes, const ROWTYPE *ai_begin,
...
private:
  ...
  bool buildChildToParentMaps(const COLTYPE nnodes, const COLTYPE base,
                              const graph::EliminationTree<COLTYPE> &etree,
                              const CSRMatrixType &L);
  bool processNode(const COLTYPE node, const COLTYPE nnodes,
                   const ROWTYPE *ai_begin, const ROWTYPE *ai_end,
                   const COLTYPE *aj, const VALTYPE *av,
                   const graph::EliminationTree<COLTYPE> &etree,
                   CSRMatrixType &L);
  bool initializeFront(const COLTYPE node, const ROWTYPE *ai_begin,
                       const ROWTYPE *ai_end, const COLTYPE *aj,
                       const VALTYPE *av, const CSRMatrixType &L,
                       const COLTYPE front_size);
  bool assembleChildren(const COLTYPE node,
                        const graph::EliminationTree<COLTYPE> &etree);
  bool factorFront(const COLTYPE node,
                   const graph::EliminationTree<COLTYPE> &etree,
                   CSRMatrixType &L, const COLTYPE front_size);
  static bool findLocalIndex(const COLTYPE *begin, const COLTYPE *end,
                             const COLTYPE label, COLTYPE &local_index);
