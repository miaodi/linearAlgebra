...
bool MultifrontalCholesky<CSRMatrixType>::apply(
    const COLTYPE nnodes, const ROWTYPE *ai_begin, const ROWTYPE *ai_end,
    const COLTYPE *aj, const VALTYPE *av,
    const graph::EliminationTree<COLTYPE> &etree, CSRMatrixType &L) {
  if (ai_begin == nullptr || ai_end == nullptr || aj == nullptr ||
      av == nullptr) {
    return false;
  }
...
template <matrix_utils::ResizableCSR CSRMatrixType>
bool MultifrontalCholesky<CSRMatrixType>::prepareNumericValues(
    CSRMatrixType &L) {
  if (L.AI() == nullptr || L.AJ() == nullptr) {
    return false;
  }
...
template <matrix_utils::ResizableCSR CSRMatrixType>
bool MultifrontalCholesky<CSRMatrixType>::buildChildToParentMaps(
    const COLTYPE nnodes, const COLTYPE base,
  ...
  for (COLTYPE node = 0; node < nnodes; node++) {
    ...
    for (ROWTYPE i = 0; i < update_size; i++) {
      ...
      map[static_cast<std::size_t>(i)] =
          static_cast<COLTYPE>(parent_pos - parent_start);
    }
  }
  return true;
}
...
template <matrix_utils::ResizableCSR CSRMatrixType>
bool MultifrontalCholesky<CSRMatrixType>::processNode(
    const COLTYPE node, const COLTYPE, const ROWTYPE *ai_begin,
    const ROWTYPE *ai_end, const COLTYPE *aj, const VALTYPE *av,
    const graph::EliminationTree<COLTYPE> &etree, CSRMatrixType &L) {
  const auto front_size =
      static_cast<COLTYPE>(L.AI()[node + 1] - L.AI()[node]);
  if (front_size <= 0) {
    return false;
  }
...
template <matrix_utils::ResizableCSR CSRMatrixType>
bool MultifrontalCholesky<CSRMatrixType>::initializeFront(
    const COLTYPE node, const ROWTYPE *ai_begin, const ROWTYPE *ai_end,
    const COLTYPE *aj, const VALTYPE *av, const CSRMatrixType &L,
  ...
  for (auto pos = ai_begin[node] - base; pos < ai_end[node] - base; pos++) {
    ...
    [[maybe_unused]] const bool found =
        findLocalIndex(vars, vars_end, col, local);
    assert(found);
    F(local, 0) = av[pos];
    F(0, local) = av[pos];
  }
...
template <matrix_utils::ResizableCSR CSRMatrixType>
bool MultifrontalCholesky<CSRMatrixType>::assembleChildren(
    const COLTYPE node, const graph::EliminationTree<COLTYPE> &etree) {
  const auto *child_offsets = etree.childOffsets();
  const auto *children = etree.children();
  auto F = _worker.front();
  for (auto child_pos = child_offsets[node];
       child_pos < child_offsets[node + 1]; child_pos++) {
    ...
    if (V.cols() != V.rows() ||
        map.size() != static_cast<std::size_t>(update_size)) {
      return false;
    }
...
template <matrix_utils::ResizableCSR CSRMatrixType>
bool MultifrontalCholesky<CSRMatrixType>::factorFront(
    const COLTYPE node, const graph::EliminationTree<COLTYPE> &etree,
    CSRMatrixType &L, const COLTYPE front_size) {
  const auto base = static_cast<COLTYPE>(L.Base());
  ...
  Eigen::Map<Eigen::Matrix<VALTYPE, Eigen::Dynamic, 1>> ltail(
      L.AV() + l_col_start + 1, update_size);
...
template <matrix_utils::ResizableCSR CSRMatrixType>
bool MultifrontalCholesky<CSRMatrixType>::findLocalIndex(
    const COLTYPE *begin, const COLTYPE *end, const COLTYPE label,
    COLTYPE &local_index) {
...
template class MultifrontalCholesky<
