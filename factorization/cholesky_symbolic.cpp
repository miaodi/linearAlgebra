...
void nnzCount( const COLTYPE nnodes,
               const ROWTYPE* ai,
               const COLTYPE* aj,
...
void SkeletonGraph<CSRMatrixType>::apply( const COLTYPE nnodes,
                                          const ROWTYPE* ai,
                                          const COLTYPE* aj,
                                          const COLTYPE* parent,
                                          CSRMatrixType& leaf )
...
#pragma omp parallel num_threads( _nthreads )
    {
        ...
        auto [start, end] = utils::LoadPrefixBalancedPartitionPos( ai, ai + nnodes, tid, nthreads );
        _XLEAFs[tid].clear();
...
template <matrix_utils::ResizableCSR CSRMatrixType>
void SymbolicCholesky<CSRMatrixType>::apply( const COLTYPE nnodes,
                                             const ROWTYPE* ai,
                                             const COLTYPE* aj,
                                             const COLTYPE* parent,
                                             const COLTYPE* XLEAF,
...
#pragma omp parallel num_threads( _nthreads )
    {
        const int tid = omp_get_thread_num();
        auto [start, end] = utils::LoadPrefixBalancedPartitionPos( ai, ai + nnodes, tid, _nthreads );
        _ais[tid].clear();
                                                ...
                                                CSRMatrixType& L )
{
    ...
    const bool has_diagonal =
        matrix_utils::Diagonal( nnodes, ai, aj, (double*)nullptr, _diag.data(), (double*)nullptr );
    if ( !has_diagonal )
    {
        return false;
    }
    ...
    _aj.resize( nnodes );
    const auto visited_size = static_cast<std::size_t>( _nthreads ) * static_cast<std::size_t>( nnodes );
...
#pragma omp parallel for simd num_threads( _nthreads )
    for ( std::ptrdiff_t i = 0; i < static_cast<std::ptrdiff_t>( visited_size ); i++ )
    {
    ...
    for ( int i = 0; i < _nthreads; i++ )
    {
        threads.emplace_back( &SymbolicCholeskyCol<CSRMatrixType>::task, this, nnodes, ai, aj, i,
                              std::ref( L ) );
    }
...
#pragma omp parallel num_threads( _nthreads )
    {
        const int tid = omp_get_thread_num();
        auto [start, end] = utils::LoadPrefixBalancedPartitionPos( L.AI(), L.AI() + nnodes, tid, _nthreads );
        for ( auto i = start; i < end; i++ )
        {
            assert( _aj[i].size() == static_cast<std::size_t>( L.AI()[i + 1] - L.AI()[i] ) );
            std::memcpy( L.AJ() + L.AI()[i] - base, _aj[i].data(), _aj[i].size() * sizeof( COLTYPE ) );
        }
    }
    return true;
}
...
{
    const auto base = ai[0];
    auto* visited = _visited.data() + static_cast<std::size_t>( tid ) * static_cast<std::size_t>( nnodes );
    ...
    auto work = [&, this]( const COLTYPE task )
    {
        ...
        for ( auto child_pos = child_offsets[task]; child_pos < child_offsets[task + 1]; child_pos++ )
        {
    ...
    while ( true )
    {
        COLTYPE task;
        {
            ...
            _cv.wait( lock, [this, nnodes] { return !_queue.empty() || _finished == nnodes; } );
            if ( _finished == nnodes )
            {
                break;
            }
        ...
        // Update the wait predicate while holding the queue mutex so waiters cannot
...
#pragma omp parallel for simd num_threads( _nthreads )
    ...
    for ( int i = 0; i < _nthreads; i++ )
    {
        threads.emplace_back( &SymbolicCholeskyColV2<CSRMatrixType>::task, this, nnodes, ai, aj, i,
                              std::ref( L ) );
    }
    for ( auto& thread : threads )
    {
...
template <matrix_utils::ResizableCSR CSRMatrixType>
void SymbolicCholeskyColV2<CSRMatrixType>::pushReady( const int tid, const COLTYPE task )
{
    {
    ...
    _readyEpoch.fetch_add( 1, std::memory_order_release );
    _readyEpoch.notify_one();
}

template <matrix_utils::ResizableCSR CSRMatrixType>
bool SymbolicCholeskyColV2<CSRMatrixType>::popReady( const int tid, COLTYPE& task )
{
...
template <matrix_utils::ResizableCSR CSRMatrixType>
void SymbolicCholeskyColV2<CSRMatrixType>::task( const COLTYPE nnodes,
                                                 const ROWTYPE* ai,
                                                 const COLTYPE* aj,
                                                 const int tid,
                                                 CSRMatrixType& L )
{
    ...
    while ( true )
    {
        ...
        if ( !popReady( tid, task ) )
        {
            ...
            const auto epoch = _readyEpoch.load( std::memory_order_acquire );
            if ( _readyTasks.load( std::memory_order_acquire ) == 0 &&
                 _finished.load( std::memory_order_acquire ) != nnodes )
            {
                _readyEpoch.wait( epoch, std::memory_order_acquire );
            }
            continue;
        }
        ...
        const auto finished = _finished.fetch_add( 1, std::memory_order_acq_rel );
        if ( finished + 1 == nnodes )
        {
            _readyEpoch.fetch_add( 1, std::memory_order_release );
            _readyEpoch.notify_all();
            break;
        }
    }
}
                                                  ...
                                                  CSRMatrixType& L )
{
    ...
    _aj.resize( nnodes );
    const auto thread_workspace_size =
        static_cast<std::size_t>( _nthreads ) * static_cast<std::size_t>( nnodes );
...
#pragma omp parallel for num_threads( _nthreads )
    for ( std::ptrdiff_t i = 0; i < static_cast<std::ptrdiff_t>( thread_workspace_size ); i++ )
    {
...
#pragma omp parallel for num_threads( _nthreads )
    ...
    for ( int i = 0; i < _nthreads; i++ )
    {
        threads.emplace_back( &SymbolicCholeskyColV3<CSRMatrixType>::task, this, nnodes, ai, aj, i,
                              std::ref( L ) );
    }
    for ( auto& thread : threads )
    {
...
template <matrix_utils::ResizableCSR CSRMatrixType>
void SymbolicCholeskyColV3<CSRMatrixType>::task( const COLTYPE nnodes,
                                                 const ROWTYPE* ai,
                                                 const COLTYPE* aj,
                                                 const int tid,
                                                 CSRMatrixType& L )
...
#define INSTANTIATE_CHOLESKY( ROWTYPE, COLTYPE )                                        \
    template void nnzCount<ROWTYPE, COLTYPE>( const COLTYPE nnodes, const ROWTYPE* ai,  \
                                              const COLTYPE* aj, const COLTYPE* parent, \
                                              ROWTYPE* row_count, ROWTYPE* col_count, COLTYPE* mark );

INSTANTIATE_CHOLESKY( std::int32_t, std::int32_t )
INSTANTIATE_CHOLESKY( std::int64_t, std::int64_t )

template class SkeletonGraph<::matrix_utils::CSRMatrix<std::int32_t, std::int32_t, double>>;
template class SkeletonGraph<::matrix_utils::CSRMatrix<std::int64_t, std::int64_t, double>>;
template class SymbolicCholesky<::matrix_utils::CSRMatrix<std::int32_t, std::int32_t, double>>;
template class SymbolicCholesky<::matrix_utils::CSRMatrix<std::int64_t, std::int64_t, double>>;
template class SymbolicCholeskyCol<::matrix_utils::CSRMatrix<std::int32_t, std::int32_t, double>>;
template class SymbolicCholeskyCol<::matrix_utils::CSRMatrix<std::int64_t, std::int64_t, double>>;
template class SymbolicCholeskyColV2<::matrix_utils::CSRMatrix<std::int32_t, std::int32_t, double>>;
template class SymbolicCholeskyColV2<::matrix_utils::CSRMatrix<std::int64_t, std::int64_t, double>>;
template class SymbolicCholeskyColV3<::matrix_utils::CSRMatrix<std::int32_t, std::int32_t, double>>;
template class SymbolicCholeskyColV3<::matrix_utils::CSRMatrix<std::int64_t, std::int64_t, double>>;
} // namespace factorization
