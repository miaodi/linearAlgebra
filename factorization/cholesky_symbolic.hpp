...
public:
    ...
    SkeletonGraph( const int nthreads )
        : _nthreads( nthreads ), _XLEAFs( nthreads ), _LEAFs( nthreads ), _XLEAF_prefix( nthreads + 1 )
    {
    }

    void apply( const COLTYPE nnodes, const ROWTYPE* ai, const COLTYPE* aj, const COLTYPE* parent, CSRMatrixType& leaf );
...
public:
    ...
    SymbolicCholesky( const int nthreads )
        : _nthreads( nthreads ), _ais_prefix( nthreads + 1 ), _ais( nthreads ), _ajs( nthreads )
    {
    }
...
public:
    ...
    bool apply( const COLTYPE nnodes, const ROWTYPE* ai, const COLTYPE* aj, const COLTYPE* parent, CSRMatrixType& L );

    const graph::EliminationTree<COLTYPE>& eliminationTree() const { return _etree; }

private:
    void task( const COLTYPE nnodes, const ROWTYPE* ai, const COLTYPE* aj, const int tid, CSRMatrixType& L );
