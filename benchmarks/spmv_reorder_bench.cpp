...
struct MatrixCache
{
    ...
    void load( const std::string& file )
    {
        ...
        matrix_utils::APlusATPrefix<int, int, false>( original.rows, original.AI(), original.AJ(),
                                                      xadj.data() );
...
static void BM_CuSparseSPMV_Reordering( benchmark::State& state, const std::string& matrix_file )
{
    ...
    state.counters["GFLOPS"] = benchmark::Counter(
        flops, benchmark::Counter::kIsIterationInvariantRate, benchmark::Counter::kIs1000 );
    state.counters["GB/s"] = benchmark::Counter(
        nnz_count * ( sizeof( int ) + sizeof( double ) ) + matrix.rows * sizeof( double ) * 2,
        benchmark::Counter::kIsIterationInvariantRate, benchmark::Counter::kIs1024 );
...
static void BM_MKLSPMV_Reordering( benchmark::State& state, const std::string& matrix_file )
{
    ...
    matrix_utils::SPMV<matrix_utils::CSRMatrix<int, int, double>, matrix_utils::MKLSPMV<MKL_INT, MKL_INT, double>> spmv;
    spmv._matrix = &matrix;
...
int main( int argc, char** argv )
{
    ...
    options.allow_unrecognised_options().add_options()(
        ...
        "f,file", "Matrix file location", cxxopts::value<std::string>()->default_value( "data/thermal2.mtx" ) )(
    ...
    // Parse optional thread list
    {
        ...
        if ( !tl.empty() )
        {
            ...
            while ( start < tl.size() )
            {
                ...
                std::string token =
                    tl.substr( start, comma == std::string::npos ? std::string::npos : ( comma - start ) );
                if ( !token.empty() )
                {
                    try
                    {
    ...
    for ( size_t reorder_idx = 0; reorder_idx < reordering_configs.size(); ++reorder_idx )
    {
        for ( int t : thread_list )
        {
            ...
            std::string name =
                "ParallelSPMV/" + reordering_configs[reorder_idx].name + "/nt=" + std::to_string( t );
    ...
    for ( size_t reorder_idx = 0; reorder_idx < reordering_configs.size(); ++reorder_idx )
    {
        for ( int t : thread_list )
        {
            if ( use_simd )
            {
                ...
                std::string name = "ALBUSSPMV-Simd/" + reordering_configs[reorder_idx].name +
                                   "/nt=" + std::to_string( t );
            ...
            else
            {
                ...
                std::string name = "ALBUSSPMV-Scalar/" + reordering_configs[reorder_idx].name +
                                   "/nt=" + std::to_string( t );
                benchmark::RegisterBenchmark( name.c_str(), bm_albus )
                    ->Args( { static_cast<int64_t>( reorder_idx ), static_cast<int64_t>( t ) } )
                    ->Unit( benchmark::kMicrosecond );
    ...
    for ( size_t reorder_idx = 0; reorder_idx < reordering_configs.size(); ++reorder_idx )
    {
        for ( int t : thread_list )
        {
            if ( use_simd )
            {
                ...
                {
                    BM_SPMV_Reordering<matrix_utils::ALBUSSPMV<int, int, double, matrix_utils::RowDotKernel::Simd, matrix_utils::WorkloadMode::CAMLB>>(
                        state, matrix_file, "CAMLBSPMV-Simd" );
                };
                std::string name = "CAMLBSPMV-Simd/" + reordering_configs[reorder_idx].name +
                                   "/nt=" + std::to_string( t );
            ...
            else
            {
                auto bm_camlb = [=]( benchmark::State& state )
                {
                    BM_SPMV_Reordering<matrix_utils::ALBUSSPMV<int, int, double, matrix_utils::RowDotKernel::Scalar, matrix_utils::WorkloadMode::CAMLB>>(
                        state, matrix_file, "CAMLBSPMV-Scalar" );
                };
                std::string name = "CAMLBSPMV-Scalar/" + reordering_configs[reorder_idx].name +
                                   "/nt=" + std::to_string( t );
                benchmark::RegisterBenchmark( name.c_str(), bm_camlb )
                    ->Args( { static_cast<int64_t>( reorder_idx ), static_cast<int64_t>( t ) } )
                    ->Unit( benchmark::kMicrosecond );
    ...
    for ( size_t reorder_idx = 0; reorder_idx < reordering_configs.size(); ++reorder_idx )
    {
        for ( int t : thread_list )
        {
            ...
            std::string name =
                "MKLSPMV/" + reordering_configs[reorder_idx].name + "/nt=" + std::to_string( t );
