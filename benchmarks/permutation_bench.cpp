...
BENCHMARK_DEFINE_F(MyFixture, isPermutation)(benchmark::State &state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(
        matrix_utils::isPermutation(size, 0, perm_pair.first->second.data()));
  }
}
...
BENCHMARK_DEFINE_F(MyFixture, isPermutation_Serial)(benchmark::State &state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(matrix_utils::isPermutationSerial(
        size, 0, perm_pair.first->second.data()));
  }
}
