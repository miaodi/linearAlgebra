...
BENCHMARK_F( MyFixture, BM_BFS )( benchmark::State& state )
{
    ...
    for ( auto _ : state )
    {
        bfs( static_cast<int>( ptr->ai.size() - 1 ), ptr->ai.data(), ptr->aj.data(), ptr->av.data(), 0 );
    }
}
...
BENCHMARK_REGISTER_F( MyFixture, BM_PBFS_NOLEVELS )->RangeMultiplier( 2 )->Range( 1, MAX_THREADS );
...
BENCHMARK_F( MyFixture, BM_BFS_DIRECT )( benchmark::State& state )
{
    ...
    for ( auto _ : state )
    {
        ...
        graph::BFSFunc<ROW, COL, false, false>( rows, ai, aj, source, shortCutWidth, height, width,
                                                levels, lastLevel, 1 );
...
BENCHMARK_DEFINE_F( MyFixture, BM_PBFS_DIRECT_TRACK_ON )( benchmark::State& state )
{
    ...
    for ( auto _ : state )
    {
        ...
        std::vector<COL> levels;
        std::vector<COL> lastLevel;
        graph::BFSFunc<ROW, COL, false, true>( rows, ai, aj, source, shortCutWidth, height, width,
                                               levels, lastLevel, state.range( 0 ) );
        benchmark::DoNotOptimize( height );
        benchmark::DoNotOptimize( width );
    }
}
...
int main( int argc, char** argv )
{
    ...
    if ( ptr )
    {
        std::cout << "rows: " << ptr->ai.size() - 1 << ", nnz: " << ptr->aj.size() << std::endl;
    }
