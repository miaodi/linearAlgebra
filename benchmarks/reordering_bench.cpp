...
public:
    ...
    Reordering()
    {
        ...
        if ( mat == nullptr )
        {
            ...
            std::cout << "read K\n";
            utils::ReadFromBinaryCSR( k_mat, csr_rows, csr_cols, csr_vals, SPARSE_INDEX_BASE_ONE );
            ...
            mat.reset( new mkl_wrapper::mkl_sparse_mat( size, size, csr_rows, csr_cols, csr_vals,
            ...
            matrix_utils::permuteMat( mat->rows(), mat->cols(), inv_perm.data(), perm.data(),
                                      mat->get_ai().get(), mat->get_aj().get(), mat->get_av().get(),
                                      ai.get(), aj.get(), av.get() );
            perm_mat.reset( new mkl_wrapper::mkl_sparse_mat( mat->rows(), mat->cols(), ai, aj, av ) );
            std::cout << "bandwidth after reordering: " << perm_mat->bandwidth() << std::endl;
            ...
            matrix_utils::permuteMat( mat->rows(), mat->cols(), inv_perm1.data(), perm1.data(),
                                      mat->get_ai().get(), mat->get_aj().get(), mat->get_av().get(),
                                      ai1.get(), aj1.get(), av1.get() );
            perm_mat1.reset( new mkl_wrapper::mkl_sparse_mat( mat->rows(), mat->cols(), ai1, aj1, av1 ) );
            std::cout << "bandwidth after reordering: " << perm_mat1->bandwidth() << std::endl;
#endif

            auto [ai2, aj2, av2] = matrix_utils::AllocateCSRData( mat->rows(), mat->nnz() );
            matrix_utils::permuteMat( mat->rows(), mat->rows(), inv_perm.data(), inv_perm.data(),
                                      mat->get_ai().get(), mat->get_aj().get(), mat->get_av().get(),
                                      ai2.get(), aj2.get(), av2.get() );
            perm_mat_sym.reset( new mkl_wrapper::mkl_sparse_mat_sym( mat->rows(), mat->cols(), ai2, aj2, av2 ) );
            std::cout << "bandwidth after reordering: " << perm_mat_sym->bandwidth() << std::endl;
        }
    }
};
