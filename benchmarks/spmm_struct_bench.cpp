...
static void BM_SpMMStruct( benchmark::State& state, const std::string& matrix_file, OuterProductBuildMethod method )
{
    ...
    // Warm-up call
    bool success = SpMMStruct( n, thrust::raw_pointer_cast( d_ai_A.data() ),
                               thrust::raw_pointer_cast( d_aj_A.data() ),
                               thrust::raw_pointer_cast( d_ai_A.data() ),
                               thrust::raw_pointer_cast( d_aj_A.data() ), base, packed_coo, method );
    ...
    for ( auto _ : state )
    {
        success = SpMMStruct( n, thrust::raw_pointer_cast( d_ai_A.data() ),
                              thrust::raw_pointer_cast( d_aj_A.data() ),
                              thrust::raw_pointer_cast( d_ai_A.data() ),
                              thrust::raw_pointer_cast( d_aj_A.data() ), base, packed_coo, method );
    ...
    state.SetBytesProcessed( state.iterations() * ( mat.ai.size() * sizeof( int ) + // A pointers
...
int main( int argc, char** argv )
{
    ...
    try
    {
        ...
        if ( builder == "shared" )
            ...
            throw std::runtime_error( "Invalid --builder value: " + builder +
                                      " (expected global|shared)" );

        // Register benchmark with the matrix file
        benchmark::RegisterBenchmark( "BM_SpMMStruct", BM_SpMMStruct, matrix_file, method )->Unit( benchmark::kMillisecond );
    }
