...
static std::unique_ptr<mkl_wrapper::incomplete_lu_k> precond;
...
public:
  MyFixture() {
    ...
    if (mat == nullptr) {
      ...
      //   std::ifstream f(
      //       "/SCRATCH/dimiao/my_repo/linearAlgebra/release/tests/data/ex5.mtx");
      //   f.clear();
...
BENCHMARK_DEFINE_F(MyFixture, SerialForward)(benchmark::State &state) {
  ...
  matrix_utils::SplitLDU(precond->rows(), (int)precond->mkl_base(),
                         precond->get_ai(), precond->get_aj(),
                         precond->get_av(), L, D, U);

  for (auto _ : state) {
    for (int i = 0; i < state.range(0); i++) {
      matrix_utils::ForwardSubstitution(L.rows, L.base, L.ai, L.aj, L.av,
                                        b.data(), x.data());
    }
  }
}

BENCHMARK_REGISTER_F(MyFixture, SerialForward)->Arg(10)->Arg(100)->Arg(1000);
  ...
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); i++) {
      matrix_utils::LevelScheduleForwardSubstitution(
          iperm, prefix, L.rows, L.base, L.ai, L.aj, L.av, b.data(), x.data());
    }
  }
}
