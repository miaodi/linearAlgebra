        ...
        for ( size_t idx = 0; idx < klevels.size(); ++idx )
        {
            ...
            if ( ok_sym && ok_num )
            {
                matrix_utils::SplitLDU( ilu_mat.rows, ilu_mat.Base(), ilu_mat.AI(), ilu_mat.AJ(),
                                        ilu_mat.AV(), data.factors[idx].L, data.factors[idx].D,
                                        data.factors[idx].U );
                data.factors[idx].levelL.analysis(
                    data.factors[idx].L.rows, data.factors[idx].L.ai.get(),
                    data.factors[idx].L.aj.get(), data.factors[idx].L.av.get(), nullptr );
                data.factors[idx].levelU.analysis(
                    data.factors[idx].U.rows, data.factors[idx].U.ai.get(), data.factors[idx].U.aj.get(),
                    data.factors[idx].U.av.get(), data.factors[idx].D.data() );
                data.factors[idx].ok = true;
                std::cout << "ILU(" << k << ") OK: levels(L)=" << data.factors[idx].levelL._levels
                          << " levels(U)=" << data.factors[idx].levelU._levels << "\n";
            }
            else
            {
...
{
    auto& d = getData();
    if ( kidx >= d.factors.size() || !d.factors[kidx].ok )
        return; // nothing to benchmark
    ...
    for ( auto _ : state )
    {
        ...
        matrix_utils::TriangularSolve<matrix_utils::TriangularMatrix::L, int, int, double>(
            F.L.rows, F.L.ai.get(), F.L.aj.get(), F.L.av.get(),
...
static void RegisterBenchmarks()
{
    ...
    {
        if ( !d.factors[kidx].ok )
            continue;
        benchmark::RegisterBenchmark(
            ( std::string( "SerialForward/ILU=" ) + std::to_string( d.factors[kidx].k ) ).c_str(),
            [kidx]( benchmark::State& st ) { SerialForward( st, kidx ); } );
        benchmark::RegisterBenchmark(
            ( std::string( "SerialBackward/ILU=" ) + std::to_string( d.factors[kidx].k ) ).c_str(),
    ...
    if ( const char* env = std::getenv( "MAX_BENCH_THREADS" ) )
    {
        ...
        if ( val > 0 )
            maxThreads = std::min( maxThreads, val );
    }
    ...
    for ( size_t kidx = 0; kidx < d.factors.size(); ++kidx )
    {
        ...
        auto* fwdReg = benchmark::RegisterBenchmark(
            nameF.c_str(), [kidx]( benchmark::State& st ) { LevelForward( st, kidx ); } );
        auto* bwdReg = benchmark::RegisterBenchmark(
            nameB.c_str(), [kidx]( benchmark::State& st ) { LevelBackward( st, kidx ); } );
        for ( int arg : threadArgs )
        {
            fwdReg->Arg( arg );
            bwdReg->Arg( arg );
        }
