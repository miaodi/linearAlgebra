...
auto KahnSerialBench = []( benchmark::State& state, const CSRMatrixType& mat )
{
    ...
    for ( auto _ : state )
    {
        kahn.operator()( mat.rows, mat.AI(), mat.AJ(), perm.data(), prefix.data() );
    }
};
...
auto TopologicalSort2Bench = []( benchmark::State& state, const CSRMatrixType& mat )
{
    ...
    for ( auto _ : state )
    {
        topologicalSort.operator()( mat.rows, mat.AI(), mat.AJ(), perm.data(), prefix.data() );
    }
};
...
int main( int argc, char** argv )
{
    ...
    options.allow_unrecognised_options().add_options()(
        ...
        "r,rnnz", "row nnz", cxxopts::value<int>()->default_value( "10" ) )( "h,help",
                                                                             "Print usage" );
