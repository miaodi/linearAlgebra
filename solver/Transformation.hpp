...
concept VectorForMatrix = SwappableResizableCSR<MatType> && VectorLike<VecType, typename MatType::VALTYPE>;

// Concept for transformation types
template <typename T, typename MatType, typename VecType>
concept Transformation =
    VectorForMatrix<VecType, MatType> && requires( T t, VecType& vec, MatType& mat, int nthreads ) {
...
public:
    ...
    {
        // out = P * in
        matrix_utils::permVec( static_cast<COLTYPE>( _perm.size() ), _base, in.data(), _perm.data(),
                               out.data(), nthreads );
    }
...
public:
    ...
    {
        // out = Q^{-1} * in
        matrix_utils::invPermVec( static_cast<COLTYPE>( _perm.size() ), _base, in.data(),
                                  _perm.data(), out.data(), nthreads );
    }
...
public:
    ...
    void applyToOperator( MatType& in, MatType& out, int nthreads = 1 ) const override
    {
        ...
        matrix_utils::permuteMat( in.rows, in.cols, _row_perm.data(), _col_perm.data(), in.AI(),
                                  in.AJ(), in.AV(), out.AI(), out.AJ(), out.AV(), nthreads );
    }

    void applyToRHS( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        // out = P_r * in (apply row permutation to RHS)
        matrix_utils::permVec( static_cast<COLTYPE>( _row_perm.size() ), _base, in.data(),
                               _row_perm.data(), out.data(), nthreads );
    }

    void applyToX( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        // out = Q_c * in (apply column permutation to solution)
        matrix_utils::permVec( static_cast<COLTYPE>( _col_perm.size() ), _base, in.data(),
                               _col_perm.data(), out.data(), nthreads );
    }

    void applyInverseToX( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        // out = Q_c^{-1} * in (apply inverse column permutation)
        matrix_utils::invPermVec( static_cast<COLTYPE>( _col_perm.size() ), _base, in.data(),
                                  _col_perm.data(), out.data(), nthreads );
    }
...
public:
    ...
    void applyToRHS( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        ...
        std::swap( in, out );
        matrix_utils::ScaleVec( static_cast<COLTYPE>( _scales.size() ), out.data(), _scales.data(), nthreads );
    }
...
public:
    ...
    void applyInverseToX( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        // out = S_c^{-1} * in
        std::swap( in, out );
        matrix_utils::InvScaleVec( static_cast<COLTYPE>( _scales.size() ), out.data(), _scales.data(), nthreads );
    }
...
public:
    ...
    explicit RowColScaling( const VALTYPE* row_scales, const VALTYPE* col_scales, size_t size, int base = 0 )
        : _row_scales( row_scales, size ), _col_scales( col_scales, size )
    {
    }

    explicit RowColScaling( std::span<const VALTYPE> row_scales, std::span<const VALTYPE> col_scales, int base = 0 )
    ...
    void applyToOperator( MatType& in, MatType& out, int nthreads = 1 ) const override
    {
        ...
        std::swap( in, out );
        matrix_utils::ScaleMat( out.rows, out.AI(), out.AJ(), out.AV(), _row_scales.data(),
                                _col_scales.data(), nthreads );
    }
    ...
    void applyToRHS( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        // out = S_r * in (scale RHS by row scales)
        std::swap( in, out );
        matrix_utils::ScaleVec( static_cast<COLTYPE>( _row_scales.size() ), out.data(),
                                _row_scales.data(), nthreads );
    }
    ...
    void applyToX( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        // out = S_c * in (scale solution by column scales)
        std::swap( in, out );
        matrix_utils::ScaleVec( static_cast<COLTYPE>( _col_scales.size() ), out.data(),
                                _col_scales.data(), nthreads );
    }
    ...
    void applyInverseToX( VecType& in, VecType& out, int nthreads = 1 ) const override
    {
        ...
        std::swap( in, out );
        matrix_utils::InvScaleVec( static_cast<COLTYPE>( _col_scales.size() ), out.data(),
                                   _col_scales.data(), nthreads );
    }
