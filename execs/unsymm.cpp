...
{
    cxxopts::Options options( "GMRES Example", "Example of using GMRES with a CSR matrix" );
    options.add_options()(
        "f,filename", "Matrix Market file to read",
        cxxopts::value<std::string>()->default_value( "../tests/data/ex5.mtx" ) )(
        "l,level", "ILU level", cxxopts::value<int>()->default_value( "0" ) )( "h,help",
                                                                               "Print usage" );
    ...
    matrix_utils::permuteMat<int, int>( csr_matrix.rows, csr_matrix.cols, matching_col.data(),
                                        (int*)nullptr, csr_matrix.AI(), csr_matrix.AJ(),
                                        permuted_matrix.AI(), permuted_matrix.AJ() );

    std::ofstream out1( "mat_csr_rowperm_1.svg" );
    matrix_utils::writeSVG( permuted_matrix.rows, permuted_matrix.cols, permuted_matrix.AI(),
                            permuted_matrix.AJ(), out1 );
    out1.close();
