...
CSR metisReorder(const CSR &matrix, const int threads, const int seed) {
  ...
  matrix_utils::APlusATPrefix<int, int, false>(matrix.rows, matrix.AI(),
                                               matrix.AJ(), xadj.data());
  const int graph_edges = xadj[matrix.rows] - xadj[0];
  std::vector<int> adjncy(static_cast<std::size_t>(graph_edges));
  matrix_utils::APlusATFill<int, int, false>(matrix.rows, matrix.AI(),
                                             matrix.AJ(), xadj.data(),
                                             adjncy.data());
  ...
  const int rc = reordering::MetisND<int, int>(
      matrix.rows, matrix.cols, xadj.data(), adjncy.data(), iperm.data(),
      perm.data(), nd_options);
  if (rc != 0) {
    throw std::runtime_error("METIS nested dissection failed with code " +
                             std::to_string(rc));
  }

  const bool perm_ok = matrix_utils::isPermutation<int>(matrix.rows, 0,
                                                        perm.data(), threads);
  const bool iperm_ok = matrix_utils::isPermutation<int>(matrix.rows, 0,
                                                         iperm.data(), threads);
  if (!perm_ok || !iperm_ok) {
  ...
  matrix_utils::permuteMat(matrix.rows, matrix.cols, perm.data(), iperm.data(),
                           ...
                           reordered.AI(), reordered.AJ(), reordered.AV(),
                           threads);
  return reordered;
#endif
}
...
Options parseOptions(const int argc, char **argv) {
  ...
  options.add_options()("i,input", "Input MatrixMarket .mtx file",
      ...
      "t,threads", "Threads used for permutation work",
...
int main(int argc, char **argv) {
  try {
    ...
    CSR matrix;
    matrix_utils::readMatrix(options.input, matrix,
                             matrix_utils::MatrixDataType::MatrixMarket);
    const auto read_end = std::chrono::steady_clock::now();

    std::cout << "Loaded " << options.input << ": rows=" << matrix.rows
    ...
    if (options.metis_reorder) {
      ...
      std::cout << "Applied METIS nested-dissection reordering in "
                << reorder_time.count() << " s\n";
    }

    if (!options.sparsity_png.empty()) {
      const auto png_start = std::chrono::steady_clock::now();
      matrix_utils::writePNG(matrix, options.sparsity_png,
                             options.sparsity_resolution);
      ...
      std::cout << "Wrote sparsity PNG " << options.sparsity_png << " at up to "
                << options.sparsity_resolution << "x"
                << options.sparsity_resolution << " pixels in "
                << png_time.count() << " s\n";
    }

    const auto write_start = std::chrono::steady_clock::now();
    matrix_utils::writeMatrix(matrix, options.output,
                              matrix_utils::MatrixDataType::Binary);
    ...
    std::cout << "Wrote " << options.output << " in " << write_time.count()
              << " s\n";
