...
int main( int argc, char** argv )
{
    ...
    options.add_options()( "l,level", "Level of ILU", cxxopts::value<int>()->default_value( "0" ) );
    ...
    matrix_utils::writeSVG( csr_matrix.rows, csr_matrix.cols, csr_matrix.AI(), csr_matrix.AJ(), out0 );
    out0.close();
    std::vector<int> parent( csr_matrix.rows );
    std::vector<int> ancestor( csr_matrix.rows );

    graph::eliminationTree( csr_matrix.rows, csr_matrix.AI(), csr_matrix.AJ(), parent.data(),
                            ancestor.data() );
    ...
    // }
#ifdef USE_BOOST_LIB
    utils::printEliminationTree( csr_matrix.rows, csr_matrix.AI()[0], parent.data(), "tree.dot" );
#endif
    ...
    postorder.apply( csr_matrix.rows, 0, parent.data(), permed_parent.data(), perm.data(), iperm.data() );
    //   {
    ...
    matrix_utils::permuteMat<int, int, double>(
        csr_matrix.rows, csr_matrix.cols, perm.data(), iperm.data(), csr_matrix.AI(), csr_matrix.AJ(),
        csr_matrix.AV(), csr_matrix_perm.AI(), csr_matrix_perm.AJ(), csr_matrix_perm.AV() );

    std::ofstream out1( "mat_csr_perm.svg" );
    matrix_utils::writeSVG( csr_matrix_perm.rows, csr_matrix_perm.cols, csr_matrix_perm.AI(),
                            csr_matrix_perm.AJ(), out1 );
    out1.close();
    // #ifdef USE_BOOST_LIB
    //   utils::printEliminationTree(csr_matrix.rows, csr_matrix.AI()[0],
    ...
    // }
    {
        ...
        matrix_utils::ICCLevelSymbolicParallel<matrix_utils::CSRMatrix<int, int, double>> icc_symbolic( 20 );
        icc_symbolic( csr_matrix_perm.rows, csr_matrix_perm.AI(), csr_matrix_perm.AJ(), level, L );
