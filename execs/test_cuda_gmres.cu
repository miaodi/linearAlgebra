 ...
 */
std::unique_ptr<Preconditioner> setupPreconditioner( PreconditionerImpl precond_impl,
                                                     ...
                                                     const cxxopts::ParseResult& options )
{
    ...
    if ( precond_impl == PreconditionerImpl::ILU || precond_impl == PreconditionerImpl::GPU_ILU0 )
    {
        if ( has_lu_files )
        {
            ...
            {
                std::cerr << "Error: L matrix dimensions (" << L_matrix.rows << "x" << L_matrix.cols
                          << ") do not match problem size (" << n << "x" << n << ")" << std::endl;
                throw std::runtime_error( "L matrix dimension mismatch" );
            }
            if ( U_matrix.rows != n || U_matrix.cols != n )
            {
                std::cerr << "Error: U matrix dimensions (" << U_matrix.rows << "x" << U_matrix.cols
                          << ") do not match problem size (" << n << "x" << n << ")" << std::endl;
                throw std::runtime_error( "U matrix dimension mismatch" );
            }

            std::cout << "L matrix loaded: " << L_matrix.rows << "x" << L_matrix.cols
                      << ", nnz: " << L_matrix.NNZ() << std::endl;
            std::cout << "U matrix loaded: " << U_matrix.rows << "x" << U_matrix.cols
                      << ", nnz: " << U_matrix.NNZ() << std::endl;
        }
        else
        {
            ...
            std::cout << "Symbolic ILU factorization time: " << elapsed.count() << " s" << std::endl;
            {
                ...
                else
                {
                    ...
                    for ( int row = 0; row < ilu_matrix.rows; ++row )
                    {
                        ...
                        if ( diag_idx < row_start || diag_idx >= row_end || aj[diag_idx] != row + base )
                        {
                            std::cerr << "Temporary check: invalid diagonal at row " << row << std::endl;
                            diag_ok = false;
                            break;
                        }
                    }
                ...
                if ( diag_ok )
                {
                    std::cout << "Temporary check: diagonal positions are valid." << std::endl;
                }
            }
            if ( !success )
            {
    ...
    if ( precond_impl == PreconditionerImpl::ILU )
    {
        ...
        precond->setup( n, L_matrix.AI(), L_matrix.AJ(), L_matrix.AV(), // L factor
    ...
    else if ( precond_impl == PreconditionerImpl::GPU_ILU0 )
    {
        ...
        precond->setup( n, csr_matrix.AI(), csr_matrix.AJ(), csr_matrix.AV() );
        return precond;
    }
    else if ( precond_impl == PreconditionerImpl::JACOBI )
    {
        ...
        precond->setupFromMatrix( n, csr_matrix.AI(), csr_matrix.AJ(), csr_matrix.AV() );
        return precond;
    }
 ...
 */
std::unique_ptr<solver::RowColPermutation<matrix_utils::CSRMatrix<int, int, double>>> reorderMatrixAndRHS(
    ...
    int nthreads = 1 )
{
    ...
    matrix_utils::APlusATPrefix<int, int, false>( n, matrix.AI(), matrix.AJ(), xadj.data() );

    int actual_edges = xadj[n] - xadj[0];
    std::vector<int> adjncy( actual_edges );
    matrix_utils::APlusATFill<int, int, false>( n, matrix.AI(), matrix.AJ(), xadj.data(), adjncy.data() );
    ...
    else if ( algorithm == "nd" )
    {
        ...
        int rc = reordering::MetisND<int, int>( n, n, xadj.data(), adjncy.data(), iperm.data(), perm.data() );
        if ( rc != 0 )
        {
    ...
    matrix_utils::permuteMat<int, int, double>(
        static_cast<int>( n ), static_cast<int>( n ), const_cast<const int*>( perm.data() ),
        const_cast<const int*>( iperm.data() ), matrix.AI(), matrix.AJ(), matrix.AV(),
        permuted_matrix.AI(), permuted_matrix.AJ(), permuted_matrix.AV(), nthreads );
    ...
    matrix_utils::permVec( static_cast<int>( n ), 0, rhs.data(), perm.data(), permuted_rhs.data(), nthreads );
    ...
    return std::make_unique<solver::RowColPermutation<matrix_utils::CSRMatrix<int, int, double>>>(
        std::span<const int>( perm.data(), perm.size() ), std::span<const int>( perm.data(), perm.size() ), 0 );
}
...
int main( int argc, char** argv )
{
    ...
    cxxopts::Options options( "CUDA GMRES Test",
    ...
    options.add_options()( "f,filename", "Matrix Market file to read",
        ...
        "L-file", "Matrix Market file containing L factor for ILU preconditioner",
        cxxopts::value<std::string>()->default_value( "" ) )(
        "U-file", "Matrix Market file containing U factor for ILU preconditioner",
        cxxopts::value<std::string>()->default_value( "" ) )(
        "b,batch-ortho", "Enable batch orthogonalization in GMRES",
        cxxopts::value<bool>()->default_value( "false" ) )(
        "reorder", "Matrix reordering algorithm: none, rcm, rcm-par, nd",
        cxxopts::value<std::string>()->default_value( "none" ) )(
        "n,nthreads", "Number of threads for parallel operations", cxxopts::value<int>()->default_value( "1" ) )(
        "method", "Solver method: gmres, bicgstab",
        cxxopts::value<std::string>()->default_value( "gmres" ) )( "h,help", "Print usage" );
    ...
    std::cout << "  RHS file: " << ( rhs_file.empty() ? "(none - using all ones)" : rhs_file ) << std::endl;
    ...
    try
    {
        ...
        // Prune zero entries from the matrix
        {
            ...
            const int num_pruned = matrix_utils::Prune(
                csr_matrix.rows, csr_matrix.AI(), csr_matrix.AJ(), csr_matrix.AV(), 0.0, row_thresholds );
        ...
        if ( reorder_alg != "none" )
        {
            ...
            perm = reorderMatrixAndRHS( reorder_alg, csr_matrix, b_host, working_matrix, working_rhs, nthreads );
        ...
        matrix_utils::sparse_cuda::copy_csr_host_to_device<int, int, double>(
            nrows, working_matrix.AI(), working_matrix.AJ(), working_matrix.AV(), &d_ia, &d_ja, &d_av );
        ...
        if ( use_bicgstab )
        {
            ...
            std::unique_ptr<Preconditioner> precond = setupPreconditioner(
                precond_impl, solver.getCusparseHandle(), n, working_matrix, parsed_options );
        ...
        std::cout << "Solver completed in " << duration.count() << " ms (total), " << solve_time_ms
                  << " ms (GPU solve time)" << std::endl;
