...
public:
    ...
    int createMatrix( string const& fileName, Eigen::SparseMatrix<RC>& M )
    {
        ...
        if ( rc != 0 )
        {
            cerr << "Error: read matrix market file KO" << endl;
            return rc;
        }
        ...
        for ( size_t k = 0; k < nnz; k++ )
            triplets.emplace_back( i[k], j[k], Mij[k] );
    ...
    int solve( EM& A, EM const* B = nullptr )
    {
        stdPb = !B ? true : false;
        if ( nbCV > A.cols() )
            nbCV = A.cols(); // Cut-off arpack workspace dim.
        ...
        if ( verbose >= 1 )
        {
            cout << endl << "arpackSolver:" << endl;
            cout << endl
                 << "mode " << mode << ", backTransform " << ( backTransform ? "yes" : "no" ) << endl;
        }
        ...
        if ( rc != 0 )
        {
            cerr << "Error: initialize solver KO" << endl;
            return rc;
        }
        rc = solve( A, B, solver );
        if ( rc != 0 )
    ...
    int checkEigVec( EM const& A, EM const* B = nullptr, double const* diffTol = nullptr )
    {
        ...
        for ( size_t i = 0; i < vec.size(); i++ )
        {
            ...
            if ( verbose >= 1 )
            {
                ...
                {
                    cout << endl
                         << rs << " vector " << setw( 3 ) << i << " (norm " << V.norm() << "): " << endl;
                    cout << endl << V << endl;
                }
            }
            ...
            {
                cerr << endl
                     << "Error: bad vector " << setw( 3 ) << i << " (norm " << V.norm() << "):" << endl;
                cerr << endl << V << endl;
                cerr << endl << "Error: left side (A*V - norm " << left.norm() << "):" << endl;
                cerr << endl << left << endl;
                cerr << endl
                     << "Error: right side (lambda*" << ( stdPb ? "" : "B*" ) << "V - norm "
                     << right.norm() << "):" << endl;
                cerr << endl << right << endl;
                cerr << endl
                     << "Error: diff (norm " << diff.norm() << ", tol " << dTol << "):" << endl;
                cerr << endl << diff << endl;
                return 1;
            }
            else
...
private:
    ...
    void makeConstant( complex<float>& cst, float const& val )
    {
        cst = complex<float>( val, val );
    };

    void makeConstant( complex<double>& cst, double const& val )
    {
        cst = complex<double>( val, val );
    };

    int readMatrixMarket( string const& fileName, a_uint& n, a_uint& m, vector<a_uint>& i, vector<a_uint>& j, vector<RC>& Mij )
    {
        ...
        do
        {
            ...
            getline( inp, inpLine );
            l++;
            while ( isspace( *inpLine.begin() ) )
                inpLine.erase( inpLine.begin() ); // Suppress leading white spaces.
            if ( inpLine.length() == 0 )
                continue; // Empty line.
            if ( inpLine[0] == '%' )
                continue; // Comments skipped, begin reading.
            ...
            { // Header.
                ...
                if ( !inpSS )
                {
                    cerr << "Error: bad header (n, m)" << endl;
                    return 1;
                }
                if ( nnz == 0 )
                {
                    inpSS >> nnz;
                    if ( inpSS )
                    { // OK, (optional) nnz has been provided.
                        i.reserve( nnz );
                        j.reserve( nnz );
                        Mij.reserve( nnz );
                    }
            ...
            { // Body.
                ...
                if ( !inpSS )
                {
                    cerr << "Error: bad line (" << fileName << ", line " << l << ")" << endl;
                    return 1;
                }
                i.push_back( k );
                j.push_back( l );
               ...
               a_int* info )
    {
        ...
        if ( symPb )
        {
            ssaupd_c( ido, bMat, nbDim, which, nbEV, tol, resid, nbCV, v, ldv, iparam, ipntr, workd,
                      workl, lworkl, info );
        }
        else
        {
            snaupd_c( ido, bMat, nbDim, which, nbEV, tol, resid, nbCV, v, ldv, iparam, ipntr, workd,
                      workl, lworkl, info );
        }
    };
               ...
               a_int* info )
    {
        ...
        if ( symPb )
        {
            dsaupd_c( ido, bMat, nbDim, which, nbEV, tol, resid, nbCV, v, ldv, iparam, ipntr, workd,
                      workl, lworkl, info );
        }
        else
        {
            dnaupd_c( ido, bMat, nbDim, which, nbEV, tol, resid, nbCV, v, ldv, iparam, ipntr, workd,
                      workl, lworkl, info );
        }
    };
               ...
               a_int* info )
    {
        ...
        cnaupd_c( ido, bMat, nbDim, which, nbEV, tol, reinterpret_cast<a_fcomplex*>( resid ), nbCV,
                  ...
                  reinterpret_cast<a_fcomplex*>( workd ), reinterpret_cast<a_fcomplex*>( workl ),
                  lworkl, rwork, info );
    };
               ...
               a_int* info )
    {
        ...
        znaupd_c( ido, bMat, nbDim, which, nbEV, tol, reinterpret_cast<a_dcomplex*>( resid ), nbCV,
                  ...
                  reinterpret_cast<a_dcomplex*>( workd ), reinterpret_cast<a_dcomplex*>( workl ),
                  lworkl, rwork, info );
    };
    ...
    void spectrum( RC* d, RC* z, a_int nbDim, a_int* iparam )
    {
        ...
        for ( a_int i = 0; d && i < nbConv; i++ )
        {
            ...
            if ( val.size() == (size_t)nbEV )
                break; // If more converged than requested, likely not accurate (check KO).
        }
    ...
    void halfSpectrum( RC* dr, RC* di, RC* z, a_int nbDim, a_int* iparam )
    {
        ...
        a_int nbConv = iparam[4];
        val.reserve( nbConv );
        for ( a_int i = 0; dr && di && i <= nbConv / 2; i++ )
        { // Scan first half of the spectrum.
              ...
              a_int& info )
    {
        ...
        if ( symPb )
        {
            float* d = new float[nbEV];
            for ( a_int k = 0; k < nbEV; k++ )
                d[k] = 0.;

            sseupd_c( rvec, howmny, select, d, z, ldz, sigmaReal, bMat, nbDim, which, nbEV, tol,
                      resid, nbCV, v, ldv, iparam, ipntr, workd, workl, lworkl, &info );
            if ( info == -14 )
                cerr << "Error: sseupd - KO: ssaupd did not find any eigenvalues to sufficient "
                        "accuracy"
                     << endl;
            if ( info < 0 && info != -14 /*-14: don't break*/ )
            {
                cerr << "Error: sseupd - KO with info " << info << endl;
                return 1;
            }
        ...
        {
            float* dr = new float[nbEV + 1];
            for ( a_int k = 0; k < nbEV + 1; k++ )
                dr[k] = 0.;
            float* di = new float[nbEV + 1];
            for ( a_int k = 0; k < nbEV + 1; k++ )
                di[k] = 0.;
            float* workev = new float[3 * nbCV];

            sneupd_c( rvec, howmny, select, dr, di, z, ldz, sigmaReal, sigmaImag, workev, bMat, nbDim,
                      which, nbEV, tol, resid, nbCV, v, ldv, iparam, ipntr, workd, workl, lworkl, &info );
            if ( info == -14 )
                cerr << "Error: sneupd - KO: snaupd did not find any eigenvalues to sufficient "
                        "accuracy"
                     << endl;
            if ( info < 0 && info != -14 /*-14: don't break*/ )
            {
                cerr << "Error: sneupd - KO with info " << info << endl;
                return 1;
            }
              ...
              a_int& info )
    {
        ...
        if ( symPb )
        {
            double* d = new double[nbEV];
            for ( a_int k = 0; k < nbEV; k++ )
                d[k] = 0.;

            dseupd_c( rvec, howmny, select, d, z, ldz, sigmaReal, bMat, nbDim, which, nbEV, tol,
                      resid, nbCV, v, ldv, iparam, ipntr, workd, workl, lworkl, &info );
            if ( info == -14 )
                cerr << "Error: dseupd - KO: dsaupd did not find any eigenvalues to sufficient "
                        "accuracy"
                     << endl;
            if ( info < 0 && info != -14 /*-14: don't break*/ )
            {
                cerr << "Error: dseupd - KO with info " << info << endl;
                return 1;
            }
        ...
        {
            double* dr = new double[nbEV + 1];
            for ( a_int k = 0; k < nbEV + 1; k++ )
                dr[k] = 0.;
            double* di = new double[nbEV + 1];
            for ( a_int k = 0; k < nbEV + 1; k++ )
                di[k] = 0.;
            double* workev = new double[3 * nbCV];

            dneupd_c( rvec, howmny, select, dr, di, z, ldz, sigmaReal, sigmaImag, workev, bMat, nbDim,
                      which, nbEV, tol, resid, nbCV, v, ldv, iparam, ipntr, workd, workl, lworkl, &info );
            if ( info == -14 )
                cerr << "Error: dneupd - KO: dnaupd did not find any eigenvalues to sufficient "
                        "accuracy"
                     << endl;
            if ( info < 0 && info != -14 /*-14: don't break*/ )
            {
                cerr << "Error: dneupd - KO with info " << info << endl;
                return 1;
            }
    ...
    int eupd( a_int rvec,
              ...
              a_int& info )
    {
        complex<float>* d = new complex<float>[nbEV + 1];
        for ( a_int k = 0; k < nbEV + 1; k++ )
            d[k] = complex<float>( 0., 0. );
        complex<float>* workev = new complex<float>[2 * nbCV];

        complex<float> sigma = complex<float>( (float)sigmaReal, (float)sigmaImag );
        cneupd_c( rvec, howmny, select, reinterpret_cast<a_fcomplex*>( d ),
                  reinterpret_cast<a_fcomplex*>( z ), ldz, reinterpret_cast<a_fcomplex&>( sigma ),
                  reinterpret_cast<a_fcomplex*>( workev ), bMat, nbDim, which, nbEV, tol,
                  reinterpret_cast<a_fcomplex*>( resid ), nbCV, reinterpret_cast<a_fcomplex*>( v ),
                  ldv, iparam, ipntr, reinterpret_cast<a_fcomplex*>( workd ),
                  reinterpret_cast<a_fcomplex*>( workl ), lworkl, rwork, &info );
        if ( info == -14 )
            cerr << "Error: cneupd - KO: cnaupd did not find any eigenvalues to sufficient accuracy" << endl;
        if ( info < 0 && info != -14 /*-14: don't break*/ )
        {
            cerr << "Error: cneupd - KO with info " << info << endl;
            return 1;
        }
    ...
    int eupd( a_int rvec,
              ...
              a_int& info )
    {
        complex<double>* d = new complex<double>[nbEV + 1];
        for ( a_int k = 0; k < nbEV + 1; k++ )
            d[k] = complex<double>( 0., 0. );
        complex<double>* workev = new complex<double>[2 * nbCV];

        complex<double> sigma = complex<double>( sigmaReal, sigmaImag );
        zneupd_c( rvec, howmny, select, reinterpret_cast<a_dcomplex*>( d ),
                  reinterpret_cast<a_dcomplex*>( z ), ldz, reinterpret_cast<a_dcomplex&>( sigma ),
                  reinterpret_cast<a_dcomplex*>( workev ), bMat, nbDim, which, nbEV, tol,
                  reinterpret_cast<a_dcomplex*>( resid ), nbCV, reinterpret_cast<a_dcomplex*>( v ),
                  ldv, iparam, ipntr, reinterpret_cast<a_dcomplex*>( workd ),
                  reinterpret_cast<a_dcomplex*>( workl ), lworkl, rwork, &info );
        if ( info == -14 )
            cerr << "Error: zneupd - KO: znaupd did not find any eigenvalues to sufficient accuracy" << endl;
        if ( info < 0 && info != -14 /*-14: don't break*/ )
        {
            cerr << "Error: zneupd - KO with info " << info << endl;
            return 1;
        }
    ...
    int setMode( int const mode, EM const& A, EM const* B, SLV& solver )
    {
        ...
        else if ( mode == 2 || mode == 3 )
        {
            if ( !stdPb && !B )
            {
                cerr << "Error: generalized problem without B" << endl;
                return 1;
            }
        ...
        {
            cerr << "Error: arpack mode must be 1, 2 or 3 - KO" << endl;
            rc = 1;
        }
    ...
    template <typename T>
    int saveSolve( string const& fileName, a_int const& nbDim, T* rv )
    {
    ...
    template <typename T>
    int restartSolve( string const& fileName, a_int const& nbDim, T* rv, bool allowZero = true )
    {
        ...
        if ( ifs.is_open() )
        {
            ...
            if ( nDim != nbDim )
            {
                cerr << "Error: bad dim - restart KO" << endl;
                return 1;
            }
            for ( a_int n = 0; rv && n < nbDim; n++ )
            {
    ...
    int initPointerSize( a_int& iparamSz, a_int& ipntrSz, string const& aeupd )
    {
        ...
        if ( aeupd == "aupd" )
        {
            if ( is_same<RC, double>::value && symPb )
                ipntrSz = 11;
            if ( is_same<RC, float>::value && symPb )
                ipntrSz = 11;
            return 0;
        }
        if ( aeupd == "eupd" )
    ...
    int solve( EM const& A, EM const* B, SLV& solver )
    {
        ...
        if ( rc != 0 )
        {
            cerr << "Error: bad iparam/ipntr initialization for aupd" << endl;
            return rc;
        }
        ...
        for ( a_int n = 0; n < 3 * nbDim; n++ )
            workd[n] = zero; // Avoid "bad" X/Y vector.
        ...
        if ( restartFromFile )
        {
            ...
            if ( rc != 0 )
            {
                cerr << "Error: bad restart (resid)" << endl;
                return rc;
            }
            rc = restartSolve( "arpackSolver.v.out", ldv * nbCV, v );
            if ( rc != 0 )
            {
                cerr << "Error: bad restart (v)" << endl;
                return rc;
            }
        }
        ...
        if ( rc != 0 )
        {
            cerr << "Error: bad arpack mode" << endl;
            return rc;
        }
        ...
        do
        {
            ...
            aupd( &ido, bMat, nbDim, which, resid, v, ldv, iparamAupd.data(), ipntrAupd.data(),
                  workd, workl, lworkl, rwork, &info );
            if ( info == 1 )
                cerr << "Error: [dz][sn]aupd - KO: maximum number of iterations taken. Increase "
                        "--maxIt..."
                     << endl;
            if ( info == 3 )
                cerr << "Error: [dz][sn]aupd - KO: no shifts could be applied. Increase --nbCV..." << endl;
            if ( info == -9 )
                cerr << "Error: [dz][sn]aupd - KO: starting vector is zero. Retry: play with "
                        "shift..."
                     << endl;
            if ( info < 0 )
            {
                cerr << "Error: [dz][sn]aupd - KO with info " << info << ", nbIt " << iparamAupd[2] << endl;
                return 1;
            }
            ...
            else if ( ido == 1 )
            {
                ...
                else if ( iparamAupd[6] == 2 )
                {
                    Y = A * X;
                    if ( symPb )
                        X = Y; // Remark 5 in dsaupd documentation.
                    auto YY = Y; // Use copy of Y (not Y) for solve (avoid potential memory overwrite as Y is both in/out).
                    Y = solver.solve( YY ); // Y = B^-1 * A * X.
                }
                else if ( iparamAupd[6] == 3 )
            ...
            {
                cerr << "Error: unexpected ido " << ido << " - KO" << endl;
                return 1;
            }
        ...
        a_int* select = new a_int[nbCV];
        for ( a_int n = 0; n < nbCV; n++ )
            select[n] = 1;
        a_int const nbZ = nbDim * ( nbEV + 1 ); // Caution: nbEV+1 for dneupd.
        RC* z = new RC[nbZ];
        for ( a_int n = 0; n < nbZ; n++ )
            z[n] = zero;
        a_int ldz = nbDim;
        rc = initPointerSize( iparamSz, ipntrSz, "eupd" );
        if ( rc != 0 )
        {
            cerr << "Error: bad iparam/ipntr initialization for eupd" << endl;
            return rc;
        }
        vector<a_int> iparamEupd( iparamSz, 0 );
        for ( a_int p = 0; p < iparamSz; p++ )
            iparamEupd[p] = iparamAupd[p]; // Initialize eupd parameters with aupd ones.
        vector<a_int> ipntrEupd( ipntrSz, 0 );
        for ( a_int p = 0; p < ipntrSz; p++ )
            ipntrEupd[p] = ipntrAupd[p]; // Initialize eupd parameters with aupd ones.
        rc = eupd( rvec, howmny, select, z, ldz, bMat, nbDim, which, resid, v, ldv,
                   iparamEupd.data(), ipntrEupd.data(), workd, workl, lworkl, rwork, info );
        if ( rc != 0 )
        {
            cerr << "Error: bad arpack eupd" << endl;
            return rc;
        }
    ...
    void makeSigma( complex<float>& sigma )
    {
        sigma = complex<float>( (float)sigmaReal, (float)sigmaImag );
    };
