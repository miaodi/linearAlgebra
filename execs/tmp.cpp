...
void register_solvers()
{
    ...
    utils::singleton<mkl_wrapper::solver_factory>::instance().reg( "gmres+ilut", gmres_ilut );
    ...
    utils::singleton<mkl_wrapper::solver_factory>::instance().reg( "cg+ic0", cg_ic0 );
...
int main( int argc, char** argv )
{
    ...
    std::shared_ptr<MKL_INT[]> k_csr_rows_ptr( k_csr_rows.data(), []( MKL_INT[] ) {} );
    std::shared_ptr<MKL_INT[]> k_csr_cols_ptr( k_csr_cols.data(), []( MKL_INT[] ) {} );
    ...
    std::shared_ptr<MKL_INT[]> m_csr_rows_ptr( m_csr_rows.data(), []( MKL_INT[] ) {} );
    std::shared_ptr<MKL_INT[]> m_csr_cols_ptr( m_csr_cols.data(), []( MKL_INT[] ) {} );
    ...
    utils::ReadFromBinaryCSR( g_mat, g_csr_rows, g_csr_cols, g_csr_vals, SPARSE_INDEX_BASE_ONE );
    for ( int i = g_csr_rows.size(); i < m_csr_rows.size(); i++ )
    {
        g_csr_rows.push_back( g_csr_rows.back() );
    }
    std::shared_ptr<MKL_INT[]> g_csr_rows_ptr( g_csr_rows.data(), []( MKL_INT[] ) {} );
    std::shared_ptr<MKL_INT[]> g_csr_cols_ptr( g_csr_cols.data(), []( MKL_INT[] ) {} );
    ...
    mkl_wrapper::mkl_sparse_mat k( size, size, k_csr_rows_ptr, k_csr_cols_ptr, k_csr_vals_ptr, SPARSE_INDEX_BASE_ONE );
    mkl_wrapper::mkl_sparse_mat m( size, size, m_csr_rows_ptr, m_csr_cols_ptr, m_csr_vals_ptr, SPARSE_INDEX_BASE_ONE );
    // {
    ...
    mkl_wrapper::mkl_sparse_mat g( size, num_ports, g_csr_rows_ptr, g_csr_cols_ptr, g_csr_vals_ptr,
                                   SPARSE_INDEX_BASE_ONE );
    ...
    double max{ 1.28061e+09 }, min{ 26141.9 };
    {
        ...
        ar_gv.which( "L" );
        utils::Elapse<>::execute( "arpack max eigen: ", [&ar_gv, &eigenvalues, &eigenvectors]()
    ...
    }
    {
        ...
        std::vector<double> eigenvectors( 1 * size, 0 );
        ar_gv.which( "L" );
        utils::Elapse<>::execute( "arpack min eigen: ", [&ar_gv, &eigenvalues, &eigenvectors]()
                                  { ar_gv.eigen_solve( eigenvalues.data(), eigenvectors.data() ); } );
        for ( auto i : eigenvalues )
        {
...
#pragma omp parallel num_threads( 1 )
    {
        ...
        auto [start, end] =
            utils::LoadBalancedPartition( select.begin(), select.end(), rank, total_omp_threads );
...
#pragma omp critical
        ...
        for ( size_t f = 0; f < frequencies.size(); f++ )
        {
            ...
            auto solver =
                utils::singleton<mkl_wrapper::solver_factory>::instance().create( "cg+ic0", mat );
            for ( auto it = start; it < end; it++ )
            {
                local_to_global.push_back( ( it - select.begin() ) * frequencies.size() + f );
                *it = 1;
                ...
                vTAI[local_to_global.back() + 1] = *vTAI_local.crbegin() - *( vTAI_local.crbegin() + 1 );
                *it = 0;
#pragma omp critical
                {
                    count += 1;
...
#pragma omp master
        {
            std::inclusive_scan( std::execution::seq, vTAI.get(), vTAI.get() + total_work + 1,
                                 vTAI.get(), std::plus<>() );
...
#pragma omp barrier
        for ( size_t i = 0; i < local_to_global.size(); i++ )
        {
            std::copy( std::execution::seq, vTAJ_local.cbegin() + vTAI_local[i],
                       vTAJ_local.cbegin() + vTAI_local[i + 1], vTAJ.get() + vTAI[local_to_global[i]] );
            std::copy( std::execution::seq, vTAV_local.cbegin() + vTAI_local[i],
                       vTAV_local.cbegin() + vTAI_local[i + 1], vTAV.get() + vTAI[local_to_global[i]] );
        }
    }
