...
/*
*  Content:
*  Intel(R) oneAPI Math Kernel Library (oneMKL) RCI (P)FGMRES ((Preconditioned)
...
int main (void)
{
  ...
  *---------------------------------------------------------------------------*/
  ...
  if (ipar[7])
    {
      printf ("As ipar[7]=" IFORMAT ", the automatic test for the maximal number of ", ipar[7]);
  ...
  if (ipar[8])
    {
      printf ("As ipar[8]=" IFORMAT ", the automatic residual test will be performed\n", ipar[8]);
    }
  else
    {
      printf ("As ipar[8]=" IFORMAT ", the automatic residual test will be skipped\n", ipar[8]);
    }
  printf ("+++\n");
  if (ipar[9])
    {
      printf ("As ipar[9]=" IFORMAT ", the user-defined stopping test will be ", ipar[9]);
      printf ("requested via\nRCI_request=2\n");
    }
  else
    {
      printf ("As ipar[9]=" IFORMAT ", the user-defined stopping test will not be ", ipar[9]);
      printf ("requested, thus,\nRCI_request will not take the value 2\n");
    }
  printf ("+++\n");
  if (ipar[10])
    {
      printf ("As ipar[10]=" IFORMAT ", the Preconditioned FGMRES iterations will be ", ipar[10]);
      printf ("performed, thus,\nthe preconditioner action will be requested via ");
      printf ("RCI_request=3\n");
    }
  else
    {
      printf ("As ipar[10]=" IFORMAT ", the Preconditioned FGMRES iterations will not ", ipar[10]);
      printf ("be performed,\nthus, RCI_request will not take the value 3\n");
    }
  printf ("+++\n");
  if (ipar[11])
    {
      printf ("As ipar[11]=" IFORMAT ", the automatic test for the norm of the next ", ipar[11]);
  ...
  if (RCI_request == 2)
    {
      ...
      /* Compute the current true residual via  Intel oneMKL (Sparse) BLAS routines */
...
COMPLETE:ipar[12] = 0;
  ...
  *---------------------------------------------------------------------------*/
  ...
  for (i = 0; i < N; i++)
    {
      printf ("computed_solution[" IFORMAT "]=", i);
      printf ("%e\n", computed_solution[i]);
    }
  printf ("\n The expected solution is: \n");
  for (i = 0; i < N; i++)
    {
      printf ("expected_solution[" IFORMAT "]=", i);
      printf ("%e\n", expected_solution[i]);
  ...
  if (itercount == expected_itercount && dvar < 1.0e-14)
  ...
  else
    {
      ...
      printf ("from the expected number of iterations " IFORMAT ", ", expected_itercount);
  ...
  /* unless you disable  Intel oneMKL Memory Manager                                   */
  /*-------------------------------------------------------------------------*/
FAILED:printf ("\nThis example FAILED as the solver has returned the ERROR code " IFORMAT, RCI_request);
