...
int main()
{
    ...
    SpMatMap mat_csr( csr_rows.size() - 1, csr_rows.size() - 1, csr_cols.size(), csr_rows.data(),
                      csr_cols.data(), csr_vals.data() );
    ...
    mkl_wrapper::mkl_sparse_mat mkl_mat( csr_rows.size() - 1, csr_rows.size() - 1, csr_rows_ptr,
                                         csr_cols_ptr, csr_vals_ptr );
    ...
    auto gen = [&dist, &mersenne_engine]() { return dist( mersenne_engine ) * 1. / 10000; };
    ...
    std::cout << "m: " << mkl_mat.rows() << " , n: " << mkl_mat.cols() << std::endl;
...

    {
        ...
        prec->set_level( 0 );
        prec->shift( true );
        utils::Elapse<>::execute( "icc k symbolic_factorize: ", [&prec, &mkl_mat_sym]()
                                  { prec->symbolic_factorize( &mkl_mat_sym ); } );
        utils::Elapse<>::execute( "icc k numeric_factorize: ", [&prec, &mkl_mat_sym]()
                                  { prec->numeric_factorize( &mkl_mat_sym ); } );
        ...
        utils::Elapse<>::execute(
            "cg solve: ", [&pcg, &rhs, &x_iter]() { pcg.solve( rhs.data(), x_iter.data() ); } );
    }
...
#ifdef USE_MUMPS_LIB
    {
        ...
        utils::Elapse<>::execute( "mumps factorize: ", [&mumps]() { mumps.factorize(); } );
...
#endif
    {
        ...
        utils::Elapse<>::execute( "pardiso factorize: ", [&pardiso]() { pardiso.factorize(); } );
