...
void writeAdjacencyListText( const COLTYPE rows, ROWTYPE const* ai, COLTYPE const* aj, const std::string& filename )
{
...
int main( int argc, char** argv )
{
    ...
    options.add_options()( "f,filename", "Matrix Market file to read",
        ...
        "l,level", "Level of ILU factorization", cxxopts::value<int>()->default_value( "0" ) )(
        "h,help", "Print usage" );
    ...
    {
        std::cout
            << "   Warning: L factor contains cycles, which is unexpected for a triangular matrix"
            << std::endl;
    }
    std::cout << std::endl;
    ...
    matrix_utils::ParallelTranspose2<int, int, double>( L.rows, L.cols, L.AI(), L.AJ(), L.AV(),
                                                        Lt_ai, Lt_aj, Lt_av );
    ...
    else
    {
        ...
        if ( svg_orig.is_open() )
        {
            matrix_utils::writeSVG( A.rows, A.cols, A.AI(), A.AJ(), svg_orig );
            svg_orig.close();
        ...
        if ( svg_L.is_open() )
        {
            matrix_utils::writeSVG( L.rows, L.cols, L.AI(), L.AJ(), svg_L );
            svg_L.close();
        ...
        if ( svg_Lt.is_open() )
        {
            matrix_utils::writeSVG( L.rows, L.cols, Lt_ai, Lt_aj, svg_Lt );
            svg_Lt.close();
        ...
        if ( svg_tr.is_open() )
        {
            matrix_utils::writeSVG( L.rows, L.cols, tr_ai, tr_aj, svg_tr );
            svg_tr.close();
        ...
        utils::writeAdjacencyGraphDOT( L.rows, Lt_ai, Lt_aj, "Lt_adjacency_graph.dot", "Lt_Graph" );
        ...
        utils::writeAdjacencyGraphDOT( L.rows, tr_ai, tr_aj, "transitive_reduction_graph.dot",
                                       "TR_Graph" );
