...
int main( int argc, char** argv )
{
    ...
    options.add_options()( "f,file", "Input matrix file (Matrix Market format)",
        ...
        "s,size", "Maximum display size for SVG (default: 2000)", cxxopts::value<int>()->default_value( "2000" ) )(
        "t,threads", "Number of threads for A+A^T computation (default: 4)",
        cxxopts::value<int>()->default_value( "4" ) )(
        "tau", "Drop tolerance for ILUM (default: 0, no dropping)",
        cxxopts::value<double>()->default_value( "0.0" ) )( "h,help", "Print help" );
    ...
    if ( ilum_level._ANextDropped.rows > 0 )
    {
        std::cout << "  ANextDropped:      " << ilum_level._ANextDropped.rows << "x"
                  << ilum_level._ANextDropped.cols << ", NNZ: " << ilum_level._ANextDropped.NNZ() << " (reduced by "
                  << ( 100.0 * ( ilum_level._ANext.NNZ() - ilum_level._ANextDropped.NNZ() ) /
                       ilum_level._ANext.NNZ() )
                  << "%)" << std::endl;
    }

    // Generate SVG files for level 1 matrices
    {
        std::string svg_file = output_prefix + "_L1_PAPT.svg";
        std::ofstream svg_out( svg_file );
        if ( svg_out.good() )
        {
            writeSVG( ilum_level._PAPT.rows, ilum_level._PAPT.cols, ilum_level._PAPT.AI(),
                      ilum_level._PAPT.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        std::string svg_file = output_prefix + "_L1_D.svg";
        std::ofstream svg_out( svg_file );
        if ( svg_out.good() )
        {
            writeSVG( ilum_level._D.rows, ilum_level._D.cols, ilum_level._D.AI(),
                      ilum_level._D.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        std::string svg_file = output_prefix + "_L1_F.svg";
        std::ofstream svg_out( svg_file );
        if ( svg_out.good() )
        {
            writeSVG( ilum_level._F.rows, ilum_level._F.cols, ilum_level._F.AI(),
                      ilum_level._F.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        std::string svg_file = output_prefix + "_L1_EDinv.svg";
        std::ofstream svg_out( svg_file );
        if ( svg_out.good() )
        {
            writeSVG( ilum_level._EDinv.rows, ilum_level._EDinv.cols, ilum_level._EDinv.AI(),
                      ilum_level._EDinv.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        std::string svg_file = output_prefix + "_L1_C.svg";
        std::ofstream svg_out( svg_file );
        if ( svg_out.good() )
        {
            writeSVG( ilum_level._C.rows, ilum_level._C.cols, ilum_level._C.AI(),
                      ilum_level._C.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        std::string svg_file = output_prefix + "_L1_ANext.svg";
        std::ofstream svg_out( svg_file );
        if ( svg_out.good() )
        {
            writeSVG( ilum_level._ANext.rows, ilum_level._ANext.cols, ilum_level._ANext.AI(),
                      ilum_level._ANext.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }
    ...
    if ( ilum_level._ANextDropped.rows > 0 )
    {
        std::string svg_file = output_prefix + "_L1_ANextDropped.svg";
        std::ofstream svg_out( svg_file );
        if ( svg_out.good() )
        {
    ...
    std::cout << "  EDinv (E*D^-1):    " << ilum_level2._EDinv.rows << "x"
              << ilum_level2._EDinv.cols << ", NNZ: " << ilum_level2._EDinv.NNZ() << std::endl;
    std::cout << "  C  (bottom-right): " << ilum_level2._C.rows << "x" << ilum_level2._C.cols
              << ", NNZ: " << ilum_level2._C.NNZ() << std::endl;
    std::cout << "  ANext (Schur):     " << ilum_level2._ANext.rows << "x"
              << ilum_level2._ANext.cols << ", NNZ: " << ilum_level2._ANext.NNZ() << std::endl;
    if ( ilum_level2._ANextDropped.rows > 0 )
    {
        std::cout << "  ANextDropped:      " << ilum_level2._ANextDropped.rows << "x"
                  << ilum_level2._ANextDropped.cols << ", NNZ: " << ilum_level2._ANextDropped.NNZ() << " (reduced by "
                  << ( 100.0 * ( ilum_level2._ANext.NNZ() - ilum_level2._ANextDropped.NNZ() ) /
                       ilum_level2._ANext.NNZ() )
                  << "%)" << std::endl;
    }
    ...
    // Generate SVG files for second level matrices
    {
        ...
        if ( svg_out.good() )
        {
            writeSVG( ilum_level2._PAPT.rows, ilum_level2._PAPT.cols, ilum_level2._PAPT.AI(),
                      ilum_level2._PAPT.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        ...
        if ( svg_out.good() )
        {
            writeSVG( ilum_level2._D.rows, ilum_level2._D.cols, ilum_level2._D.AI(),
                      ilum_level2._D.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        ...
        if ( svg_out.good() )
        {
            writeSVG( ilum_level2._F.rows, ilum_level2._F.cols, ilum_level2._F.AI(),
                      ilum_level2._F.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        ...
        if ( svg_out.good() )
        {
            writeSVG( ilum_level2._EDinv.rows, ilum_level2._EDinv.cols, ilum_level2._EDinv.AI(),
                      ilum_level2._EDinv.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        ...
        if ( svg_out.good() )
        {
            writeSVG( ilum_level2._C.rows, ilum_level2._C.cols, ilum_level2._C.AI(),
                      ilum_level2._C.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }

    {
        ...
        if ( svg_out.good() )
        {
            writeSVG( ilum_level2._ANext.rows, ilum_level2._ANext.cols, ilum_level2._ANext.AI(),
                      ilum_level2._ANext.AJ(), svg_out, max_display_size );
            svg_out.close();
        }
    }
    ...
    std::cout << "  2. " << output_prefix << "_L1_PAPT.svg (permuted matrix P*A*P^T)" << std::endl;
    std::cout << "  3. " << output_prefix << "_L1_D.svg (diagonal block)" << std::endl;
    std::cout << "  4. " << output_prefix << "_L1_F.svg (top-right block)" << std::endl;
    std::cout << "  5. " << output_prefix << "_L1_EDinv.svg (E*D^{-1}, bottom-left)" << std::endl;
    std::cout << "  6. " << output_prefix << "_L1_C.svg (bottom-right block)" << std::endl;
    std::cout << "  7. " << output_prefix << "_L1_ANext.svg (Schur complement C-EDinv*F)" << std::endl;
    std::cout << "  8. " << output_prefix
              << "_L1_ANextDropped.svg (after drop tolerance, if tau > 0)" << std::endl;
    std::cout << "\nLevel 2:" << std::endl;
    ...
    std::cout << " 15. " << output_prefix
              << "_L2_ANextDropped.svg (after drop tolerance, if tau > 0)" << std::endl;
