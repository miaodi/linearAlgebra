...
public:
    CudaLUBenchmark(int matrix_size, double sparsity = 0.1,
                    const std::string &l_file = "", const std::string &u_file = "") : n(matrix_size)
    {
        // Initialize cuSPARSE
    ...
    void generateLUMatrices(double sparsity)
    {
        ...
        std::cout << "L matrix: " << h_L.NNZ() << " non-zeros (density: " << std::fixed << std::setprecision(4)
                  << 100.0 * h_L.NNZ() / (double(n) * n) << "%)" << std::endl;
        std::cout << "U matrix: " << h_U.NNZ() << " non-zeros (density: " << std::fixed << std::setprecision(4)
    ...
    void createSparseDescriptors()
    {
        ...
        CHECK_CUSPARSE(cusparseCreateCsr(&matL, n, n, h_L.NNZ(),
                                         d_L_rowPtr, d_L_colInd, d_L_val,
                                         CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                         CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));

        CHECK_CUSPARSE(cusparseCreateCsr(&matU, n, n, h_U.NNZ(),
                                         d_U_rowPtr, d_U_colInd, d_U_val,
                                         CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                         CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
    ...
    void analyzeSpSV()
    {
        ...
        CHECK_CUSPARSE(cusparseSpSV_analysis(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                             &alpha, matL, vecB, vecY, CUDA_R_64F,
                                             CUSPARSE_SPSV_ALG_DEFAULT, spsvDescrL, externalBufferL));
        ...
        CHECK_CUSPARSE(cusparseSpSV_analysis(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                             &alpha, matU, vecY, vecX, CUDA_R_64F,
                                             CUSPARSE_SPSV_ALG_DEFAULT, spsvDescrU, externalBufferU));
    }
    ...
    void analyzeSpSM()
    {
        ...
        CHECK_CUSPARSE(cusparseSpSM_bufferSize(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
        ...
        CHECK_CUSPARSE(cusparseSpSM_analysis(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                             CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matL, matB, matY,
                                             CUDA_R_64F, CUSPARSE_SPSM_ALG_DEFAULT, spsmDescrL, externalBufferSML));
        ...
        CHECK_CUSPARSE(cusparseSpSM_analysis(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                             CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matU, matY, matX,
                                             CUDA_R_64F, CUSPARSE_SPSM_ALG_DEFAULT, spsmDescrU, externalBufferSMU));
    }
    ...
    void cudaForwardBackwardSpSV()
    {
        ...
        CHECK_CUSPARSE(cusparseSpSV_solve(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                          &alpha, matL, vecB, vecY, CUDA_R_64F,
    ...
    void printResults(double gpu_time_ms, const std::string &method)
    {
        ...
        std::cout << "Relative L2 error vs CPU reference: " << std::scientific << std::setprecision(2) << error << std::endl;
    ...
    void printResultsSpSM(double gpu_time_ms)
    {
        ...
        std::cout << "Performance: " << std::setprecision(3) << gflops << " GFLOP/s" << std::endl;
        std::cout << "Relative L2 error vs CPU reference: " << std::scientific << std::setprecision(2) << error << std::endl;
        std::cout << "Implementation: cuSPARSE SpSM (Sparse matrix-dense matrix multiply)" << std::endl;
        std::cout << "L matrix nnz: " << h_L.NNZ() << ", U matrix nnz: " << h_U.NNZ() << std::endl;
    }
};
...
int main(int argc, char **argv)
{
    ...
    options.add_options()("n,size", "Matrix size (ignored if file is provided)",
                          cxxopts::value<int>()->default_value("10000"))("s,sparsity", "Matrix sparsity for generated matrices (0.0-1.0)",
                                                                         cxxopts::value<double>()->default_value("0.05"))("l,lfile", "L matrix Market file path (optional)",
                                                                                                                          cxxopts::value<std::string>()->default_value(""))("u,ufile", "U matrix Market file path (optional)",
                                                                                                                                                                            cxxopts::value<std::string>()->default_value(""))("i,iterations", "Number of benchmark iterations",
                                                                                                                                                                                                                              cxxopts::value<int>()->default_value("1000"))("m,method", "Benchmark method: spsv, spsm, or both",
    ...
    {
        std::cerr << "Error: Both L file (-l) and U file (-u) must be specified together" << std::endl;
        return 1;
    }
