...
void register_solvers()
{
    ...
    utils::singleton<mkl_wrapper::solver_factory>::instance().reg( "cg+incomplete_cholesky_k",
                                                                   cg_incomplete_cholesky_k );
...
int main()
{
    ...
    std::cout << "before permute bandwidth: " << mat.bandwidth()
              << " , after permute bandwidth: " << perm_mat.bandwidth() << std::endl;
...

    {
        ...
        for ( int i = 0; i < iperm.size(); i++ )
        {
            ...
            {
                std::cout << "fucked: " << iperm[i] - 1 << " " << iperm0[i] << std::endl;
            }
            if ( perm[i] - 1 != perm0[i] )
            {
                std::cout << "fucked\n";
