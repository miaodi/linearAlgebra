...
int main()
{
    ...
    matrix_utils::writeMatrixMarket( csr_matrix.rows, csr_matrix.cols, csr_matrix.AI(),
                                     csr_matrix.AJ(), csr_matrix.AV(), out0 );
    out0.close();
    int size = csr_matrix.rows;
    std::vector<int> ai_AAT( size + 1 );
    matrix_utils::APlusATPrefix( size, csr_matrix.AI(), csr_matrix.AJ(), ai_AAT.data() );

    std::vector<int> aj_AAT( ai_AAT.back() );
    std::vector<double> av_AAT( ai_AAT.back(), 1. );
    matrix_utils::APlusATFill( size, csr_matrix.AI(), csr_matrix.AJ(), ai_AAT.data(), aj_AAT.data() );
    std::ofstream out1( "mat_AAT.mtx" );
    matrix_utils::writeMatrixMarket( size, size, ai_AAT.data(), aj_AAT.data(), av_AAT.data(), out1 );
    out1.close();
    ...
    std::cout << "is permutation: " << matrix_utils::isPermutation( size, ai_AAT[0], perm.data() ) << "\n";
    std::vector<int> perm_ai( size + 1 );
    std::vector<int> perm_aj( ai_AAT.back() );
    std::vector<double> perm_av( ai_AAT.back(), 1. );

    matrix_utils::permuteMat<int, int, double>( size, size, perm.data(), iperm.data(),
                                                ai_AAT.data(), aj_AAT.data(), av_AAT.data(),
                                                perm_ai.data(), perm_aj.data(), perm_av.data() );

    std::ofstream out2( "mat_AAT_perm.mtx" );
    matrix_utils::writeMatrixMarket( size, size, perm_ai.data(), perm_aj.data(), perm_av.data(), out2 );
    out2.close();
    ...
    matrix_utils::permuteMat<int, int, double>( size, size, iperm.data(), perm.data(),
                                                perm_ai.data(), perm_aj.data(), perm_av.data(),
                                                ai_AAT.data(), aj_AAT.data(), av_AAT.data() );
    matrix_utils::writeMatrixMarket( size, size, ai_AAT.data(), aj_AAT.data(), av_AAT.data(), out3 );
    out3.close();

    {
        ...
        std::cout << "is permutation: " << matrix_utils::isPermutation( size, ai_AAT[0], perm2.data() ) << "\n";
        ...
        matrix_utils::writeMatrixMarket( size, size, perm_ai.data(), perm_aj.data(), perm_av.data(), out4 );
        out4.close();
    }
}
