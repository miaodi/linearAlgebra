...
void register_solvers()
{
    ...
    utils::singleton<mkl_wrapper::solver_factory>::instance().reg( "gmres+ilu0", gmres_ilu0 );
    ...
    utils::singleton<mkl_wrapper::solver_factory>::instance().reg( "gmres+iluk", gmres_iluk );
    ...
    utils::singleton<mkl_wrapper::solver_factory>::instance().reg( "cg+incomplete_cholesky_k", cg_ic0 );
    ...
    utils::singleton<mkl_wrapper::solver_factory>::instance().reg( "cg+incomplete_cholesky_fm",
                                                                   cg_incomplete_cholesky_fm );
...
std::vector<double> permuteVec( const std::vector<MKL_INT>& iperm, const std::vector<double>& v, const MKL_INT base = 0 )
{
...
int main( int argc, char** argv )
{
    ...
    std::string k_mat( "/remote/tcad25/dimiao/work/examples/xiaopeng_czm/failedMat.bin" );
    ...
    mkl_wrapper::mkl_sparse_mat mat( size, size, k_csr_rows_ptr, k_csr_cols_ptr, k_csr_vals_ptr,
    ...
    matrix_utils::permuteMat( mat.rows(), mat.cols(), iperm.data(), perm.data(), mat.get_ai().get(),
                              mat.get_aj().get(), mat.get_av().get(), ai.get(), aj.get(), av.get() );
    mkl_wrapper::mkl_sparse_mat perm_mat( mat.rows(), mat.cols(), ai, aj, av, SPARSE_INDEX_BASE_ONE );
    std::cout << "bandwidth after rcm reordering: " << perm_mat.bandwidth() << std::endl;

    std::vector<double> rhs( size );
    std::vector<double> x( size );

    std::string rhs_vec( "/remote/tcad25/dimiao/work/examples/xiaopeng_czm/failedRHS.bin" );
    ...
    auto solver = utils::singleton<mkl_wrapper::solver_factory>::instance().create(
