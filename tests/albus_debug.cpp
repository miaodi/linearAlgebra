...
int main() {
  ...
  SerialSPMV spmv;
  spmv(n, base, csr_rows.data(), csr_cols.data(), csr_vals.data(), b.data(),
  ...
  for (int i = 0; i < n; i++) {
    ...
    double rel_err = std::abs(x_serial[i]) > 1e-15 
                         ? abs_err / std::abs(x_serial[i])
                         : abs_err;
  ...
  std::cout << "Row start: " << csr_rows[max_error_row] << ", end: " << csr_rows[max_error_row + 1] << std::endl;
