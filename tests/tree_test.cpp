...
TEST( ParentToChildCSR, GroupsChildrenByParent )
{
    ...
    for ( const int base : { 0, 1 } )
    {
        ...
        const auto nroots = graph::parentToChildCSR<true>(
            nnodes, base, parent.data(), child_offsets.data(), children.data(), roots.data() );
...
TEST( ParentToChildCSR, FillRootsFalseDoesNotWriteRoots )
{
    ...
    const auto nroots = graph::parentToChildCSR<false>(
        nnodes, 0, parent.data(), child_offsets.data(), children.data(), roots.data() );
...
TEST( EliminationTree, AnalyzesParentChildListsAndTopologicalOrder )
{
    ...
    EXPECT_EQ( makeVector( etree.childOffsets(), nnodes + 1 ), std::vector<int>( { 0, 1, 1, 1, 4, 4, 4 } ) );
    EXPECT_EQ( makeVector( etree.children(), 4 ), std::vector<int>( { 5, 1, 2, 4 } ) );
    EXPECT_EQ( makeVector( etree.childCounts(), nnodes ), std::vector<int>( { 1, 0, 0, 3, 0, 0 } ) );
    ...
    for ( int node = 0; node < nnodes; node++ )
    {
        ...
        const auto node_pos = std::find( topo.begin(), topo.end(), node ) - topo.begin();
        const auto parent_pos = std::find( topo.begin(), topo.end(), parent_node ) - topo.begin();
...
TEST( EliminationTree, ComputesParentFromMatrixStructure )
{
    ...
    EXPECT_EQ( makeVector( etree.childOffsets(), nnodes + 1 ), std::vector<int>( { 0, 0, 1, 2 } ) );
    EXPECT_EQ( makeVector( etree.children(), 2 ), std::vector<int>( { 0, 1 } ) );
    EXPECT_EQ( makeVector( etree.topologicalOrder(), nnodes ), std::vector<int>( { 0, 1, 2 } ) );
}
...
TEST( ParentToChildSibling, TemplateRootFillControlsRootWrites )
{
    ...
    const auto nroots_without_fill =
        graph::parentToChildSibling<false>( nnodes, 0, parent.data(), first_child.data(),
                                            next_sibling.data(), roots.data(), child_count.data() );

    EXPECT_EQ( nroots_without_fill, 2 );
    EXPECT_EQ( roots, std::vector<int>( { untouched, untouched } ) );
    EXPECT_EQ( child_count, std::vector<int>( { 1, 0, 0, 3, 0, 0 } ) );

    const auto nroots_with_fill =
        graph::parentToChildSibling<true>( nnodes, 0, parent.data(), first_child.data(),
                                           next_sibling.data(), roots.data(), child_count.data() );
