...
void CreateTestMatrixForDiagonalPrune( COLTYPE rows,
                                       int base,
...
template <typename ROWTYPE, typename COLTYPE, typename VALTYPE>
matrix_utils::CSRMatrixVec<ROWTYPE, COLTYPE, VALTYPE> TileCOOToSortedCSR( COLTYPE rows,
                                                                          COLTYPE cols,
                                                                          ROWTYPE base,
...
TEST( CudaCSRUtils, CSRToTileCOORoundTripFromMatrixMarket )
{
    ...
    for ( int i = 0; i < nnz; ++i )
    {
        ...
        ASSERT_LT( tiled_rows[static_cast<size_t>( i )], rows + base )
            << "row_ind out of range at nnz " << i;
        ASSERT_GE( tiled_cols[static_cast<size_t>( i )], base ) << "col_ind out of range at nnz " << i;
        ASSERT_LT( tiled_cols[static_cast<size_t>( i )], cols + base )
            << "col_ind out of range at nnz " << i;
    }
