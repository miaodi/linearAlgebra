...
TEST( bfs, serial )
{
    ...
    std::vector<int> ajA{ 1, 2, 3, 0, 2, 0, 1, 3, 0, 2, 4, 5, 3, 5,
                          6, 7, 3, 4, 6, 7, 4, 5, 7, 8, 4, 5, 6, 6 };
...
TEST( bfs, serial_vs_parallel )
{
    ...
    for ( const auto& fn : files )
    {
        ...
        for ( int s = 0; s < n; s++ )
        {
            ...
            const std::unordered_set<int> bfs_lastLevel_set( bfs.getLastLevel().begin(),
                                                             bfs.getLastLevel().end() );
            ...
            for ( int t : { 1, 2, 4, 8 } )
            {
                ...
                const std::unordered_set<int> pbfs_lastLevel_set( pbfs.getLastLevel().begin(),
                                                                  pbfs.getLastLevel().end() );
