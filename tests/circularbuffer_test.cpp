...
TEST(circular_buffer, unshift) {
  ...
  EXPECT_TRUE(cb.push_front(1));
  EXPECT_EQ(cb.size(), 1);
  EXPECT_EQ(cb.available(), 4);
  EXPECT_TRUE(cb.push_front(2));
  EXPECT_EQ(cb.size(), 2);
  EXPECT_EQ(cb.available(), 3);
  EXPECT_TRUE(cb.push_front(3));
  EXPECT_EQ(cb.size(), 3);
  EXPECT_EQ(cb.available(), 2);
  EXPECT_TRUE(cb.push_front(4));
  EXPECT_EQ(cb.size(), 4);
  EXPECT_EQ(cb.available(), 1);
  EXPECT_TRUE(cb.push_front(5));
  EXPECT_EQ(cb.size(), 5);
  EXPECT_EQ(cb.available(), 0);
  EXPECT_FALSE(cb.push_front(6));
  EXPECT_EQ(cb.size(), 5);
  EXPECT_EQ(cb.available(), 0);
}
...
TEST(circular_buffer, push) {
  utils::CircularBuffer<int> cb(5);
  EXPECT_TRUE(cb.push_back(1));
  EXPECT_EQ(cb.size(), 1);
  EXPECT_EQ(cb.available(), 4);
  EXPECT_TRUE(cb.push_back(2));
  EXPECT_EQ(cb.size(), 2);
  EXPECT_EQ(cb.available(), 3);
  EXPECT_TRUE(cb.push_back(3));
  EXPECT_EQ(cb.size(), 3);
  EXPECT_EQ(cb.available(), 2);
  EXPECT_TRUE(cb.push_back(4));
  EXPECT_EQ(cb.size(), 4);
  EXPECT_EQ(cb.available(), 1);
  EXPECT_TRUE(cb.push_back(5));
  EXPECT_EQ(cb.size(), 5);
  EXPECT_EQ(cb.available(), 0);
  EXPECT_FALSE(cb.push_back(6));
  EXPECT_EQ(cb.size(), 5);
  EXPECT_EQ(cb.available(), 0);
}
