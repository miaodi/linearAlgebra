...
TEST( transpose_and_partranspose, base0 )
{
    ...
    matrix_utils::SerialTranspose( mat.rows(), mat.cols(), mat.get_ai().get(), mat.get_aj().get(),
                                   mat.get_av().get(), std::get<0>( t_csr ).get(),
                                   std::get<1>( t_csr ).get(), std::get<2>( t_csr ).get() );

    auto tt_csr = matrix_utils::AllocateCSRData( mat.cols(), mat.nnz() );
    matrix_utils::SerialTranspose( mat.cols(), mat.rows(), std::get<0>( t_csr ).get(),
                                   std::get<1>( t_csr ).get(), std::get<2>( t_csr ).get(),
    ...
    matrix_utils::ParallelTranspose( mat.cols(), mat.rows(), std::get<0>( t_csr ).get(),
                                     std::get<1>( t_csr ).get(), std::get<2>( t_csr ).get(),
    ...
    matrix_utils::ParallelTranspose2( mat.rows(), mat.cols(), mat.get_ai().get(), mat.get_aj().get(),
                                      mat.get_av().get(), std::get<0>( pt2_csr ).get(),
...
TEST( transpose_and_partranspose, base1 )
{
    ...
    matrix_utils::ParallelTranspose( mat.rows(), mat.cols(), mat.get_ai().get(), mat.get_aj().get(),
                                     mat.get_av().get(), std::get<0>( pt_csr ).get(),
...
TEST( transpose_and_partranspose, no_av )
{
    ...
    auto t_csr = matrix_utils::AllocateCSRData( mat.cols(), mat.nnz() );
    matrix_utils::SerialTranspose( mat.rows(), mat.cols(), mat.get_ai().get(), mat.get_aj().get(),
                                   (double*)nullptr, std::get<0>( t_csr ).get(),
                                   std::get<1>( t_csr ).get(), std::get<2>( t_csr ).get() );
}
...
TEST( SplitLDU, base0 )
{
    ...
    std::vector<double> D;
    matrix_utils::SplitLDU( mat.rows(), (int)mat.mkl_base(), mat.get_ai().get(), mat.get_aj().get(),
                            mat.get_av().get(), L, D, U );
...
TEST( SplitLDU, base1 )
{
    ...
    matrix_utils::SplitLDU( mat.rows(), (int)mat.mkl_base(), mat.get_ai().get(), mat.get_aj().get(),
                            mat.get_av().get(), L, D, U );
    mkl_wrapper::mkl_sparse_mat matL( mat.rows(), mat.rows(), L.ai, L.aj, L.av, SPARSE_INDEX_BASE_ONE );
    mkl_wrapper::mkl_sparse_mat matU( mat.rows(), mat.rows(), U.ai, U.aj, U.av, SPARSE_INDEX_BASE_ONE );
    auto tmp = mkl_wrapper::mkl_sparse_sum( matU, mat, -1. );
    auto matD = mkl_wrapper::mkl_sparse_sum( matL, tmp, -1. );
    matD.prune( 1e-11 );
...
TEST( LowerTri, small )
{
    ...
    for ( int i = 0; i < 20; i++ )
    {
        ...
        matrix_utils::SplitTriangle<matrix_utils::TriangularMatrix::L>(
            mat.rows(), base, mat.get_ai().get(), mat.get_aj().get(), mat.get_av().get(), T );

        mkl_wrapper::mkl_sparse_mat triangle( mat.rows(), mat.rows(), T.ai, T.aj, T.av, mat.mkl_base() );
        ...
        for ( int i = 0; i < triangle.rows(); i++ )
        {
            for ( int j = triangle.get_ai()[i] - base; j < triangle.get_ai()[i + 1] - base; j++ )
            {
                int mat_j = j - ( triangle.get_ai()[i] - base ) + ( mat.get_ai()[i] - base );
...
TEST( UpperTri, small )
{
    ...
    for ( int i = 0; i < 20; i++ )
    {
        ...
        for ( int i = 0; i < triangle.rows(); i++ )
        {
            for ( int j = 1; j <= triangle.get_ai()[i + 1] - triangle.get_ai()[i]; j++ )
            {
...
TEST( UpperTrigToFull, small )
{
    ...
    for ( int i = 0; i < 20; i++ )
    {
        ...
        matrix_utils::SplitTriangle<matrix_utils::TriangularMatrix::U>(
            mat.rows(), mat.mkl_base(), mat.get_ai().get(), mat.get_aj().get(), mat.get_av().get(), U );
        matrix_utils::TriangularToFull<matrix_utils::TriangularMatrix::U>( U.rows, U.Base(), U.AI(),
                                                                           U.AJ(), U.AV(), F );

        mkl_wrapper::mkl_sparse_mat full( mat.rows(), mat.rows(), F.ai, F.aj, F.av, mat.mkl_base() );
...
TEST( TopologicalSort, L )
{
    ...
    for ( int base = 0; base <= 1; base++ )
    {
        ...
        {
            EXPECT_TRUE( std::is_sorted( L.AJ() + L.AI()[i] - base, L.AJ() + L.AI()[i + 1] - base ) );
        }
        std::vector<std::int32_t> perm( size );
        std::vector<std::int32_t> prefix( size + 1 );
        graph::KahnSerial<std::int32_t, std::int32_t> kahn;
        std::int32_t level = kahn( size, L.AI(), L.AJ(), perm.data(), prefix.data() );
        ...
        graph::KahnParallel<std::int32_t, std::int32_t> kahn_parallel( 5 );
        std::int32_t level_parallel =
            kahn_parallel( size, L.AI(), L.AJ(), perm_parallel.data(), prefix_parallel.data() );
        ...
        std::int32_t level_2 = topSort( size, L.AI(), L.AJ(), perm_2.data(), prefix_2.data() );
        ...
        for ( int i = 0; i < level; i++ )
        {
            ...
            std::sort( perm_2.data() + prefix_2[i] - base, perm_2.data() + prefix_2[i + 1] - base );
            for ( int j = prefix[i] - base; j < prefix[i + 1] - base; j++ )
            {
...
{
    auto expect_level = []( const std::vector<int>& perm, const std::vector<int>& prefix, int base,
                            int level, std::vector<int> expected )
    {
        std::vector<int> actual( perm.begin() + prefix[level] - base, perm.begin() + prefix[level + 1] - base );
    ...
    for ( int base = 0; base <= 1; ++base )
    {
        ...
        const std::vector<int> lower_aj{ base,     base,     base + 1, base,
                                         base + 2, base + 1, base + 2, base + 3 };
        const std::vector<int> upper_ai{ base, base + 3, base + 5, base + 7, base + 8 };
        const std::vector<int> upper_aj{ base,     base + 1, base + 2, base + 1,
                                         base + 3, base + 2, base + 3, base + 3 };
        const std::vector<int> lu_ai{ base, base + 3, base + 6, base + 9, base + 12 };
        const std::vector<int> lu_aj{ base, base + 1, base + 2, base,     base + 1, base + 3,
...
TEST( TopologicalSort, KahnFromSuccessors )
{
    ...
    for ( int base = 0; base <= 1; ++base )
    {
        ...
        const std::vector<int> successor_aj{ base,     base + 1, base + 2, base + 1,
                                             base + 3, base + 2, base + 3, base + 3 };
        ...
        graph::KahnSerial<int, int> kahn;
        EXPECT_EQ( kahn.fromSuccessors( nodes, successor_ai.data(), successor_aj.data(),
                                        in_degree.data(), perm.data(), prefix.data() ),
                   3 );
        EXPECT_EQ( prefix[0], base );
        ...
        graph::KahnParallel<int, int> kahn_parallel( 2 );
        EXPECT_EQ( kahn_parallel.fromSuccessors( nodes, successor_ai.data(), successor_aj.data(),
                                                 in_degree.data(), perm.data(), prefix.data() ),
                   3 );
        EXPECT_EQ( prefix[0], base );
        EXPECT_EQ( prefix[3] - base, nodes );
...
TEST( TopologicalSort, U )
{
    ...
    for ( int base = 0; base <= 1; base++ )
    {
        ...
        for ( int i = 0; i < size; i++ )
        {
            EXPECT_TRUE( std::is_sorted( U.AJ() + U.AI()[i] - base, U.AJ() + U.AI()[i + 1] - base ) );
        }
        std::vector<std::int32_t> perm( size );
        std::vector<std::int32_t> prefix( size + 1 );
        graph::KahnSerial<std::int32_t, std::int32_t> kahn;
        std::int32_t level = kahn.operator()( size, U.AI(), U.AJ(), perm.data(), prefix.data() );
        EXPECT_EQ( prefix[0], base );
        std::map<std::int32_t, std::int32_t> idx_map;
        for ( int i = 0; i < size; i++ )
        {
        ...
        std::int32_t level_2 = topSort.operator()( size, U.AI(), U.AJ(), perm_2.data(), prefix_2.data() );
        EXPECT_EQ( prefix_2[0], base );
        EXPECT_EQ( prefix_2[level_2] - base, size );
        EXPECT_EQ( level, level_2 );
