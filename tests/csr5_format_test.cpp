...
TEST(CSR5ConvertTest, SmallMatrixConversion) {
    ...
    convertCSRtoCSR5<int, int, double, Policy>(
        num_rows, ai.data(), aj.data(), av.data(), csr5_data);
...
TEST(CSR5FormatTest, MemoryEstimation) {
    ...
    // 8 * 128 * (sizeof(int) + sizeof(double)) + 8 * sizeof(uint64_t)
    // = 8 * 128 * 12 + 8 * 8
    // = 12288 + 64 = 12352 bytes
    
    int num_tiles = (nnz + Policy::TILE_SIZE - 1) / Policy::TILE_SIZE;
    size_t expected = num_tiles * Policy::TILE_SIZE * (sizeof(int) + sizeof(double)) +
...
TEST(CSR5ConvertTest, EmptyMatrix) {
    ...
    convertCSRtoCSR5<int, int, double, Policy>(
        num_rows, ai.data(), nullptr, nullptr, csr5_data);
