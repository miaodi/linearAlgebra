...
TEST( triangular_solve_real_spd_Test, solve_cmake_spd_matrices_with_csc_forward_and_csr_backward )
{
    ...
    for ( const auto& matrix_file : spd_matrices )
    {
        ...
        std::copy( A.AI(), A.AI() + n + 1, sparse.outerIndexPtr() );
        std::copy( A.AJ(), A.AJ() + A.NNZ(), sparse.innerIndexPtr() );
        std::copy( A.AV(), A.AV() + A.NNZ(), sparse.valuePtr() );
        sparse.makeCompressed();
        ASSERT_EQ( base, 0 );

        Eigen::SimplicialLLT<SparseMatrixType, Eigen::Lower, Eigen::NaturalOrdering<int>> llt;
        llt.compute( sparse );
        ASSERT_EQ( llt.info(), Eigen::Success );
        Eigen::SparseMatrix<double, Eigen::ColMajor, int> lower_col = llt.matrixL();
        lower_col.makeCompressed();

        std::vector<double> diag( n, 0.0 );
        for ( int col = 0; col < n; ++col )
        {
            for ( int p = lower_col.outerIndexPtr()[col]; p < lower_col.outerIndexPtr()[col + 1]; ++p )
            {
                if ( lower_col.innerIndexPtr()[p] == col )
                {
                    diag[col] = lower_col.valuePtr()[p];
                    break;
                }
            }
            ASSERT_NE( diag[col], 0.0 );
        }

        Eigen::VectorXd expected( n );
        for ( int i = 0; i < n; ++i )
        {
            expected[i] = 1.0 + 0.125 * static_cast<double>( i % 17 );
        }
        const Eigen::VectorXd rhs = sparse * expected;
        std::vector<double> b( rhs.data(), rhs.data() + n );
        std::vector<double> y( n, 0.0 );
        std::vector<double> x( n, 0.0 );

        TriangularSolveCSC<TriangularMatrix::L>(
            n, lower_col.outerIndexPtr(), lower_col.innerIndexPtr(), lower_col.valuePtr(),
            diag.data(), b.data(), y.data() );
        // The CSC columns of L are also the CSR rows of U = L^T.
        TriangularSolve<TriangularMatrix::U>(
            n, lower_col.outerIndexPtr(), lower_col.innerIndexPtr(), lower_col.valuePtr(),
            diag.data(), y.data(), x.data() );

        Eigen::Map<const Eigen::VectorXd> solved( x.data(), n );
        const double relative_residual = ( sparse * solved - rhs ).norm() / rhs.norm();
        EXPECT_LT( relative_residual, 1e-8 );
        const double relative_error = ( solved - expected ).norm() / expected.norm();
        EXPECT_LT( relative_error, 1e-7 );
    }
}
