...
TEST(CudaILUBase, MatchesCPULevel0Ex5)
{
    ...
    const int levels = topological_sort(n, lu_cpu.AI(), lu_cpu.AJ(), level_perm.data(), level_prefix.data());
    ...
    ASSERT_CUDA_OK(cudaMemcpy(d_a_ai, csr_rows.data(), static_cast<size_t>(n + 1) * sizeof(int), cudaMemcpyHostToDevice));
    ASSERT_CUDA_OK(cudaMemcpy(d_a_aj, csr_cols.data(), static_cast<size_t>(nnz) * sizeof(int), cudaMemcpyHostToDevice));
    ASSERT_CUDA_OK(cudaMemcpy(d_a_av, csr_vals.data(), static_cast<size_t>(nnz) * sizeof(double), cudaMemcpyHostToDevice));
    ASSERT_CUDA_OK(cudaMemcpy(d_lu_ai, lu_cpu.AI(), static_cast<size_t>(n + 1) * sizeof(int), cudaMemcpyHostToDevice));
    ASSERT_CUDA_OK(cudaMemcpy(d_lu_aj, lu_cpu.AJ(), static_cast<size_t>(lu_cpu.NNZ()) * sizeof(int), cudaMemcpyHostToDevice));
    ASSERT_CUDA_OK(cudaMemcpy(d_lu_diag, lu_cpu.Diagonal(), static_cast<size_t>(n) * sizeof(int), cudaMemcpyHostToDevice));
    ASSERT_CUDA_OK(cudaMemcpy(d_level_perm, level_perm.data(), static_cast<size_t>(n) * sizeof(int), cudaMemcpyHostToDevice));
    ...
    ASSERT_CUDA_OK(ILUBaseNumericFactorizationAsync<int, int, double>(
        ...
        level_prefix.data(), levels, base, d_lu_av, d_status, stream));
    ...
    ASSERT_CUDA_OK(cudaMemcpy(lu_gpu.data(), d_lu_av, static_cast<size_t>(lu_cpu.NNZ()) * sizeof(double), cudaMemcpyDeviceToHost));
