...
TEST( permutation, is_permutation )
{
    ...
    EXPECT_TRUE( matrix_utils::isPermutation( static_cast<int>( perm.size() ), 0, perm.data() ) );
    perm[0] = 1;
    EXPECT_FALSE( matrix_utils::isPermutation( static_cast<int>( perm.size() ), 0, perm.data() ) );
}
...
TEST( permutation, is_permutation_threads )
{
    ...
    {
        EXPECT_TRUE( matrix_utils::isPermutation( static_cast<int>( perm.size() ), 0, perm.data(), nthreads ) );
    }
    perm[0] = 1;
    for ( int nthreads : { 1, 2, 4, 8 } )
    {
        EXPECT_FALSE( matrix_utils::isPermutation( static_cast<int>( perm.size() ), 0, perm.data(), nthreads ) );
    }
}
...
TEST( permutation, rand_perm )
{
    ...
    for ( int i = 0; i < 100; i++ )
    {
        ...
        EXPECT_TRUE( matrix_utils::isPermutation( static_cast<int>( perm.size() ), 1, perm.data() ) );
        perm[0] += 1;
        EXPECT_FALSE( matrix_utils::isPermutation( static_cast<int>( perm.size() ), 1, perm.data() ) );
    }
}
...
TEST( permutation, inv_perm )
{
    ...
    matrix_utils::invPerm( static_cast<int>( perm.size() ), base, perm.data(), iperm.data() );
...
TEST( permutation, inv_perm_threads )
{
    ...
    for ( int nthreads : { 1, 2, 4, 8 } )
    {
        ...
        matrix_utils::randPerm( static_cast<int>( perm.size() ), base, perm.data() );
        std::vector<int> iperm( perm.size() );
        matrix_utils::invPerm( static_cast<int>( perm.size() ), base, perm.data(), iperm.data(), nthreads );
        for ( size_t i = 0; i < size; i++ )
        {
            EXPECT_EQ( iperm[perm[i]], i );
            EXPECT_EQ( perm[iperm[i]], i );
...
TEST( permutation, perm_vec )
{
    ...
    matrix_utils::permVec( static_cast<int>( perm.size() ), base, vec.data(), perm.data(), perm_vec.data() );
    for ( size_t i = 0; i < size; i++ )
    {
...
TEST( permutation, perm_vec_threads )
{
    ...
    {
        std::vector<int> perm_vec( size );
        matrix_utils::permVec( static_cast<int>( perm.size() ), base, vec.data(), perm.data(),
                               perm_vec.data(), nthreads );
        for ( size_t i = 0; i < size; i++ )
        {
            EXPECT_EQ( perm_vec[i], vec[perm[i] - base] );
        }
...
TEST( permutation, inv_perm_vec_threads )
{
    ...
    for ( int nthreads : { 1, 2, 4, 8 } )
    {
        ...
        matrix_utils::invPermVec( static_cast<int>( perm.size() ), base, vec.data(), perm.data(),
                                  inv_perm_vec.data(), nthreads );
        for ( size_t i = 0; i < size; i++ )
        {
...
TEST( permutation, perm_row_ptr )
{
    ...
    for ( auto base : { 0, 1 } )
    {
        ...
        matrix_utils::permRowPtr( static_cast<int>( perm.size() ), ai.data(), perm.data(), perm_ai.data() );
...
TEST( permutation, perm_row_ptr_threads )
{
    ...
    for ( int nthreads : { 1, 2, 4, 8 } )
    {
        for ( auto base : { 0, 1 } )
        {
            ...
            matrix_utils::randPerm( static_cast<int>( perm.size() ), base, perm.data() );
            std::vector<int> perm_ai( size + 1 );
            matrix_utils::permRowPtr( static_cast<int>( perm.size() ), ai.data(), perm.data(),
                                      perm_ai.data(), nthreads );
            std::vector<int> row_size( size );
            std::vector<int> perm_row_size( size );
            for ( size_t i = 0; i < size; i++ )
            {
