...
TEST_F( precond_Test, icc_level_symbolic_factorize )
{
    for ( auto& mat_info : _mats )
    {
        ...
        matrix_utils::ICCLevelSymbolic1( mat.rows(), U.ai.get(), U.aj.get(), U.ai.get(), lvl, ICC1 );
        mkl_wrapper::mkl_sparse_mat matICC1( mat.rows(), mat.rows(), ICC1.ai, ICC1.aj, ICC1.av, mat.mkl_base() );
        matrix_utils::ICCLevelSymbolic2( mat.rows(), U.ai.get(), U.aj.get(), U.ai.get(), lvl, ICC2 );
        mkl_wrapper::mkl_sparse_mat matICC2( mat.rows(), mat.rows(), ICC2.ai, ICC2.aj, ICC2.av, mat.mkl_base() );
        matrix_utils::ICCLevelSymbolic3( mat.rows(), U.ai.get(), U.aj.get(), U.ai.get(), lvl, ICC3 );
        mkl_wrapper::mkl_sparse_mat matICC3( mat.rows(), mat.rows(), ICC3.ai, ICC3.aj, ICC3.av, mat.mkl_base() );
...
TEST_F( precond_Test, ilu_level_symbolic_parallel_matches_upper )
{
    for ( auto& mat_info : _mats )
    {
        ...
        auto upper_cols = [base]( const CSRMatrix<MKL_INT, MKL_INT, double>& m, MKL_INT row )
        {
        ...
        for ( int lvl = 0; lvl <= 5; ++lvl )
        {
            ...
            ASSERT_TRUE( serial( size, mat.get_ai().get(), mat.get_aj().get(), lvl, ilu_serial ) );
            const bool ok_parallel = parallel( size, mat.get_ai().get(), mat.get_aj().get(), lvl, ilu_parallel );
            if ( !ok_parallel )
            {
                GTEST_SKIP() << "ILULevelSymbolicParallelU not implemented for lvl=" << lvl;
            }
            ...
            for ( MKL_INT row = 0; row < size; ++row )
            {
                ...
                ASSERT_EQ( expected.size(), actual.size() ) << "lvl=" << lvl << " row=" << row;
                for ( size_t i = 0; i < expected.size(); ++i )
                {
                    EXPECT_EQ( expected[i], actual[i] ) << "lvl=" << lvl << " row=" << row << " idx=" << i;
                }
            }
        }
    }
...
TEST_F( precond_Test, ilu_level_symbolic_parallel_matches_lower )
{
    for ( auto& mat_info : _mats )
    {
        ...
        const auto size = mat.rows();
        ILULevelSymbolic<CSRMatrix<MKL_INT, MKL_INT, double>> serial;
        ILULevelSymbolicParallel<CSRMatrix<MKL_INT, MKL_INT, double>, enums::matrix_utils::L> parallel( 4 );

        auto lower_cols = [base]( const CSRMatrix<MKL_INT, MKL_INT, double>& m, MKL_INT row )
        {
            std::vector<MKL_INT> cols;
            for ( auto idx = m.ai[row] - base; idx < m.ai[row + 1] - base; ++idx )
            {
        ...
        for ( int lvl = 0; lvl <= 5; ++lvl )
        {
            ...
            if ( !ok_parallel )
            {
                GTEST_SKIP() << "ILULevelSymbolicParallel not implemented for lvl=" << lvl;
            }
...
TEST_F( precond_Test, ilu_level_symbolic_parallel_lu_includes_diag )
{
    for ( auto& mat_info : _mats )
    {
        ...
        const auto size = mat.rows();
        ILULevelSymbolic<CSRMatrix<MKL_INT, MKL_INT, double>> serial;
        ILULevelSymbolicParallel<CSRMatrix<MKL_INT, MKL_INT, double>, enums::matrix_utils::LU, true> parallel( 4 );
        ...
        auto has_diag = [base]( const CSRMatrix<MKL_INT, MKL_INT, double>& m, MKL_INT row )
        {
            for ( auto idx = m.ai[row] - base; idx < m.ai[row + 1] - base; ++idx )
            {
                auto c = m.aj[idx] - base;
        ...
        for ( int lvl = 0; lvl <= 5; ++lvl )
        {
            ...
            ASSERT_TRUE( serial( size, mat.get_ai().get(), mat.get_aj().get(), lvl, ilu_serial ) );
            ASSERT_TRUE( parallel( size, mat.get_ai().get(), mat.get_aj().get(), lvl, ilu_parallel ) );
            ...
            for ( MKL_INT row = 0; row < size; ++row )
            {
                ...
                EXPECT_EQ( serial_has_diag, parallel_has_diag ) << "lvl=" << lvl << " row=" << row;
                ...
                EXPECT_EQ( ilu_parallel.AJ()[diag_idx], row + base ) << "lvl=" << lvl << " row=" << row;
                auto expected = lower_cols( ilu_serial, row );
                auto actual = lower_cols( ilu_parallel, row );
                ASSERT_EQ( expected.size(), actual.size() ) << "lvl=" << lvl << " row=" << row;
                for ( size_t i = 0; i < expected.size(); ++i )
