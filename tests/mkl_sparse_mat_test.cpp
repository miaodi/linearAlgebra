...
TEST_F( sparse_matrix_Test, mult_mat )
{
    std::shared_ptr<MKL_INT[]> aiC( new MKL_INT[4]{ 0, 3, 6, 9 } );
    std::shared_ptr<MKL_INT[]> ajC( new MKL_INT[9]{ 0, 1, 2, 0, 1, 2, 0, 1, 2 } );
    std::shared_ptr<double[]> avC( new double[9]{ 14, 23, 18, 23, 41, 30, 18, 30, 36 } );
...
{
    mkl_wrapper::mkl_sparse_mat mat( csr_rows.size() - 1, csr_rows.size() - 1, csr_rows, csr_cols, csr_vals );
...
TEST_F( sparse_matrix_Test, pardiso_full_vs_sym )
{
    ...
    // }
    auto solver0 = utils::singleton<mkl_wrapper::solver_factory>::instance().create( "direct", mat );
    solver0->solve( rhs.data(), x0.data() );
    mkl_wrapper::mkl_sparse_mat_sym sym( mat );
    auto solver1 = utils::singleton<mkl_wrapper::solver_factory>::instance().create( "direct", sym );
...
TEST_F( sparse_matrix_Test, mult_full_vs_sym )
{
    ...
    std::shared_ptr<MKL_INT[]> aj( new MKL_INT[11]{ 0, 1, 0, 1, 3, 2, 3, 4, 1, 3, 4 } );
...
TEST_F( sparse_matrix_Test, permute )
{
    ...
    {
        // test zero based
        std::vector<MKL_INT> perm0 = utils::randomPermute( 3, (int)A.mkl_base() ), inv_perm;

        auto [aiB, ajB, avB] = matrix_utils::AllocateCSRData( A.rows(), A.nnz() );
        matrix_utils::permuteMat( A.rows(), A.cols(), perm0.data(), perm0.data(), A.get_ai().get(),
        ...
        matrix_utils::permuteMat( B.rows(), B.cols(), inv_perm.data(), inv_perm.data(), B.get_ai().get(),
        ...
        // test one based
        {
            ...
            matrix_utils::permuteMat( A1.rows(), A1.cols(), perm1.data(), perm1.data(),
                                      A1.get_ai().get(), A1.get_aj().get(), A1.get_av().get(),
                                      aiB.get(), ajB.get(), avB.get() );
            ...
            matrix_utils::permuteMat( B.rows(), B.cols(), inv_perm.data(), inv_perm.data(),
                                      B.get_ai().get(), B.get_aj().get(), B.get_av().get(),
                                      aiC.get(), ajC.get(), avC.get() );
...
TEST_F( sparse_matrix_Test, permute2 )
{
    ...
    {
        // test zero based
        std::vector<MKL_INT> perm0 = utils::randomPermute( 10000, (int)A.mkl_base() ), inv_perm;
        auto [aiB, ajB, avB] = matrix_utils::AllocateCSRData( A1.rows(), A1.nnz() );
        matrix_utils::permuteMat( A.rows(), A.cols(), perm0.data(), perm0.data(), A.get_ai().get(),
                                  A.get_aj().get(), A.get_av().get(), aiB.get(), ajB.get(), avB.get() );
        ...
        // test one based
        {
            ...
            matrix_utils::permuteMat( A1.rows(), A1.cols(), perm1.data(), perm1.data(),
                                      A1.get_ai().get(), A1.get_aj().get(), A1.get_av().get(),
                                      aiB.get(), ajB.get(), avB.get() );
            mkl_wrapper::mkl_sparse_mat B( 10000, 10000, aiB, ajB, avB, SPARSE_INDEX_BASE_ONE );
            ...
            matrix_utils::permuteMat( B.rows(), B.cols(), inv_perm.data(), inv_perm.data(),
                                      B.get_ai().get(), B.get_aj().get(), B.get_av().get(),
                                      aiC.get(), ajC.get(), avC.get() );
            mkl_wrapper::mkl_sparse_mat C( 10000, 10000, aiC, ajC, avC, SPARSE_INDEX_BASE_ONE );
