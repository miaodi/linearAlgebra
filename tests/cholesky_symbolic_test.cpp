...
void expectSamePattern(const CSRMatrixType &expected,
                       const CSRMatrixType &actual) {
  ASSERT_EQ(expected.rows, actual.rows);
  ASSERT_EQ(expected.cols, actual.cols);
  ASSERT_EQ(expected.NNZ(), actual.NNZ());

  const std::vector<int> expected_ai(expected.AI(),
                                     expected.AI() + expected.rows + 1);
  const std::vector<int> actual_ai(actual.AI(), actual.AI() + actual.rows + 1);
  EXPECT_EQ(expected_ai, actual_ai);

  const std::vector<int> expected_aj(expected.AJ(),
                                     expected.AJ() + expected.NNZ());
