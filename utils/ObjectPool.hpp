...
class ObjectPool
{
    ...
    public:
        ...
        void operator()( T* obj )
        {
            ...
            {
                std::default_delete<T>()( obj ); // Fallback to default delete if pool is no longer available
            }
        }
...
public:
    ...
    ptr_type acquire()
    {
        ...
        if ( _objectPrep )
        {
            _objectPrep( obj ); // Prepare the object if a preparation function is provided
        }
    ...
    void setObjectPrep( std::function<void( T* )> prepFunc ) { _objectPrep = prepFunc; }
