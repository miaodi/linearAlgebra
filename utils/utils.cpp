...
template <typename COLTYPE>
void inversePermute( std::vector<COLTYPE>& iperm, const std::vector<COLTYPE>& perm, const COLTYPE base )
{
...
void printEliminationTree( const COLTYPE size, const COLTYPE base, COLTYPE* const parent, const std::string& filename )
{
    ...
    using graph_type =
        boost::adjacency_list<boost::vecS, boost::vecS, boost::directedS, boost::property<boost::vertex_name_t, std::string>>;

    // Get the vertex property map type
    using VertexNameMap = boost::property_map<graph_type, boost::vertex_name_t>::type;
    ...
    for ( COLTYPE i = 0; i < size; i++ )
    {
        if ( parent[i] != i + base )
        { // If not root
            boost::add_edge( boost::vertex( i, graph ), boost::vertex( parent[i] - base, graph ), graph );
        }
    }

    // Write to dot file
    std::ofstream dot( filename );
    boost::write_graphviz( dot, graph, boost::make_label_writer( boost::get( boost::vertex_name, graph ) ) );
}
                             ...
                             const std::string& title )
{
    ...
    std::vector<std::string> colors = { "lightblue", "lightgreen", "lightcoral", "lightyellow",
                                        "lightpink", "lightgray",  "lightcyan",  "wheat",
                                        "lavender",  "mistyrose",  "palegreen",  "peachpuff",
                                        "plum",      "powderblue", "rosybrown",  "sandybrown",
                                        "silver",    "skyblue",    "tan",        "thistle" };
    ...
    for ( COLTYPE i = 0; i < rows; ++i )
    {
        for ( ROWTYPE j = ai[i] - base; j < ai[i + 1] - base; ++j )
        {
            ...
            if ( target < rows && target != i )
            { // Skip self-loops and invalid targets
                ...
                if ( partition != nullptr && num_partitions > 1 )
                {
                    ...
                    else
                    {
                        ...
                        file << "  " << ( i + base ) << " -> " << ( target + base )
                             << " [color=red, penwidth=3, style=bold];\n";
                    }
                }
                else
                {
...
#define INSTANTIATE_UTILS( T )                                                                              \
    ...
    template void inversePermute( std::vector<T>& iperm, const std::vector<T>& perm, const T base );        \
    ...
    ParallelPrefixSum<typename std::vector<T>::iterator, typename std::vector<T>::iterator>(                \
        const int nthreads, typename std::vector<T>::iterator first,                                        \
        typename std::vector<T>::iterator last, typename std::vector<T>::iterator d_first );                \
    template typename std::vector<T>::iterator ParallelPrefixSumInplace<typename std::vector<T>::iterator>( \
        const int nthreads, typename std::vector<T>::iterator first, typename std::vector<T>::iterator last );
