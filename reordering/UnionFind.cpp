...
template <typename T, bool Rank>
void UnionFind<T, Rank>::reset( const T size )
{
...
template <typename ROWTYPE, typename COLTYPE>
void ParUnionFindRem( COLTYPE rows, ROWTYPE const* const ai, COLTYPE const* const aj, COLTYPE* parents, int numthreads )
{
    const COLTYPE base = ai[0];
    std::iota( parents, parents + rows, 0 );
...
DisjointSets::DisjointSets( uint32_t rows ) : mData( rows )
{
...
template <typename ROWTYPE, typename COLTYPE>
void DisjointSets::execute( COLTYPE rows, ROWTYPE const* const ai, COLTYPE const* const aj )
{
    const COLTYPE base = ai[0];
#pragma omp parallel for
    for ( COLTYPE i = 0; i < rows; i++ )
...
#define INSTANTIATE_UNION_FIND_INDEX_TYPE( T )                                                          \
    ...
    template void UnionFindRank<T, T>( T rows, T const* ai, T const* aj, T* parents );                  \
    template void UnionFindRem<T, T>( T rows, T const* ai, T const* aj, T* parents );                   \
    template void ParUnionFindRem<T, T>( T rows, T const* ai, T const* aj, T* parents, int );           \
    template void DisjointSets::execute<T, T>( T rows, T const* ai, T const* aj );                      \
    template T CountComponents<T>( T * parents, T size );                                               \
    template void ComponentsStat<T>( const T* parents, T size, const T base, std::vector<T>& compRoots, \
