...
public:
    ...
    {
        inline bool operator()( const Node* a, const Node* b ) const { return a->id() < b->id(); }
    };
    ...
    struct Node
    {
        ...
        inline int ordering() const { return _elimination_order; }
        void addNeighbor( Node* n );

    protected:
        inline bool isElement() const { return _is_element; }
        ...
        void approxDegreeUpdate( QuotientGraph& graph, NodePtrSet& super_neighbors, int active_matrix_size );
    ...
    inline NodeVector& nodes() { return _nodes; }

    inline Policy policy() const { return _policy; }

    void setPolicy( Policy policy_ );

    inline bool superVaribleEnabled() { return _enable_supervariables; }
