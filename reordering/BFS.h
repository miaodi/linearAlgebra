...
{
public:
    using FN = typename std::function<
        bool( COLTYPE, ROWTYPE const*, COLTYPE const*, VALTYPE const*, COLTYPE, COLTYPE, COLTYPE&, COLTYPE&, std::vector<COLTYPE>&, std::vector<COLTYPE>& )>;

    BFS( FN fn ) : _fn{ fn } {}
    template <bool LASTLEVEL = false>
    bool operator()( const COLTYPE rows, ROWTYPE const* const ai, COLTYPE const* const aj, VALTYPE const* const av, const COLTYPE source )
    {
        return _fn( rows, ai, aj, av, source, _shortCut, _height, _width, _levels, _lastLevel );
    }
...
template <bool LASTLEVEL = false, typename ROWTYPE = int, typename COLTYPE = int, typename VALTYPE = double>
...
template <bool LASTLEVEL = false, bool RECORDLEVEL = true, typename ROWTYPE = int, typename COLTYPE = int, typename VALTYPE = double>
