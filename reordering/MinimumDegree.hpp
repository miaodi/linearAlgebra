...
class QuotientGraph {
  struct Node {
    ...
    std::conditional_t<MD == MinimumDegree::Approximate, COLTYPE,
                       std::monostate>
...
public:
  ...
  mutable reordering::UnionFind<COLTYPE, false>
      _union_find; // union-find structure
  std::map<COLTYPE,
           typename utils::ObjectPool<utils::CircularBuffer<COLTYPE>>::ptr_type>
      _degree_to_principle; // map variable to element

  utils::ObjectPool<std::vector<COLTYPE>>
      _pool; // object pool of vectors of size nnodes
  utils::ObjectPool<utils::CircularBuffer<COLTYPE>>
...
template <typename COLTYPE>
void vectorSubtract(std::vector<COLTYPE> &op1, const std::vector<COLTYPE> &op2,
                    std::optional<COLTYPE> disregard_value = std::nullopt) {
  auto it1 = op1.begin();
...
template <typename COLTYPE>
COLTYPE vectorSubtractSize(const std::vector<COLTYPE> &op1,
