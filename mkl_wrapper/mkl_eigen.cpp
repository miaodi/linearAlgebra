...
{
mkl_eigen_sparse_d_gv::mkl_eigen_sparse_d_gv( mkl_sparse_mat* A, mkl_sparse_mat* B )
    : mkl_eigen( A, B )
{
    _tol = 6; // tolerance = 1e-{_tol}+1
}

bool mkl_eigen_sparse_d_gv::eigen_solve( double* eigenValues, double* eigenVectors )
{
    _pm[1] = _tol;
    ...
    if ( _B )
    {
        info = mkl_sparse_d_gv( &_which[0], _pm, _A->mkl_handler(), _A->mkl_descr(), _B->mkl_handler(),
                                _B->mkl_descr(), _nev, &_num_found, eigenValues, eigenVectors, res.data() );
    }
    else
    {
        info = mkl_sparse_d_ev( &_which[0], _pm, _A->mkl_handler(), _A->mkl_descr(), _nev,
...
power_sparse_gv::power_sparse_gv( mkl_sparse_mat* A, mkl_sparse_mat* B ) : mkl_eigen( A, B ) {}
