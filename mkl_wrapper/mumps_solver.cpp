...
mumps_solver::mumps_solver( mkl_sparse_mat const* A ) : mkl_solver()
{
    ...
    {
        for ( int j = A->get_ai()[i] - A->mkl_base(); j < A->get_ai()[i + 1] - A->mkl_base(); j++ )
        {
            row_ptr[j] = i + 1;
...
bool mumps_solver::factorize()
{
    ...
    if ( id.MUMPS_INFOG( 1 ) != 0 )
    {
        std::cerr << "WARNING mumps_solver Analysis "
                  << " Error " << id.MUMPS_INFOG( 1 ) << " returned in substitution dmumps()\n";
        return false;
    }
        ...
        while ( true )
        {
            ...
            if ( id.MUMPS_INFOG( 1 ) < 0 )
            {
                if ( id.MUMPS_INFOG( 1 ) == -8 || id.MUMPS_INFOG( 1 ) == -9 )
                {
                    ...
                    {
                        std::cerr << "Memory relaxation limit reached for MUMPS factorization\n";
                        return false;
                    }
...
void mumps_solver::set_params()
{
    ...
    id.MUMPS_ICNTL( 33 ) = 0; // the determinant of the input matrix is not computed.
}
