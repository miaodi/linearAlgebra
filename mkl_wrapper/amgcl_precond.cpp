...
bool amgcl_precond::symbolic_factorize( mkl_sparse_mat const* const A )
{
    ...
    _amgclPrecond.reset(
        new amgcl::amg<amgcl::backend::builtin<double>, amgcl::coarsening::smoothed_aggregation, amgcl::relaxation::spai1>(
            std::make_tuple( n, amgcl::make_iterator_range( _ai.get(), _ai.get() + n + 1 ),
...
bool amgcl_precond::numeric_factorize( mkl_sparse_mat const* const A )
{
    ...
    std::transform( std::execution::par_unseq, ai.get(), ai.get() + _nrow + 1, _ai.get(),
                    [base]( const MKL_INT n ) { return n - base; } );

    std::transform( std::execution::par_unseq, aj.get(), aj.get() + _nnz, _aj.get(),
                    [base]( const MKL_INT n ) { return n - base; } );
    ...
    //               amgcl::make_iterator_range(_aj.get(), _aj.get() + _nnz),
    //               amgcl::make_iterator_range(_av.get(), _av.get() + _nnz))));
    _amgclPrecond->rebuild(
        std::make_tuple( _nrow, amgcl::make_iterator_range( _ai.get(), _ai.get() + _nrow + 1 ),
                         amgcl::make_iterator_range( _aj.get(), _aj.get() + _nnz ),
                         amgcl::make_iterator_range( _av.get(), _av.get() + _nnz ) ) );
    return true;
}
