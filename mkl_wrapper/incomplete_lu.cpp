...
bool incomplete_lu_base::solve( double const* const b, double* const x )
{
    ...
    _mkl_descr.mode = SPARSE_FILL_MODE_LOWER;
    _mkl_descr.diag = SPARSE_DIAG_UNIT;
    _mkl_stat = mkl_sparse_d_trsv( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, _mkl_mat, _mkl_descr, b,
                                   _interm_vec.data() );

    _mkl_descr.mode = SPARSE_FILL_MODE_UPPER;
    _mkl_descr.diag = SPARSE_DIAG_NON_UNIT;
    _mkl_stat = mkl_sparse_d_trsv( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, _mkl_mat, _mkl_descr,
                                   _interm_vec.data(), x );
    return true;
}
...
bool mkl_ilu0::symbolic_factorize( mkl_sparse_mat const* const A )
{
    ...
    std::copy( std::execution::seq, A->get_ai().get(), A->get_ai().get() + _nrow + 1, _ai.get() );
...
bool mkl_ilu0::numeric_factorize( mkl_sparse_mat const* const A )
{
    ...
    MKL_INT ierr = 0;
    dcsrilu0( &_nrow, A->get_av().get(), _ai.get(), _aj.get(), _av.get(), ipar, dpar, &ierr );
...
bool mkl_ilut::numeric_factorize( mkl_sparse_mat const* const A )
{
    ...
    dcsrilut( &_nrow, A->get_av().get(), Aai.get(), Aaj.get(), _av.get(), _ai.get(), _aj.get(),
              &_tau, &_max_fill, ipar, dpar, &ierr );
    if ( ierr != 0 )
        return false;
...
bool incomplete_lu_k::symbolic_factorize( mkl_sparse_mat const* const A )
{
    ...
    if ( _level == 0 )
    {
        ...
        _aj.reset( new MKL_INT[_nnz] );
        _av.reset( new double[_nnz] );
        std::copy( std::execution::par_unseq, A->get_ai().get(), A->get_ai().get() + n + 1, _ai.get() );
        std::copy( std::execution::par_unseq, A->get_aj().get(), A->get_aj().get() + _nnz, _aj.get() );
    }
    else
    {
        _ai[0] = base;
        ...
        for ( MKL_INT i = 0; i < n; i++ )
        {
            ...
            auto mid = std::upper_bound( _aj.get() + _ai[i] - base, _aj.get() + _ai[i + 1] - base, i + base );
...
bool incomplete_lu_k::numeric_factorize( mkl_sparse_mat const* const A )
{
    ...
    for ( MKL_INT i = 0; i < n; i++ )
    {
        for ( k_idx = _ai[i] - base, A_k_idx = ai[i] - base; k_idx != _ai[i + 1] - base; k_idx++ )
        {
        ...
        for ( k_idx = _ai[i] - base; _aj[k_idx] - base != i && k_idx != _ai[i + 1] - base; k_idx++ )
        {
            k = _aj[k_idx] - base;
            _av[k_idx] /= diag[k];
