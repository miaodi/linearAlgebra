...
bool mkl_direct_solver::factorize()
{
    ...
    pardiso( _pt, &_maxfct, &_mnum, &_mtype, &phase, &n, _A->get_av().get(), _A->get_ai().get(),
             _A->get_aj().get(), NULL, &nrhs, _iparm, &_msglvl, NULL, NULL, &error );
...
bool mkl_direct_solver::solve( double const* const b, double* const x )
{
    ...
    pardiso( _pt, &_maxfct, &_mnum, &_mtype, &phase, &n, _A->get_av().get(), _A->get_ai().get(),
             _A->get_aj().get(), NULL, &nrhs, _iparm, &_msglvl, const_cast<double* const>( b ), x, &error );
...
bool mkl_direct_solver::forward_substitution( double const* const b, double* const x )
{
    MKL_INT phase = 331;
    MKL_INT error = 0;
...
bool mkl_direct_solver::backward_substitution( double const* const b, double* const x )
{
    MKL_INT phase = 333;
    MKL_INT error = 0;
        ...
        case 2: // then do the user-defined stopping test
        {
            ...
            if ( _print_level == 1 )
                fprintf( stderr, "%3d = %lg/%lg=%lg, %lg (%lg)\n", ipar[3], residual, residual0,
                         residual / residual0, dpar[3], dpar[6] );
...
bool mkl_fgmres_solver::solve( double const* const b, double* const x )
{
    ...
    // set the desired parameters:
    ipar[4] = _maxiter; // max nr of iterations
    ipar[6] = 0;        // do not output warning message. Note dfgmres_check generate false
                        // warnings
    ...
    ipar[9] = 1;                                            // use user defined stopping test
    ipar[10] = _P ? 1 : 0;                                  // preconditioning
    ipar[14] = _num_restart != 0 ? _num_restart : _maxiter; // number of non-restarted iterations
    ...
    {
        // compute the solution by RCI
        dfgmres( &n, x, const_cast<double* const>( b ), &rci_request, ipar, dpar, ptmp );
        ...
        case 2: // then do the user-defined stopping test
        {
            residual = dpar[4];
            if ( _print_level == 1 )
                fprintf( stderr, "%3d = %lg/%lg=%lg\n", ipar[3], residual, residual0, residual / residual0 );
            if ( residual / residual0 < _rel_tol || residual < _abs_tol )
            {
                bsuccess = true;
                bdone = true;
    ...
    // get convergence information
    ipar[12] = 0;
    dfgmres_get( &n, x, const_cast<double* const>( b ), &rci_request, ipar, dpar, ptmp, &niter );
    // if (_print_level > 0) {
...
solver_factory::solver_ptr solver_factory::create( const std::string& name, mkl_sparse_mat& m )
{
    if ( auto it = _methods.find( name ); it != _methods.end() )
        return it->second( m );
    return nullptr;
