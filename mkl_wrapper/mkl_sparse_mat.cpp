...
mkl_sparse_mat::mkl_sparse_mat( const MKL_INT row, const MKL_INT col, const MKL_INT nnz )
{
    _nrow = row;
    _ncol = col;
    _nnz = nnz;
...
mkl_sparse_mat::mkl_sparse_mat( sparse_matrix_t mkl_mat )
{
    ...
    _mkl_stat = mkl_sparse_d_export_csr( mkl_mat, &_mkl_base, &_nrow, &_ncol, &rows_start,
...
mkl_sparse_mat::mkl_sparse_mat( const MKL_INT row,
                                const MKL_INT col,
...
void mkl_sparse_mat::get_adjacency_graph( std::vector<MKL_INT>& xadj, std::vector<MKL_INT>& adjncy ) const
{
    xadj.resize( rows() + 1 );
    ...
    bool has_diag = matrix_utils::CSRToMetisGraph( rows(), _ai.get(), _aj.get(), xadj.data(),
                                                   adjncy.data(), nthreads );
...
void mkl_sparse_mat::sp_fill()
{
    ...
    _mkl_stat = mkl_sparse_d_create_csr( &_mkl_mat, _mkl_base, _nrow, _ncol, _ai.get(),
                                         _ai.get() + 1, _aj.get(), _av.get() );
    if ( _mkl_stat != SPARSE_STATUS_SUCCESS )
    {
        std::cerr << "mkl_sparse_mat::sp_fill(): Matrix is not created, state:" << _mkl_stat << std::endl;
    }
...
void mkl_sparse_mat::optimize()
{
    ...
    mkl_sparse_set_mv_hint( _mkl_mat, SPARSE_OPERATION_NON_TRANSPOSE, _mkl_descr, 1000 );
...
{
    _mkl_stat = mkl_sparse_d_mv( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, _mkl_mat, _mkl_descr, b, 0.0, x );
    return _mkl_stat == SPARSE_STATUS_SUCCESS;
}

bool mkl_sparse_mat::transpose_mult_vec( double const* const b, double* const x )
{
    _mkl_stat = mkl_sparse_d_mv( SPARSE_OPERATION_TRANSPOSE, 1.0, _mkl_mat, _mkl_descr, b, 0.0, x );
    return _mkl_stat == SPARSE_STATUS_SUCCESS;
}

void mkl_sparse_mat::print() const
{
    std::cout << "nrow: " << _nrow << " ncol: " << _ncol << " nnz: " << _nnz << std::endl;
    ...
    for ( MKL_INT i = 0; i < _nrow; i++ )
    {
        for ( MKL_INT j = _ai[i] - (MKL_INT)_mkl_base; j < _ai[i + 1] - (MKL_INT)_mkl_base; j++ )
            std::cout << _aj[j] << " ";
        std::cout << std::endl;
    }
    // for (MKL_INT i = 0; i < _nnz; i++) {
...
int mkl_sparse_mat::check() const
{
    ...
    printf( "Matrix check details: (" IFORMAT ", " IFORMAT ", " IFORMAT ")\n", pt.check_result[0],
            pt.check_result[1], pt.check_result[2] );
...
void mkl_sparse_mat::transpose()
{
    ...
    matrix_utils::ParallelTranspose2( _nrow, _ncol, _ai.get(), _aj.get(), _av.get(),
...
#pragma omp parallel reduction( max : bw )
    {
        ...
        auto [start, end] = utils::LoadBalancedPartition( _ai.get(), _ai.get() + rows(), tid, nthreads );
...
void mkl_sparse_mat::print_svg( std::ostream& out ) const
{
    ...
    for ( MKL_INT j = 0; j < _nrow; ++j )
    {
        ...
        for ( MKL_INT i = previousStart; i < end; ++i )
        {
            out << "  <rect class=\"pixel\" x=\"" << _aj[i] - base + 1 << "\" y=\"" << j + 1
                << "\" width=\".9\" height=\".9\"/>\n";
        }
    }
    out << "</svg>" << std::endl;
...
{
    diag.resize( _nrow );
    bool success = matrix_utils::Diagonal( _nrow, _ai.get(), _aj.get(), _av.get(), diag.data(),
                                           static_cast<double*>( nullptr ) );
    std::for_each( std::execution::par, diag.begin(), diag.end(), [&]( auto& d ) { d -= _mkl_base; } );
    return success;
}
...
mkl_sparse_mat mkl_sparse_sum( const mkl_sparse_mat& A, const mkl_sparse_mat& B, double c )
{
    ...
    auto status =
        mkl_sparse_d_add( SPARSE_OPERATION_NON_TRANSPOSE, A.mkl_handler(), c, B.mkl_handler(), &result );
...
// opA(A)*B
mkl_sparse_mat mkl_sparse_mult( const mkl_sparse_mat& A,
                                const mkl_sparse_mat& B,
                                ...
                                const sparse_operation_t opB )
{
    ...
    sparse_matrix_t result;
    auto status = mkl_sparse_sp2m( opA, A.mkl_descr(), A.mkl_handler(), opB, B.mkl_descr(),
    ...
    if ( status != SPARSE_STATUS_SUCCESS )
    {
        std::cerr << "mkl_sparse_sp2m mkl_sparse_order failed, code: " << status << std::endl;
        return mkl_sparse_mat();
    }
    auto res = mkl_sparse_mat( result );
    mkl_sparse_destroy( result );
...
mkl_sparse_mat mkl_sparse_mult_papt( mkl_sparse_mat& A, mkl_sparse_mat& P )
{
    ...
    return mkl_sparse_mult( PA, P, SPARSE_OPERATION_NON_TRANSPOSE, SPARSE_OPERATION_TRANSPOSE );
}

// c*A+B
mkl_sparse_mat_sym mkl_sparse_sum( const mkl_sparse_mat_sym& A, const mkl_sparse_mat_sym& B, double c )
{
    auto sum = mkl_sparse_sum( (const mkl_sparse_mat&)A, (const mkl_sparse_mat&)B, c );
    return mkl_sparse_mat_sym( sum.rows(), sum.cols(), sum.get_ai(), sum.get_aj(), sum.get_av(),
                               sum.mkl_base() );
}

// PT*A*P
mkl_sparse_mat_sym mkl_sparse_mult_ptap( mkl_sparse_mat_sym& A, mkl_sparse_mat& P )
{
    sparse_matrix_t result;
    auto status = mkl_sparse_sypr( SPARSE_OPERATION_TRANSPOSE, P.mkl_handler(), A.mkl_handler(),
                                   A.mkl_descr(), &result, SPARSE_STAGE_FULL_MULT );
...
{
    sparse_matrix_t result;
    auto status = mkl_sparse_sypr( SPARSE_OPERATION_NON_TRANSPOSE, P.mkl_handler(), A.mkl_handler(),
                                   A.mkl_descr(), &result, SPARSE_STAGE_FULL_MULT );
...
mkl_sparse_mat_sym::mkl_sparse_mat_sym( const mkl_sparse_mat& A ) : mkl_sparse_mat()
{
    ...
    matrix_utils::SplitTriangle<matrix_utils::TriangularMatrix::U>( _nrow, _mkl_base, ai.get(),
                                                                    aj.get(), av.get(), csr );
...
mkl_sparse_mat_sym::mkl_sparse_mat_sym( const mkl_sparse_mat_sym& A ) : mkl_sparse_mat( A )
{
    sp_fill();
}

mkl_sparse_mat_sym::mkl_sparse_mat_sym( sparse_matrix_t mkl_mat ) : mkl_sparse_mat( mkl_mat )
{
    sp_fill();
}

mkl_sparse_mat_sym::mkl_sparse_mat_sym( const MKL_INT row,
                                        const MKL_INT col,
                                        const std::shared_ptr<MKL_INT[]>& ai,
                                        const std::shared_ptr<MKL_INT[]>& aj,
                                        const std::shared_ptr<double[]>& av,
...
mkl_sparse_mat_diag::mkl_sparse_mat_diag( const MKL_INT size, const double val ) : mkl_sparse_mat()
{
    _nrow = size;
    _ncol = size;
    _nnz = size;
...
bool dense_product( const dense_mat& A, const dense_mat& B, dense_mat& C, const CBLAS_TRANSPOSE opA, const CBLAS_TRANSPOSE opB )
{
    if ( opA == CblasNoTrans )
    {
        if ( opB == CblasNoTrans )
    ...
    cblas_dgemm( CblasColMajor, opA, opB, C.rows(), C.cols(),
                 opA == CblasNoTrans ? A.cols() : A.rows(), 1., A.get_av().get(), A.rows(),
                 B.get_av().get(), B.rows(), 0., C.get_av().get(), C.rows() );
    return true;
}

bool mkl_sparse_dense_mat_prod( const mkl_sparse_mat& A, const dense_mat& B, dense_mat& C, const sparse_operation_t opA )
{
    ...
    C.resize( opA == SPARSE_OPERATION_NON_TRANSPOSE ? A.rows() : A.cols(), B.cols() );
    auto state = mkl_sparse_d_mm( opA, 1., A.mkl_handler(), A.mkl_descr(), SPARSE_LAYOUT_COLUMN_MAJOR,
...
mkl_sparse_mat random_sparse( const MKL_INT row, const MKL_INT nnzRow )
{
    ...
    matrix_utils::RandomCSR<MKL_INT, MKL_INT, double>( row, row, ai.get(), aj.get(), av.get() );
    return res;
}
} // namespace mkl_wrapper
