...
std::function<bool( double* const b, double* const x )> arpack_gv::op( const int mode )
{
    ...
    case 2:
    {
        _solver = utils::singleton<solver_factory>::instance().create( "direct", *_B );
        tmp.resize( _A->rows() );
        return [this]( double* const b, double* const x ) -> bool
        {
    ...
    case 3:
    {
        ...
        _solver = utils::singleton<solver_factory>::instance().create( "direct", _op );
        tmp.resize( _A->rows() );
        return [this]( double* const b, double* const x ) -> bool
        {
...
bool arpack_gv::eigen_solve( double* eigenValues, double* eigenVectors )
{
    ...
    iparam[0] = 1; // exact shifts with respect to the reduced tridiagonal matrix T.
    ...
    std::vector<double> workd( 3 * n, 0. ); // Distributed array to be used in the basic Arnoldi iteration
    ...
    do
    {
        dsaupd_c( &ido, &bMat, n, _which.c_str(), _nev, _tol, resid.data(), _ncv, v.data(), ldv,
        ...
        if ( info < 0 )
        {
            std::cerr << "Error: dsaupd with info " << info << ", nbIt " << iparam[2] << std::endl;
            return false;
        }
    ...
    dseupd_c( rvec, &howmny, select.data(), eigenValues, eigenVectors, n * _nev, _shift, &bMat, n,
              _which.c_str(), _nev, _tol, resid.data(), _ncv, v.data(), v.size(), iparam.data(),
