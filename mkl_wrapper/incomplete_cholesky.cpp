...
{
    mkl_sparse_d_trsv( SPARSE_OPERATION_TRANSPOSE, 1.0, _mkl_mat, _mkl_descr, b, _interm_vec.data() );

    mkl_sparse_d_trsv( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, _mkl_mat, _mkl_descr, _interm_vec.data(), x );
...
void incomplete_choleksy_base::optimize()
{
    ...
    // _mkl_descr,
    //                        1000);
    mkl_sparse_set_sv_hint( _mkl_mat, SPARSE_OPERATION_NON_TRANSPOSE, _mkl_descr, 1000 );
    mkl_sparse_set_sv_hint( _mkl_mat, SPARSE_OPERATION_TRANSPOSE, _mkl_descr, 1000 );
...
void incomplete_choleksy_base::aij_update( IDX _ai,
                                           IDX _aj,
                                           VAL _av,
                                           MKL_INT j_idx,
                                           MKL_INT k,
...
bool incomplete_cholesky_k::symbolic_factorize( mkl_sparse_mat const* const A )
{
    ...
    if ( _level == 0 && sym )
    {
        ...
        std::copy( std::execution::seq, A->get_ai().get(), A->get_ai().get() + n + 1, _ai.get() );
        std::copy( std::execution::seq, A->get_aj().get(), A->get_aj().get() + _nnz, _aj.get() );
    }
    else
    {
        _ai[0] = base;
...
#ifdef USE_BOOST_LIB
        std::forward_list<std::pair<MKL_INT, MKL_INT>, boost::fast_pool_allocator<std::pair<MKL_INT, MKL_INT>>> _rowLevels;
#else
        ...
        for ( MKL_INT i = 0; i < n; i++ )
        {
            ...
            if ( _ai[i + 1] - base > aj_size )
            {
                ...
                std::copy( std::execution::seq, _aj.get(), _aj.get() + aj_size, new_aj.get() );
                std::copy( std::execution::seq, av_levels.get(), av_levels.get() + aj_size,
                           new_levels.get() );
                std::swap( new_aj, _aj );
...
incomplete_cholesky_k_2::incomplete_cholesky_k_2() : incomplete_choleksy_base() {}

bool incomplete_cholesky_k_2::symbolic_factorize( mkl_sparse_mat const* const A )
{
    _nrow = A->rows();
    _ncol = A->cols();
    _mkl_base = A->mkl_base();
    ...
    bool success = matrix_utils::Diagonal( _nrow, ai.get(), aj.get(), _av.get(), _diagPos.data(),
    ...
    matrix_utils::ICCLevelSymbolic1( rows(), ai.get(), aj.get(), _diagPos.data(), _level, ICC );
    _ai = ICC.ai;
    _aj = ICC.aj;
    _av = ICC.av;
    return true;
...
bool incomplete_cholesky_fm::symbolic_factorize( mkl_sparse_mat const* const A )
{
    ...
    if ( !A->diag_pos( _diagPos ) )
        return false;
    const MKL_INT l_tails = std::min( _lsize * ( _lsize + 1 ) / 2, _nrow * ( _nrow + 1 ) / 2 );
    const MKL_INT l_capacity = sym ? ( A->nnz() + _nrow * _lsize - l_tails )
    ...
    if ( _rsize )
    {
        ...
        const MKL_INT r_tails = std::min( _rsize * ( _rsize + 1 ) / 2, _nrow * ( _nrow + 1 ) / 2 );
...
bool incomplete_cholesky_fm::numeric_factorize( mkl_sparse_mat const* const A )
{
    ...
    auto compMax = []( const std::pair<MKL_INT, double>& v1, const std::pair<MKL_INT, double>& v2 )
    { return std::abs( v1.second ) > std::abs( v2.second ); };
    auto compMin = []( const std::pair<MKL_INT, double>& v1, const std::pair<MKL_INT, double>& v2 )
    { return std::abs( v1.second ) < std::abs( v2.second ); };

    utils::MaxHeap<std::pair<MKL_INT, double>, decltype( compMax )> max_heap_L( compMax );
    utils::MaxHeap<std::pair<MKL_INT, double>, decltype( compMax )> max_heap_R( compMax );
    utils::MaxHeap<std::pair<MKL_INT, double>, decltype( compMin )> min_heap( compMin );
...
#ifdef USE_BOOST_LIB
    std::forward_list<std::pair<MKL_INT, double>, boost::fast_pool_allocator<std::pair<MKL_INT, double>>> _rowVals;
#else
...
#pragma omp parallel for reduction( min : minDiag )
    ...
    do
    {
        ...
        for ( i = 0; i < n; i++ )
        {
            ...
            {
                rowIt = _rowVals.insert_after( rowIt, std::make_pair( aj[A_k_idx] - base, av[A_k_idx] ) );
            }
            ...
            for ( auto& k_pair : jKRowU[i] )
            {
                ...
                if constexpr ( buildR )
                {
                    aij_update( _ai_r, _aj_r, _av_r, curR[k], k, base, aki, list_size, _rowVals );
                }
            }
            jKRowU.to_next();
            ...
            const double aii = std::sqrt( _av[k_idx] );
            _av[k_idx++] = aii;
            const MKL_INT row_l_size = ai[i + 1] - _diagPos[i] + std::min( _lsize, _nrow - i - 1 );
            ...
            else
            {
                ...
                std::sort( heapL.begin(), heapL.end(),
                           []( const std::pair<MKL_INT, double>& a, const std::pair<MKL_INT, double>& b )
