...
public:
    ...
    mkl_sparse_mat( const MKL_INT row,
                    const MKL_INT col,
    ...
    void get_adjacency_graph( std::vector<MKL_INT>& xadj, std::vector<MKL_INT>& adjncy ) const;
...
// c*A+B
mkl_sparse_mat mkl_sparse_sum( const mkl_sparse_mat& A, const mkl_sparse_mat& B, double c = 1. );
...
public:
    ...
    mkl_sparse_mat_sym( const MKL_INT row,
                        const MKL_INT col,
                        const std::shared_ptr<MKL_INT[]>& ai,
                        const std::shared_ptr<MKL_INT[]>& aj,
                        const std::shared_ptr<double[]>& av,
...
// c*A+B
mkl_sparse_mat_sym mkl_sparse_sum( const mkl_sparse_mat_sym& A, const mkl_sparse_mat_sym& B, double c = 1. );
...
public:
    dense_mat() = default;
    dense_mat( MKL_INT m, MKL_INT n ) : _m( m ), _n( n ) { _av.reset( new double[_m * _n] ); }
...
bool mkl_sparse_dense_mat_prod( const mkl_sparse_mat& A,
                                const dense_mat& B,
                                dense_mat& C,
                                const sparse_operation_t opA = SPARSE_OPERATION_NON_TRANSPOSE );

bool dense_product( const dense_mat& A,
                    const dense_mat& B,
                    dense_mat& C,
