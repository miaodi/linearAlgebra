...
int main( int argc, char* argv[] )
{
    ...
    if ( out_orig.is_open() )
    {
        ...
        matrix_utils::writeSVG( matrix.rows, matrix.cols, matrix.AI(), matrix.AJ(), out_orig, opts.max_display_size );
        out_orig.close();
    ...
    if ( opts.algorithm == "rcm" )
    {
        ...
        reordering::RCM_MultiComponent<reordering::RCMKernel::ParallelSort>(
            matrix.rows, xadj.data(), adjncy.data(), perm.data(), iperm.data(), opts.threads );
...
#ifdef USE_METIS_LIB
        int rc = reordering::MetisND<int, int>( matrix.rows, matrix.cols, xadj.data(),
    ...
    {
        std::cerr << "Unknown algorithm: " << opts.algorithm << ". Use 'rcm', 'rcm-trad', or 'nd'."
                  << std::endl;
        return -1;
    }
    ...
    matrix_utils::permuteMat( matrix.rows, matrix.cols, perm.data(), iperm.data(), matrix.AI(),
                              matrix.AJ(), matrix.AV(), permuted.AI(), permuted.AJ(), permuted.AV(),
                              opts.threads );
    ...
    matrix_utils::writeSVG( permuted.rows, permuted.cols, permuted.AI(), permuted.AJ(), out, opts.max_display_size );
    out.close();
    std::cout << "Reordered matrix written to: " << opts.output_file << std::endl;

    // Summary
    std::cout << "\n=== Summary ===" << std::endl;
    std::cout << "Total time: " << ( aplusat_time.count() + order_time + perm_time.count() ) << " s"
              << std::endl;
