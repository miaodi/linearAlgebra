...
int main( int argc, char* argv[] )
{
...

    {
        ...
        matrix_utils::writeSVG( pruned.rows, pruned.cols, pruned.AI(), pruned.AJ(), out_pruned,
                                opts.max_display_size );
        out_pruned.close();
    ...
    if ( compute_permuted_pair( csr_matrix, pruned, opts, permuted_original, permuted_pruned ) )
    {
        ...
        if ( out_perm_orig.is_open() )
        {
            ...
            matrix_utils::writeSVG( permuted_original.rows, permuted_original.cols,
                                    permuted_original.AI(), permuted_original.AJ(), out_perm_orig,
                                    opts.max_display_size );
            out_perm_orig.close();
        ...
        if ( out_perm_pruned.is_open() )
        {
            ...
            matrix_utils::writeSVG( permuted_pruned.rows, permuted_pruned.cols, permuted_pruned.AI(),
                                    permuted_pruned.AJ(), out_perm_pruned, opts.max_display_size );
                                   ...
                                   matrix_utils::CSRMatrix<int, int, double>& permuted_pruned )
{
    ...
    matrix_utils::APlusATFill<int, int, false>( pruned.rows, pruned.AI(), pruned.AJ(), xadj.data(),
                                                adjncy.data() );
    ...
    else // default to nd
    {
        ...
        nd_opts.seed = 42;
        rc = reordering::MetisND( pruned.rows, pruned.cols, xadj.data(), adjncy.data(),
    ...
    std::cout << "Permutation (" << opts.perm_algorithm
    ...
    matrix_utils::permuteMat( original.rows, original.cols, perm.data(), iperm.data(),
                              original.AI(), original.AJ(), original.AV(), permuted_original.AI(),
                              permuted_original.AJ(), permuted_original.AV(), opts.threads );
    ...
    matrix_utils::permuteMat( pruned.rows, pruned.cols, perm.data(), iperm.data(), pruned.AI(),
                              pruned.AJ(), pruned.AV(), permuted_pruned.AI(), permuted_pruned.AJ(),
                              permuted_pruned.AV(), opts.threads );
...
public:
    ...
    ILUPrec( const COLTYPE size, const CSRMatrixType& ilu, const int num_threads = omp_get_max_threads() )
        : _size( size ), _ilu( ilu ), _nthreads( num_threads ), tmp( size ), forward( num_threads ), backward( num_threads )
    {
    ...
    matrix_utils::LevelScheduleTriangularSubstitution<matrix_utils::TriangularMatrix::L, ROWTYPE, COLTYPE, VALTYPE> forward;
    matrix_utils::LevelScheduleTriangularSubstitution<matrix_utils::TriangularMatrix::U, ROWTYPE, COLTYPE, VALTYPE> backward;
};
                                                 ...
                                                 const matrix_utils::CSRMatrix<int, int, double>& pruned )
{
    ...
    if ( opts.factorization != Factorization::JACOBI )
    {
        ...
        if ( opts.factorization == Factorization::ILUT )
        {
            std::cout << "  Fill ratio: " << static_cast<double>( ilu_matrix.NNZ() ) / pruned.NNZ()
                      << std::endl;
        }
    }
...
void precondExplore( const Options& opts, const matrix_utils::CSRMatrix<int, int, double>& pruned )
{
    ...
    matrix_utils::ParallelTranspose2( L.rows, L.cols, L.AI(), L.AJ(), L.AV(), L_csr.AI(),
                                      L_csr.AJ(), L_csr.AV() );
    ...
    spadd_op.analysis( L_csr.rows, L_csr.cols, L_csr.AI(), L_csr.AJ(), U.rows, U.cols, U.AI(), U.AJ(), LplusU );

    // Numerical phase: compute sum (using alpha=1.0, beta=1.0 for simple addition)
    spadd_op( L_csr.rows, L_csr.cols, L_csr.AI(), L_csr.AJ(), L_csr.AV(), 1.0, U.rows, U.cols,
              U.AI(), U.AJ(), U.AV(), 1.0, LplusU );
