...
{
    cxxopts::Options options("jaccard_compare", "Compare Jaccard similarity between multiple matrices (optionally after drop)");
    options.add_options()
        ("f,filename", "Matrix Market files to read (multiple allowed)", cxxopts::value<std::vector<std::string>>())
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        int removed = matrix_utils::DiagonalScaledPrune(pruned[i].rows, pruned[i].AI(), pruned[i].AJ(), pruned[i].AV(), threshold);
        int pruned_nnz = pruned[i].NNZ();
        std::cout << "Matrix " << i << " dropped: " << removed << " entries (" << original_nnz << " -> " << pruned_nnz << ", " << std::fixed << std::setprecision(2) << (pruned_nnz * 100.0 / original_nnz) << "% retained)" << std::endl;
    }
    // Compute Jaccard similarity matrix (after drop)
    std::vector<std::vector<double>> jaccard(n, std::vector<double>(n, 0.0));
    for (size_t i = 0; i < n; ++i) {
        for (size_t j = 0; j < n; ++j) {
            jaccard[i][j] = graph::jaccardSimilarity(
                rows, cols,
                pruned[i].AI(), pruned[i].AJ(),
                rows, cols,
                pruned[j].AI(), pruned[j].AJ()
            );
        }
    }
    // Print matrix
