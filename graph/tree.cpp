                              ...
                              COLTYPE* child_count )
{
    ...
    std::fill( first_child, first_child + nnodes, std::numeric_limits<COLTYPE>::max() );
    std::fill( next_sibling, next_sibling + nnodes, std::numeric_limits<COLTYPE>::max() );
                              ...
                              COLTYPE* child_count )
{
    if ( roots != nullptr )
    {
        return parentToChildSibling<true>( nnodes, base, parent, first_child, next_sibling, roots, child_count );
    }
    return parentToChildSibling<false>( nnodes, base, parent, first_child, next_sibling, roots, child_count );
}
                          ...
                          COLTYPE* roots )
{
    ...
    std::inclusive_scan( child_offsets, child_offsets + nnodes + 1, child_offsets );
    ...
    std::rotate( std::reverse_iterator( child_offsets + nnodes + 1 ),
                 std::reverse_iterator( child_offsets + nnodes ), std::reverse_iterator( child_offsets ) );
    child_offsets[0] = 0;
...
template <typename COLTYPE>
bool EliminationTree<COLTYPE>::analyze( const COLTYPE nnodes_in, const COLTYPE base_in, const COLTYPE* parent_in )
{
    ...
    _nroots = parentToChildCSR<true>( _nnodes, _base, _parent.data(), _childOffsets.data(),
                                      _children.data(), _roots.data() );
    ...
    _topologicalLevels =
        parentTopologicalOrder( _nnodes, _base, _parent.data(), _childCounts.data(), _scratch.data(),
...
template <typename COLTYPE>
void PostOrder<COLTYPE>::buildChildren( const COLTYPE nnodes, const COLTYPE base, const COLTYPE* parent )
{
    ...
    std::inclusive_scan( _childrenPrefix.begin(), _childrenPrefix.end(), _childrenPrefix.begin() );
    ...
    std::rotate( _childrenPrefix.rbegin(), _childrenPrefix.rbegin() + 1, _childrenPrefix.rend() );
    _childrenPrefix[0] = 0;
...
template <typename COLTYPE>
void PostOrder<COLTYPE>::dfs( const COLTYPE root, const COLTYPE base, COLTYPE*& post )
{
...
template <typename COLTYPE>
void PostOrder<COLTYPE>::apply( const COLTYPE nnodes,
                                const COLTYPE base,
                                const COLTYPE* parent,
...
template <typename COLTYPE>
void PostOrderNoRecur<COLTYPE>::apply( const COLTYPE nnodes,
                                       const COLTYPE base,
                                       const COLTYPE* parent,
                                       COLTYPE* permed_parent,
                                       COLTYPE* perm,
                                       COLTYPE* iperm )
{
    ...
    const auto nroots = parentToChildSibling<true>( nnodes, base, parent, _firstChild.data(),
...
#define INSTANTIATE_TREE( ROWTYPE, COLTYPE )                                                              \
    template void eliminationTree<ROWTYPE, COLTYPE>(                                                      \
        const COLTYPE nnodes, const ROWTYPE* ai, const COLTYPE* aj, COLTYPE* parent, COLTYPE* ancestor ); \
    template COLTYPE parentToChildSibling<COLTYPE>(                                                       \
        const COLTYPE nnodes, const COLTYPE base, const COLTYPE* parent, COLTYPE* first_child,            \
        COLTYPE* next_sibling, COLTYPE* roots, COLTYPE* child_count );                                    \
    ...
    template COLTYPE parentToChildSibling<false, COLTYPE>(                                                \
        const COLTYPE nnodes, const COLTYPE base, const COLTYPE* parent, COLTYPE* first_child,            \
        COLTYPE* next_sibling, COLTYPE* roots, COLTYPE* child_count );                                    \
    template COLTYPE parentToChildCSR<true, COLTYPE>( const COLTYPE nnodes, const COLTYPE base,           \
                                                      const COLTYPE* parent, COLTYPE* child_offsets,      \
                                                      COLTYPE* children, COLTYPE* roots );                \
    template COLTYPE parentToChildCSR<false, COLTYPE>( const COLTYPE nnodes, const COLTYPE base,          \
                                                       const COLTYPE* parent, COLTYPE* child_offsets,     \
                                                       COLTYPE* children, COLTYPE* roots );               \
    template COLTYPE parentTopologicalOrder<COLTYPE>(                                                     \
        const COLTYPE nnodes, const COLTYPE base, const COLTYPE* parent,                                  \
        const COLTYPE* child_count, COLTYPE* scratch, COLTYPE* perm, COLTYPE* prefix );                   \
