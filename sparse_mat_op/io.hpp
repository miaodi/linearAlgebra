...
template <typename CSRMatrixType>
void writePNG(const CSRMatrixType &csr_matrix, const std::string &filename,
              const typename CSRMatrixType::COLTYPE max_display_size = 4096) {
  writePNG(csr_matrix.rows, csr_matrix.cols, csr_matrix.AI(), csr_matrix.AJ(),
           filename, max_display_size);
}
} // namespace matrix_utils
