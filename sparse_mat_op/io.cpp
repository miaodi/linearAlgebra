...
template <typename T>
void writeBinaryValue( std::ostream& stream, const T& value, const std::string& filename )
{
...
{
    if ( filename.size() >= 4 && filename.compare( filename.size() - 4, 4, ".mtx" ) == 0 )
    {
    ...
    throw std::invalid_argument( "Unsupported matrix file extension, expected .mtx or .bin: " + filename );
}
...
void readBinaryMatrix( const std::string& filename, CSRMatrixType& csr_matrix )
{
    ...
    std::sort( order.begin(), order.end(),
               [&rows, &cols]( const auto lhs, const auto rhs )
               {
    ...
    for ( COLTYPE row = 0; row < rows; ++row )
    {
        for ( auto pos = ai[row] - base; pos < ai[row + 1] - base; ++pos )
        {
            const std::tuple<std::int32_t, std::int32_t, double> entry{
                toBinaryIndex( row + 1 ), toBinaryIndex( aj[pos] - base + 1 ), static_cast<double>( av[pos] ) };
...
template <ResizableCSR CSRMatrixType>
void readMatrix( const std::string& filename, CSRMatrixType& csr_matrix, const fast_matrix_market::read_options& options )
{
...
template <ResizableCSR CSRMatrixType>
void readMatrixMarket( std::istream& instream, CSRMatrixType& csr_matrix, const fast_matrix_market::read_options& options )
{
    using ROWTYPE = typename CSRMatrixType::ROWTYPE;
    using COLTYPE = typename CSRMatrixType::COLTYPE;
    using VALTYPE = typename CSRMatrixType::VALTYPE;
    ...
    std::copy( mat.outerIndexPtr(), mat.outerIndexPtr() + mat.rows() + 1, csr_matrix.AI() );
    std::copy( mat.innerIndexPtr(), mat.innerIndexPtr() + mat.nonZeros(), csr_matrix.AJ() );
                        ...
                        const fast_matrix_market::write_options& options )
{
    ...
    else if ( header.field != fast_matrix_market::pattern && options.fill_header_field_type )
    {
    ...
    auto formatter = fast_matrix_market::csc_formatter(
        lf, ai, ai + rows, aj, aj + nnz, av, header.field == fast_matrix_market::pattern ? av : av + nnz, true );
    fast_matrix_market::write_body( outstream, formatter, options );
}

template <typename ROWTYPE, typename COLTYPE>
void writeSVG( const COLTYPE rows, const COLTYPE cols, const ROWTYPE* ai, const COLTYPE* aj, std::ostream& outstream, const COLTYPE max_display_size )
{
    ...
    outstream << "<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" "
              ...
              << "   <rect x=\"1\" y=\"1\" width=\"" << display_cols + 0.1 << "\" height=\""
              << display_rows + 0.1 << "\" fill=\"rgb(255, 255, 255)\"/>\n\n";
    ...
    if ( needs_sampling )
    {
        ...
        for ( COLTYPE i = 0; i < display_rows; i++ )
        {
            for ( COLTYPE j = 0; j < display_cols; j++ )
            {
                ...
                {
                    outstream << "  <rect class=\"pixel\" x=\"" << j + 1 << "\" y=\"" << i + 1
                              << "\" width=\".9\" height=\".9\"/>\n";
                }
            }
        }
    ...
    else
    {
        ...
        for ( COLTYPE i = 0; i < rows; i++ )
        {
            ...
            for ( ROWTYPE j = row_start; j < row_end; j++ )
            {
                outstream << "  <rect class=\"pixel\" x=\"" << aj[j] - base + 1 << "\" y=\""
                          << i + 1 << "\" width=\".9\" height=\".9\"/>\n";
            }
        }
    }
...
template <typename T>
void readMatrixMarketVec( std::istream& instream, std::vector<T>& vec, const fast_matrix_market::read_options& options )
{
    fast_matrix_market::matrix_market_header header;
    fast_matrix_market::read_matrix_market_array( instream, header, vec, fast_matrix_market::row_major, options );
}

#define INSTANTIATE_READMATRIXMARKET( CSRMatrixType )                                                   \
    template void readMatrixMarket<CSRMatrixType>( std::istream & instream, CSRMatrixType & csr_matrix, \
                                                   const fast_matrix_market::read_options& options );
#define INSTANTIATE_READMATRIX( CSRMatrixType )                                                      \
    template void readMatrix<CSRMatrixType>( const std::string& filename, CSRMatrixType& csr_matrix, \
                                             const MatrixDataType data_type,                         \
                                             const fast_matrix_market::read_options& options );
...
#define INSTANTIATE_WRITEMATRIX( CSRMatrixType )                                                           \
    template void writeMatrix<CSRMatrixType>( const CSRMatrixType& csr_matrix,                             \
                                              const std::string& filename, const MatrixDataType data_type, \
                                              const fast_matrix_market::write_options& options );
#define INSTANTIATE_WRITEMATRIX_DEDUCED( CSRMatrixType )                                                    \
    template void writeMatrix<CSRMatrixType>( const CSRMatrixType& csr_matrix, const std::string& filename, \
                                              const fast_matrix_market::write_options& options );
#define INSTANTIATE_READMATRIXMARKET_CSR_VECTORS( ROWTYPE, COLTYPE, VALTYPE )          \
    template void readMatrixMarket<ROWTYPE, COLTYPE, VALTYPE>(                         \
        std::istream & instream, std::vector<ROWTYPE> & ai, std::vector<COLTYPE> & aj, \
        std::vector<VALTYPE> & av, const fast_matrix_market::read_options& options );
#define INSTANTIATE_WRITEMATRIXMARKET( ROWTYPE, COLTYPE, VALTYPE )                    \
    template void writeMatrixMarket<ROWTYPE, COLTYPE, VALTYPE>(                       \
        const COLTYPE rows, const COLTYPE cols, const ROWTYPE* ai, const COLTYPE* aj, \
        const VALTYPE* av, std::ostream& outstream, const fast_matrix_market::write_options& options );
#define INSTANTIATE_WRITESVG( ROWTYPE, COLTYPE )                                      \
    template void writeSVG<ROWTYPE, COLTYPE>( const COLTYPE rows, const COLTYPE cols, \
                                              const ROWTYPE* ai, const COLTYPE* aj,   \
                                              std::ostream& outstream, const COLTYPE max_display_size );
#define INSTANTIATE_WRITEPNG( ROWTYPE, COLTYPE )                                                         \
    template void writePNG<ROWTYPE, COLTYPE>( const COLTYPE rows, const COLTYPE cols, const ROWTYPE* ai, \
                                              const COLTYPE* aj, const std::string& filename,            \
                                              const COLTYPE max_display_size );
#define INSTANTIATE_READMATRIXMARKETVEC( T )                                             \
    template void readMatrixMarketVec<T>( std::istream & instream, std::vector<T> & vec, \
                                          const fast_matrix_market::read_options& options );
