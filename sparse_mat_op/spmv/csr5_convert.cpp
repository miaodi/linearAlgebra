...
void unpackCSR5TileDesc(uint64_t desc, uint32_t& bit_flag, uint32_t& y_offset, 
                        uint16_t& seg_offset, int omega) {
...
static void processTile(
    ...
    ROWTYPE base, COLTYPE num_rows,
    ROWTYPE csr_start, int tile_length,
    COLTYPE* tile_col, VALTYPE* tile_val,
    uint32_t& bit_flag, COLTYPE& current_row) {
...
void convertCSRtoCSR5(COLTYPE num_rows, 
    ...
    for (ROWTYPE tile_idx = 0; tile_idx < num_tiles; ++tile_idx) {
        ...
        uint32_t bit_flag = 0;
        processTile<ROWTYPE, COLTYPE, VALTYPE, OMEGA>(
            ai, aj, av, base, num_rows, csr_start, tile_length,
...
template<typename ROWTYPE, typename COLTYPE, typename VALTYPE, typename Policy>
void CSR5Data<ROWTYPE, COLTYPE, VALTYPE, Policy>::unpackTileDesc(
    ROWTYPE tile_idx, uint32_t& bit_flag, COLTYPE& y_offset, uint16_t& seg_offset) const {
...
template void CSR5Data<int, int, double, CSR5_AVX2_Policy<double>>::unpackTileDesc(
    int, uint32_t&, int&, uint16_t&) const;

// int, int, float with AVX2
template void convertCSRtoCSR5<int, int, float, CSR5_AVX2_Policy<float>>(
    int, const int*, const int*, const float*, 
    CSR5Data<int, int, float, CSR5_AVX2_Policy<float>>&);
template void CSR5Data<int, int, float, CSR5_AVX2_Policy<float>>::unpackTileDesc(
    int, uint32_t&, int&, uint16_t&) const;

// int64_t, int64_t, double with AVX2
template void convertCSRtoCSR5<int64_t, int64_t, double, CSR5_AVX2_Policy<double>>(
    int64_t, const int64_t*, const int64_t*, const double*, 
    CSR5Data<int64_t, int64_t, double, CSR5_AVX2_Policy<double>>&);
template void CSR5Data<int64_t, int64_t, double, CSR5_AVX2_Policy<double>>::unpackTileDesc(
    int64_t, uint32_t&, int64_t&, uint16_t&) const;

// int64_t, int64_t, float with AVX2
template void convertCSRtoCSR5<int64_t, int64_t, float, CSR5_AVX2_Policy<float>>(
    int64_t, const int64_t*, const int64_t*, const float*, 
    CSR5Data<int64_t, int64_t, float, CSR5_AVX2_Policy<float>>&);
template void CSR5Data<int64_t, int64_t, float, CSR5_AVX2_Policy<float>>::unpackTileDesc(
    int64_t, uint32_t&, int64_t&, uint16_t&) const;
