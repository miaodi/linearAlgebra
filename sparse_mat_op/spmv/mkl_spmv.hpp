...
public:
    ...
    void preprocess(COLTYPE size, ROWTYPE const* __restrict ai,
        ...
        if constexpr (std::is_same_v<VALTYPE, double>) {
            status = mkl_sparse_d_create_csr(&_mkl_mat, mkl_base, size, size,
                                            const_cast<ROWTYPE*>(ai),
                                            const_cast<ROWTYPE*>(ai + 1),
                                            const_cast<COLTYPE*>(aj),
                                            const_cast<VALTYPE*>(av));
    ...
    void operator()(VALTYPE const * __restrict const b,
        ...
        sparse_status_t status;
        if constexpr (std::is_same_v<VALTYPE, double>) {
            status = mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, alpha, _mkl_mat,
                                    _descr, b, beta, x);
        } else if constexpr (std::is_same_v<VALTYPE, float>) {
            status = mkl_sparse_s_mv(SPARSE_OPERATION_NON_TRANSPOSE, alpha, _mkl_mat,
                                    _descr, b, beta, x);
        } else {
    ...
    static constexpr bool is_compatible() {
        return std::is_same_v<ROWTYPE, MKL_INT> && 
               std::is_same_v<COLTYPE, MKL_INT> &&
