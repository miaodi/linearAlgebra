...
template void SerialTranspose<int, int, double>( const int rows,
                                                 const int cols,
                                                 int const* ai,
                                                 int const* aj,
                                                 double const* av,
...
template void ParallelTranspose<int, int, double>( const int rows,
                                                   const int cols,
                                                   int const* ai,
                                                   int const* aj,
                                                   double const* av,
...
template void ParallelTranspose2<int, int, double>( const int rows,
                                                    const int cols,
                                                    int const* ai,
                                                    int const* aj,
                                                    double const* av,
...
template bool Diagonal<int, int, double>( const int rows,
                                          int const* ai,
                                          int const* aj,
                                          double const* av,
                                          int* diagpos,
...
template bool IsSymmetry<int, int, double>( const int size, int const* ai, int const* aj, double const* av );

template void SplitLDU( const int rows,
                        const int base,
                        int const* ai,
                        int const* aj,
                        double const* av,
                        CSRMatrix<int, int, double>& L,
                        std::vector<double>& D,
                        CSRMatrix<int, int, double>& U );

template <ResizableCSR CSRMatrixType>
void SplitLU<CSRMatrixType>::operator()( const COLTYPE rows,
                                         ROWTYPE const* ai,
                                         ROWTYPE const* diag,
                                         COLTYPE const* aj,
                                         VALTYPE const* av,
...
#pragma omp parallel num_threads( num_threads )
    {
        ...
        auto [start, end] = utils::LoadPrefixBalancedPartitionPos( ai, ai + rows, tid, num_threads );
        prefixL[tid + 1] = 0;
        prefixU[tid + 1] = 0;
...
template void SplitTriangle<TriangularMatrix::U, int, int, double, CSRMatrix<int, int, double>>(
    const int rows,
    const int base,
    int const* ai,
    int const* aj,
...
template void SplitTriangle<TriangularMatrix::U, int, int, double, CSRMatrixVec<int, int, double>>(
    const int rows,
    const int base,
    int const* ai,
    int const* aj,
    double const* av,
    CSRMatrixVec<int, int, double>& U );

template void SplitTriangle<TriangularMatrix::L, int, int, double, CSRMatrix<int, int, double>>(
    const int rows,
    const int base,
    int const* ai,
    int const* aj,
...
template void SplitTriangle<TriangularMatrix::L, int, int, double, CSRMatrixVec<int, int, double>>(
    const int rows,
    const int base,
    int const* ai,
    int const* aj,
...
template void TriangularToFull<TriangularMatrix::U, int, int, double, CSRMatrix<int, int, double>>(
    const int rows,
    const int base,
    int const* ai,
    int const* aj,
    double const* av,
    CSRMatrix<int, int, double>& F );

template void TriangularToFull<TriangularMatrix::U, int, int, double, CSRMatrixVec<int, int, double>>(
    const int rows,
    const int base,
    int const* ai,
    int const* aj,
    double const* av,
    CSRMatrixVec<int, int, double>& F );
...
bool CSRToMetisGraph( const COLTYPE nrows, ROWTYPE const* ai, COLTYPE const* aj, ROWTYPE* xadj, COLTYPE* adjncy, const int nthreads )
{
...
#define INSTANTIATE_MATRIX_OPS( ROWTYPE, COLTYPE, VALTYPE )                                               \
    template ROWTYPE Prune<ROWTYPE, COLTYPE, VALTYPE>( const COLTYPE rows, ROWTYPE* ai, COLTYPE* aj,      \
                                                       VALTYPE* av, const VALTYPE threshold,              \
                                                       VALTYPE const* row_thresholds );                   \
    template ROWTYPE RowMaxPrune<ROWTYPE, COLTYPE, VALTYPE>(                                              \
        const COLTYPE rows, ROWTYPE* ai, COLTYPE* aj, VALTYPE* av, const VALTYPE threshold );             \
    template ROWTYPE DiagonalScaledPrune<ROWTYPE, COLTYPE, VALTYPE>(                                      \
        const COLTYPE rows, ROWTYPE* ai, COLTYPE* aj, VALTYPE* av, const VALTYPE threshold );             \
    template void EmbedCSR<ROWTYPE, COLTYPE, VALTYPE>(                                                    \
        const COLTYPE rows, ROWTYPE const* ai_source, COLTYPE const* aj_source, VALTYPE const* av_source, \
        ROWTYPE const* ai_target, COLTYPE const* aj_target, VALTYPE* av_target, const int num_threads );
...
template bool CSRToMetisGraph<int, int>( const int nrows, int const* ai, int const* aj, int* xadj, int* adjncy, const int nthreads );
