...
concept CSR = requires( T obj ) {
    ...
    { static_cast<const T&>( obj ).AI() } -> std::same_as<typename T::ROWTYPE const*>;
    { static_cast<const T&>( obj ).AJ() } -> std::same_as<typename T::COLTYPE const*>;
    { static_cast<const T&>( obj ).AV() } -> std::same_as<typename T::VALTYPE const*>;
...
concept ResizableCSR = CSR<T> && requires( T obj ) {
    { obj.ResizeAI( std::declval<std::size_t>() ) } -> std::same_as<typename T::ROWTYPE*>;
    { obj.ResizeAJ( std::declval<std::size_t>() ) } -> std::same_as<typename T::COLTYPE*>;
...
concept HasDiagonal = requires( T obj ) {
    ...
    { static_cast<const T&>( obj ).Diagonal() } -> std::same_as<typename T::ROWTYPE const*>;
};

template <typename T>
concept ResizableDiagonal = ResizableCSR<T> && HasDiagonal<T> && requires( T obj ) {
    { obj.ResizeDiagonal( std::declval<std::size_t>() ) } -> std::same_as<typename T::ROWTYPE*>;
};

// Operator concepts
template <typename T>
concept SpmvOp = requires( const T op,
                           typename T::VALTYPE const* const b,
                           typename T::VALTYPE* const x,
                           typename T::VALTYPE alpha,
                           typename T::VALTYPE beta ) {
    typename T::VALTYPE;
...
template <typename T>
concept PrecOp =
    requires( const T prec, typename T::VALTYPE const* const b, typename T::VALTYPE* const x ) {
        typename T::VALTYPE;
...
{
    static constexpr bool value = std::is_same_v<typename CSRMatrixType::ROWTYPE, R> &&
