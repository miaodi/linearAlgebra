...
struct PairSrc
{
    __host__ __device__ COLTYPE operator()( const Pair<COLTYPE>& p ) const { return p.first; }
};
...
template <typename COLTYPE>
__global__ void lookup_frontier_degrees_kernel( const Pair<COLTYPE>* current_frontier,
                                                int current_size,
...
__global__ void insert_and_check_kernel( const Pair<COLTYPE>* pairs, int n, Ref ref, bool* inserted )
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if ( idx >= n )
        return;
...
template <typename ROWTYPE, typename COLTYPE>
__global__ void build_next_frontier_kernel( const Pair<COLTYPE>* current_frontier,
                                            int current_size,
...
struct FilterU_lt
{
    __device__ bool operator()( const Pair<COLTYPE>& p ) const { return p.first < p.second; }
};
...
template <typename ROWTYPE, typename COLTYPE>
__global__ void extract_u_pattern_kernel( const Pair<COLTYPE>* pairs,
                                          int n,
                                          const bool* mask,
                                          COLTYPE base,
                                          bool keepdiag,
                        ...
                        ROWTYPE* u_nnz )
{
    ...
    init_frontier_kernel<<<blocks, threads>>>( thrust::raw_pointer_cast( current_frontier.data() ), n );
    ...
    constexpr visited_key_t empty_key{ std::numeric_limits<COLTYPE>::max(),
    ...
    compute_all_degrees_kernel<<<blocks, threads>>>(
        d_ai, n, base, thrust::raw_pointer_cast( all_degrees.data() ) );
    cudaDeviceSynchronize();
    ...
    for ( int level = 0; level <= lvl; ++level )
    {
        ...
        blocks = ( actual_next_size + threads - 1 ) / threads;
        insert_and_check_kernel<<<blocks, threads>>>( thrust::raw_pointer_cast( next_frontier.data() ),
                                                      actual_next_size, insert_ref,
    ...
    cudaMemcpy( d_u_ai, thrust::raw_pointer_cast( u_ai_dev.data() ), ( n + 1 ) * sizeof( ROWTYPE ),
...
template <typename COLTYPE>
__host__ __device__ inline COLTYPE triplet_src( const Triplet<COLTYPE>& t )
{
    return t.first.first;
}

template <typename COLTYPE>
__host__ __device__ inline COLTYPE triplet_cur( const Triplet<COLTYPE>& t )
{
    return t.first.second;
}

template <typename COLTYPE>
__host__ __device__ inline COLTYPE triplet_path_max( const Triplet<COLTYPE>& t )
{
    return t.second;
}
...
struct TripletLess
{
    ...
    {
        if ( triplet_src( a ) != triplet_src( b ) )
            return triplet_src( a ) < triplet_src( b );
...
template <typename COLTYPE>
__global__ void lookup_triplet_frontier_degrees_kernel( const Triplet<COLTYPE>* current_frontier,
                                                        int current_size,
                                                        const int* all_degrees,
                                                        int* frontier_degrees )
{
...
template <typename ROWTYPE, typename COLTYPE>
__global__ void build_next_triplet_frontier_kernel( const Triplet<COLTYPE>* current_frontier,
                                                    int current_size,
                                                    const int* inclusive_sum,
                                                    const ROWTYPE* d_ai,
                                                    const COLTYPE* d_aj,
...
__global__ void check_and_insert_triplet_kernel( const Triplet<COLTYPE>* triplets,
                                                 int n,
                                                 MapRef map_ref,
...
__global__ void scatter_keep_fill_kernel( const Triplet<COLTYPE>* next_frontier,
                       ...
                       ROWTYPE* lu_nnz )
{
    ...
    using visited_map_t = cuco::static_map<map_key_t, map_value_t, cuco::extent<std::size_t>, cuda::thread_scope_device,
                                           ::cuda::std::equal_to<map_key_t>, probing_scheme_t>;
    visited_map_t visited_map( map_capacity, cuco::empty_key<map_key_t>{ empty_key },
    ...
    // Insert initial diagonal entries into map with path_max = 0
    {
        ...
        thrust::tabulate(
            init_pairs.begin(), init_pairs.end(), [=] __device__( int i )
    ...
    blocks = ( n + threads - 1 ) / threads;
    init_triplet_frontier_kernel<<<blocks, threads>>>( thrust::raw_pointer_cast( current_frontier.data() ), n );
    cudaDeviceSynchronize();
    ...
    for ( int level = 0; level <= lvl; ++level )
    {
        ...
        check_and_insert_triplet_kernel<<<blocks, threads>>>(
            thrust::raw_pointer_cast( next_frontier.data() ), actual_next_size, map_ref,
        ...
        blocks = ( actual_next_size + threads - 1 ) / threads;
        count_flags_kernel<256><<<blocks, threads>>>( thrust::raw_pointer_cast( flags.data() ), actual_next_size,
        ...
        {
            int h_counts[2] = { 0, 0 };
            cudaMemcpy( h_counts, thrust::raw_pointer_cast( counts.data() ), sizeof( h_counts ),
        ...
        scatter_keep_fill_kernel<COLTYPE, 256><<<blocks, threads>>>(
            thrust::raw_pointer_cast( next_frontier.data() ), thrust::raw_pointer_cast( flags.data() ),
            actual_next_size, thrust::raw_pointer_cast( counts.data() ),
        ...
        std::cout << "Level " << level << " | frontier=" << current_size
                  << " next_size=" << actual_next_size << " | degree=" << time_degree_lookup << "ms"
                  << " scan=" << time_scan << "ms"
                  << " build=" << time_build_frontier << "ms"
                  << " hash=" << time_hash_insert << "ms"
                  << " scatter=" << time_extract_fill << "ms" << std::endl;
    }
    ...
    std::cout << "Total BFS time:  "
              << ( total_degree_lookup + total_scan + total_build_frontier + total_hash_insert + total_extract_fill )
              << " ms" << std::endl;
    std::cout << "=========================\n" << std::endl;
    ...
    thrust::lower_bound( row_it, row_it + lu_pattern.size(), row_indices.begin(), row_indices.end(),
                         lu_ai_dev.begin() );
    ...
    if ( base != 0 )
    {
        thrust::transform( lu_ai_dev.begin(), lu_ai_dev.end(), cuda::make_constant_iterator( ROWTYPE( base ) ),
...
template bool ILUSymbolicU_CUDA<int, int>( int n,
                                           const int* d_ai,
                                           const int* d_aj,
                                           int lvl,
                                           int base,
...
template bool ILUSymbolicU_CUDA<int64_t, int>( int n,
                                               const int64_t* d_ai,
                                               const int* d_aj,
                                               int lvl,
                                               int base,
...
template bool ILUSymbolic_CUDA<int, int>( int n,
                                          const int* d_ai,
                                          const int* d_aj,
                                          int lvl,
                                          int base,
...
template bool ILUSymbolic_CUDA<int64_t, int>( int n,
                                              const int64_t* d_ai,
                                              const int* d_aj,
                                              int lvl,
                                              int base,
