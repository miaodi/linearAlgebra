...
SpTRSVOperator<ROWTYPE, COLTYPE, VALTYPE>::SpTRSVOperator(
    COLTYPE n,
    const ROWTYPE* d_ia,
    const COLTYPE* d_ja,
    const VALTYPE* d_av,
...
void SpTRSVOperator<ROWTYPE, COLTYPE, VALTYPE>::solve(const VALTYPE* d_b, VALTYPE* d_x)
{
    ...
    if (_tri_type == TriangularType::Lower) {
        if (_diag_type == DiagonalType::Unit) {
            sptrsv_kernel<ROWTYPE, COLTYPE, VALTYPE, true, true><<<num_blocks, block_size>>>(
                _d_ia, _d_ja, _d_av, d_b, d_x, _n);
        } else {
            sptrsv_kernel<ROWTYPE, COLTYPE, VALTYPE, true, false><<<num_blocks, block_size>>>(
                _d_ia, _d_ja, _d_av, d_b, d_x, _n);
        }
    } else {
        if (_diag_type == DiagonalType::Unit) {
            sptrsv_kernel<ROWTYPE, COLTYPE, VALTYPE, false, true><<<num_blocks, block_size>>>(
                _d_ia, _d_ja, _d_av, d_b, d_x, _n);
        } else {
            sptrsv_kernel<ROWTYPE, COLTYPE, VALTYPE, false, false><<<num_blocks, block_size>>>(
                _d_ia, _d_ja, _d_av, d_b, d_x, _n);
        }
    }
...
void sptrsv_lower(COLTYPE n,
                  ...
                  DiagonalType diag_type)
{
    SpTRSVOperator<ROWTYPE, COLTYPE, VALTYPE> op(
        n, d_ia, d_ja, d_av, TriangularType::Lower, diag_type);
    op.solve(d_b, d_x);
}
...
{
    SpTRSVOperator<ROWTYPE, COLTYPE, VALTYPE> op(
        n, d_ia, d_ja, d_av, TriangularType::Upper, diag_type);
    op.solve(d_b, d_x);
}
...
// int, int, double
template __global__ void sptrsv_kernel<int, int, double, true, true>(const int*, const int*, 
                                                                      const double*, const double*, 
                                                                      double*, int);
template __global__ void sptrsv_kernel<int, int, double, true, false>(const int*, const int*, 
                                                                       const double*, const double*, 
                                                                       double*, int);
template __global__ void sptrsv_kernel<int, int, double, false, true>(const int*, const int*, 
                                                                       const double*, const double*, 
                                                                       double*, int);
template __global__ void sptrsv_kernel<int, int, double, false, false>(const int*, const int*, 
                                                                        const double*, const double*, 
                                                                        double*, int);

// int, int, float
template __global__ void sptrsv_kernel<int, int, float, true, true>(const int*, const int*, 
                                                                     const float*, const float*, 
                                                                     float*, int);
template __global__ void sptrsv_kernel<int, int, float, true, false>(const int*, const int*, 
                                                                      const float*, const float*, 
                                                                      float*, int);
template __global__ void sptrsv_kernel<int, int, float, false, true>(const int*, const int*, 
                                                                      const float*, const float*, 
                                                                      float*, int);
template __global__ void sptrsv_kernel<int, int, float, false, false>(const int*, const int*, 
                                                                       const float*, const float*, 
                                                                       float*, int);

// int64_t, int64_t, double
template __global__ void sptrsv_kernel<int64_t, int64_t, double, true, true>(const int64_t*, const int64_t*, 
                                                                              const double*, const double*, 
                                                                              double*, int64_t);
template __global__ void sptrsv_kernel<int64_t, int64_t, double, true, false>(const int64_t*, const int64_t*, 
                                                                               const double*, const double*, 
                                                                               double*, int64_t);
template __global__ void sptrsv_kernel<int64_t, int64_t, double, false, true>(const int64_t*, const int64_t*, 
                                                                               const double*, const double*, 
                                                                               double*, int64_t);
template __global__ void sptrsv_kernel<int64_t, int64_t, double, false, false>(const int64_t*, const int64_t*, 
                                                                                const double*, const double*, 
                                                                                double*, int64_t);

// int64_t, int64_t, float
template __global__ void sptrsv_kernel<int64_t, int64_t, float, true, true>(const int64_t*, const int64_t*, 
                                                                             const float*, const float*, 
                                                                             float*, int64_t);
template __global__ void sptrsv_kernel<int64_t, int64_t, float, true, false>(const int64_t*, const int64_t*, 
                                                                              const float*, const float*, 
                                                                              float*, int64_t);
template __global__ void sptrsv_kernel<int64_t, int64_t, float, false, true>(const int64_t*, const int64_t*, 
                                                                              const float*, const float*, 
                                                                              float*, int64_t);
template __global__ void sptrsv_kernel<int64_t, int64_t, float, false, false>(const int64_t*, const int64_t*, 
                                                                               const float*, const float*, 
                                                                               float*, int64_t);

// Convenience function instantiations
template void sptrsv_lower<int, int, double>(int, const int*, const int*, const double*, 
                                              const double*, double*, DiagonalType);
template void sptrsv_lower<int, int, float>(int, const int*, const int*, const float*, 
                                             const float*, float*, DiagonalType);
template void sptrsv_lower<int64_t, int64_t, double>(int64_t, const int64_t*, const int64_t*, 
                                                      const double*, const double*, double*, DiagonalType);
template void sptrsv_lower<int64_t, int64_t, float>(int64_t, const int64_t*, const int64_t*, 
                                                     const float*, const float*, float*, DiagonalType);

template void sptrsv_upper<int, int, double>(int, const int*, const int*, const double*, 
                                              const double*, double*, DiagonalType);
template void sptrsv_upper<int, int, float>(int, const int*, const int*, const float*, 
                                             const float*, float*, DiagonalType);
template void sptrsv_upper<int64_t, int64_t, double>(int64_t, const int64_t*, const int64_t*, 
                                                      const double*, const double*, double*, DiagonalType);
template void sptrsv_upper<int64_t, int64_t, float>(int64_t, const int64_t*, const int64_t*, 
                                                     const float*, const float*, float*, DiagonalType);
