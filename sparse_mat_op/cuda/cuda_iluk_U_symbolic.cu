...
bool frontier_advance( const DeviceArray<uint64_t>& packed_coo,
                       const DeviceArray<uint64_t>& packed_coo_prev,
                       COLTYPE n,
                       ROWTYPE base,
                       ...
                       DeviceArray<uint64_t>& packed_coo_prev_out )
{
    ...
    if ( coo_sz == 0 )
    {
        ...
        thrust::fill( thrust::device, frontier_matrix.ai.data(),
                      frontier_matrix.ai.data() + ( n + 1 ), static_cast<ROWTYPE>( base ) );
    ...
    {
        thrust::transform( thrust::device, d_lower.data(), d_lower.data() + lower_sz,
                           d_lower.data(), PackedCooSwap{} );
        thrust::sort( thrust::device, d_lower.data(), d_lower.data() + lower_sz );
        if ( !PackedCOOtoCSR<ROWTYPE, COLTYPE>( d_lower.data(), static_cast<ROWTYPE>( lower_sz ), n,
                                                base, frontier_matrix ) )
            return false;
    }
    ...
    {
        union_end = thrust::set_union( thrust::device, packed_coo.data(),
                                       packed_coo.data() + coo_sz, packed_coo_prev.data(),
                                       packed_coo_prev.data() + prev_sz, d_union.data() );
    }
...
template bool ILUSymbolicU_SpMM_CUDA<int, int>( int n,
                                                const int* d_ai,
                                                const int* d_aj,
                                                int lvl,
                                                int base,
                                                DeviceCSRMatrix<int, int>& U_matrix );
template bool ILUSymbolicU_SpMM_CUDA<std::int64_t, int>( int n,
