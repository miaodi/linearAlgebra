...
__global__ void compute_norms( COLTYPE rows, const ROWTYPE* ai, const COLTYPE* aj, const VALTYPE* av, VALTYPE* row_norms, VALTYPE* col_norms )
{
...
template <typename ROWTYPE, typename COLTYPE, typename VALTYPE, CudaRuizScalingNormType NORM>
__global__ void compute_norms_sparse_coo_global( ROWTYPE sparse_nnz,
                                                 ROWTYPE base,
                                                 const COLTYPE* row_ind,
                                                 const COLTYPE* col_ind,
                                                 const VALTYPE* values,
...
__global__ void scale_matrix_and_track_change( COLTYPE rows,
                                               const ROWTYPE* ai,
                                               const COLTYPE* aj,
                                               VALTYPE* av,
...
template <typename ROWTYPE, typename COLTYPE, typename VALTYPE>
__global__ void scale_sparse_coo_global( ROWTYPE sparse_nnz,
                                         ROWTYPE base,
                                         const COLTYPE* row_ind,
                                         const COLTYPE* col_ind,
                                         VALTYPE* values,
...
template <typename ROWTYPE, typename COLTYPE, typename VALTYPE>
__global__ void scale_sparse_tiles_global( const COLTYPE* sparse_tile_ids,
                                           COLTYPE n_sparse_tiles,
                                           int* sparse_tile_cursor,
                                           ROWTYPE base,
                                           const ROWTYPE* tile_nnz_prefix,
                                           const COLTYPE* row_ind,
                                           const COLTYPE* col_ind,
...
template <typename ROWTYPE, typename COLTYPE, typename VALTYPE, CudaRuizScalingNormType NORM>
bool RuizScaleCudaCSRTemplate( const COLTYPE rows,
                               const COLTYPE cols,
                               const ROWTYPE* d_ai,
                               const COLTYPE* d_aj,
                               VALTYPE* d_av,
                               ...
                               const int max_iters )
{
    ...
    for ( int iter = 0; iter < max_iters; ++iter )
    {
        ...
        cuda_check( cudaStreamWaitEvent( accum_stream, scale_factors_ready, 0 ),
                    "Stream wait failed" );
...
COLTYPE compute_sparse_split_point( const DeviceTileCOOMatrix<ROWTYPE, COLTYPE, VALTYPE>& tile_mat )
{
    ...
    compute_tile_nnz_from_prefix<<<classify_grid, classify_block>>>(
        n_tiles, tile_mat.tileNnzPrefixData(), tile_nnz.data() );
