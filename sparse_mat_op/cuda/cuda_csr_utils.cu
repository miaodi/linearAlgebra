...
__global__ void CSRFindDiagonalKernel(
    COLTYPE rows,
    const ROWTYPE* __restrict__ d_ai,
...
__global__ void CSRGenDiagScaledPruneMaskKernel(
    COLTYPE rows,
    const ROWTYPE* __restrict__ d_ai,
    const COLTYPE* __restrict__ d_aj,
    const VALTYPE* __restrict__ d_av,
    ...
    ROWTYPE base)
{
    ...
    if(i == j)
    {
        ...
        d_mask[k] = (fabs(aij) * fabs(aij) >= fabs(di) * fabs(dj) * threshold) ? FLAGTYPE(1) : FLAGTYPE(0);
    }
}
    ...
    cudaStream_t stream)
{
    ...
    detail::CSRFindDiagonalKernel<<<grid, block, 0, stream>>>(rows, d_ai, d_aj, d_av, d_diag_pos, d_diag_val, base);
    ...
    cudaStream_t stream)
{
    ...
    ROWTYPE base{};
    ROWTYPE last{};
    detail::check_cuda(cudaMemcpy(&base, d_ai_in, sizeof(ROWTYPE), cudaMemcpyDeviceToHost), "load CSR base");
    detail::check_cuda(cudaMemcpy(&last, d_ai_in + rows, sizeof(ROWTYPE), cudaMemcpyDeviceToHost), "load CSR nnz bound");
    ...
    cub::DeviceSegmentedReduce::Sum(
        nullptr, segmented_bytes, d_keep_mask,
    ...
    cub::DeviceSegmentedReduce::Sum(
        segmented_storage.data().get(), segmented_bytes, d_keep_mask,
        thrust::raw_pointer_cast(row_counts.data()), num_segments,
        thrust::raw_pointer_cast(offsets.data()),
        thrust::raw_pointer_cast(offsets.data()) + 1, stream);
    ...
    thrust::transform(exec, row_prefix.begin(), row_prefix.end(), 
                      thrust::device_pointer_cast(d_ai_out), 
                      detail::AddBase<ROWTYPE>{base});

    // Compute new nnz
    ROWTYPE new_bound{};
    detail::check_cuda(cudaMemcpy(&new_bound, d_ai_out + rows, sizeof(ROWTYPE), cudaMemcpyDeviceToHost), "copy new nnz bound");
    ...
    auto col_result = thrust::copy_if(exec, col_in_begin, col_in_begin + nnz_count, flags_begin, col_out_begin, detail::KeepFlag<FLAGTYPE>{});
    if (static_cast<ROWTYPE>(col_result - col_out_begin) != new_nnz)
    {   
        std::cout<<"Expected nnz: " << new_nnz << ", copied nnz: " << (col_result - col_out_begin) << std::endl;
    ...
    if (d_av_in && d_av_out)
    {
        ...
        auto val_result = thrust::copy_if(exec, val_in_begin, val_in_begin + nnz_count, flags_begin, val_out_begin, detail::KeepFlag<FLAGTYPE>{});
    }
    ...
    cudaStream_t stream)
{
    ...
    CSRFindDiagonalDevice(rows, d_ai, d_aj, d_av, thrust::raw_pointer_cast(diag_pos.data()), thrust::raw_pointer_cast(diag.data()), stream);
...
template void CSRFindDiagonalDevice<std::int64_t, int, float>(int, const std::int64_t*, const int*, const float*, std::int64_t*, float*, cudaStream_t);
template void CSRFindDiagonalDevice<std::int64_t, int, double>(int, const std::int64_t*, const int*, const double*, std::int64_t*, double*, cudaStream_t);

template int CSRSelectByMaskDevice<int, int, float, int>(int, const int*, const int*, const float*, const int*, int*, int*, float*, cudaStream_t);
template int CSRSelectByMaskDevice<int, int, double, int>(int, const int*, const int*, const double*, const int*, int*, int*, double*, cudaStream_t);
template std::int64_t CSRSelectByMaskDevice<std::int64_t, int, float, int>(int, const std::int64_t*, const int*, const float*, const int*, std::int64_t*, int*, float*, cudaStream_t);
template std::int64_t CSRSelectByMaskDevice<std::int64_t, int, double, int>(int, const std::int64_t*, const int*, const double*, const int*, std::int64_t*, int*, double*, cudaStream_t);
