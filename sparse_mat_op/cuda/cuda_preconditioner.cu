...
void NoPreconditioner::operator()( const DeviceVectorView& d_input, DeviceVectorView& d_output )
{
    // Only copy if input and output point to different memory locations
    if ( d_input.data() != d_output.data() )
    {
        check_cuda_error( cudaMemcpy( d_output.data(), d_input.data(),
...
JacobiPreconditioner::JacobiPreconditioner() : _d_inv_diag(), _n( 0 ), _is_setup( false ) {}
...
void JacobiPreconditioner::operator()( const DeviceVectorView& d_input, DeviceVectorView& d_output )
{
    if ( !_is_setup )
    {
...
void CuSparseILUPrecBase::operator()( const DeviceVectorView& d_input, DeviceVectorView& d_output )
{
    if ( !_is_setup )
    {
    ...
    // Solve L * tmp = input
    check_cusparse_error( cusparseSpSV_solve( _cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha,
                             ...
                             const double* h_va_U )
{
    ...
    check_cusparse_error(
        cusparseCreateCsr( &_mat_L, n, n, _nnz_L, _d_ia_L.data(), _d_ja_L.data(), _d_va_L.data(),
    ...
    check_cusparse_error(
        cusparseCreateCsr( &_mat_U, n, n, _nnz_U, _d_ia_U.data(), _d_ja_U.data(), _d_va_U.data(),
    ...
    check_cusparse_error( cusparseSpSV_createDescr( &_spv_descr_L ),
                          "Failed to create SpSV L descriptor" );
    check_cusparse_error( cusparseSpSV_createDescr( &_spv_descr_U ),
                          "Failed to create SpSV U descriptor" );
    ...
    check_cusparse_error(
        cusparseSpSV_bufferSize( _cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, _mat_L,
                                 dummy_input.descriptor(), dummy_output.descriptor(), CUDA_R_64F,
                                 CUSPARSE_SPSV_ALG_DEFAULT, _spv_descr_L, &buffer_size_L ),
        "Failed to get SpSV L buffer size" );
    _d_buffer_L.resize( buffer_size_L );

    check_cusparse_error(
        cusparseSpSV_bufferSize( _cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, _mat_U,
                                 dummy_input.descriptor(), dummy_output.descriptor(), CUDA_R_64F,
    ...
    // Analyze phase
    check_cusparse_error(
        cusparseSpSV_analysis( _cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, _mat_L,
                               dummy_input.descriptor(), dummy_output.descriptor(), CUDA_R_64F,
                               CUSPARSE_SPSV_ALG_DEFAULT, _spv_descr_L, _d_buffer_L.data() ),
        "Failed to analyze L solve" );

    check_cusparse_error(
        cusparseSpSV_analysis( _cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, _mat_U,
                               dummy_input.descriptor(), dummy_output.descriptor(), CUDA_R_64F,
                               CUSPARSE_SPSV_ALG_DEFAULT, _spv_descr_U, _d_buffer_U.data() ),
...
CuSparseILU0Prec::CuSparseILU0Prec( cusparseHandle_t handle )
    : CuSparseILUPrecBase( handle ), _d_ia(), _d_ja(), _d_va(), _d_ilu0_buffer(), _d_buffer_L(), _d_buffer_U(), _ilu0_info( nullptr )
{
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    if ( _ilu0_info )
        cusparseDestroyCsrilu02Info( _ilu0_info );
#pragma GCC diagnostic pop
}
...
void CuSparseILU0Prec::setup( size_t n, const int* h_ia, const int* h_ja, const double* h_va )
{
    cleanup();
...
void CuSparseILU0Prec::setupFromDevice( size_t n, size_t nnz, const int* d_ia, const int* d_ja, const double* d_va, int index_base )
{
    cleanup();
...
void CuSparseILU0Prec::performFactorization( size_t n, size_t nnz, int index_base, int* d_ia, int* d_ja, double* d_va )
{
    // Create matrix descriptor for deprecated API
    cusparseMatDescr_t descr_A;
    check_cusparse_error( cusparseCreateMatDescr( &descr_A ),
                          "Failed to create matrix descriptor" );
    check_cusparse_error( cusparseSetMatType( descr_A, CUSPARSE_MATRIX_TYPE_GENERAL ),
                          "Failed to set matrix type" );
    check_cusparse_error(
        cusparseSetMatIndexBase( descr_A, ( index_base == 0 ) ? CUSPARSE_INDEX_BASE_ZERO : CUSPARSE_INDEX_BASE_ONE ),
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    ...
    // Get buffer size
    int buffer_size;
    check_cusparse_error( cusparseDcsrilu02_bufferSize( _cusparse_handle, n, nnz, descr_A, d_va,
                                                        d_ia, d_ja, _ilu0_info, &buffer_size ),
    ...
    // Analysis phase
    check_cusparse_error(
        cusparseDcsrilu02_analysis( _cusparse_handle, n, nnz, descr_A, d_va, d_ia, d_ja, _ilu0_info,
                                    CUSPARSE_SOLVE_POLICY_USE_LEVEL, _d_ilu0_buffer.data() ),
    ...
    // Factorization phase
    check_cusparse_error( cusparseDcsrilu02( _cusparse_handle, n, nnz, descr_A, d_va, d_ia, d_ja, _ilu0_info,
                                             CUSPARSE_SOLVE_POLICY_USE_LEVEL, _d_ilu0_buffer.data() ),
...
void CuSparseILU0Prec::setupSpSVDescriptors( size_t n,
                                             size_t nnz,
                                             int index_base_value,
                                             const int* d_ia,
                                             const int* d_ja,
                                             const double* d_va )
{
    ...
    // Create L matrix descriptor (lower triangular with unit diagonal)
    check_cusparse_error( cusparseCreateCsr( &_mat_L, n, n, nnz, const_cast<int*>( d_ia ),
                                             const_cast<int*>( d_ja ), const_cast<double*>( d_va ),
    ...
    // Create U matrix descriptor (upper triangular with non-unit diagonal)
    check_cusparse_error( cusparseCreateCsr( &_mat_U, n, n, nnz, const_cast<int*>( d_ia ),
                                             const_cast<int*>( d_ja ), const_cast<double*>( d_va ),
                                             CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, index_base, CUDA_R_64F ),
