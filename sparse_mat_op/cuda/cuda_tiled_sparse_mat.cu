...
__global__ void BuildTileKeys( NnzType nnz,
                               int k,
...
template <typename NnzType, typename IndexType>
__global__ void DecodeTileKeys( NnzType n,
...
__global__ void BuildNnzReorderMapByTile( COLTYPE n_tiles,
                                          const COLTYPE* __restrict__ sorted_tile_ids,
...
template <typename NnzType, typename IndexType>
void LaunchDecodeTileKeys( NnzType n, const uint64_t* d_keys, int col_bits, IndexType* d_tile_rows, IndexType* d_tile_cols, cudaStream_t stream )
{
    if ( n <= 0 )
        return;
...
void TileKeysToCOOMeta( NnzType n,
                        NnzType n_tiles,
                        ...
                        cudaStream_t stream )
{
    ...
    const auto reduce_result = thrust::reduce_by_key(
        exec, keys_begin, keys_begin + static_cast<size_t>( n ),
...
template <typename NnzType, typename IndexType>
void LaunchBuildTileKeys( NnzType nnz,
                          int k,
...
void CSRToTileCOO( COLTYPE rows,
                   COLTYPE cols,
                   const ROWTYPE* d_ai,
                   const COLTYPE* d_aj,
                   const VALTYPE* d_av,
                   ...
                   cudaStream_t stream )
{
    ...
    TileKeysToCOOMeta<size_t, ROWTYPE, COLTYPE>(
        nnz_count, n_tiles, keys_sorted.data(), col_bits, unique_keys.data(), tile_nnz.data(),
        out.tile_row_ind.data(), out.tile_col_ind.data(), stream );

    thrust::exclusive_scan( thrust::cuda::par.on( stream ), tile_nnz.data(),
                            tile_nnz.data() + n_tiles, out.tile_nnz_prefix.data() );
...
template __global__ void BuildTileKeys<std::int64_t, int>( std::int64_t, int, const int*, const int*, int, int, uint64_t* );
template void LaunchBuildTileKeys<int, int>( int, int, const int*, const int*, int, int, uint64_t*, cudaStream_t );
template void LaunchBuildTileKeys<std::int64_t, int>( std::int64_t, int, const int*, const int*, int, int, uint64_t*, cudaStream_t );
...
template void TileKeysToCOOMeta<int, int, int>( int, int, const uint64_t*, int, uint64_t*, int*, int*, int*, cudaStream_t );
template void TileKeysToCOOMeta<std::int64_t, std::int64_t, int>( std::int64_t,
                                                                  std::int64_t,
                                                                  const uint64_t*,
                                                                  int,
                                                                  uint64_t*,
                                                                  ...
                                                                  int*,
                                                                  cudaStream_t );
template void TileKeysToCOOMeta<size_t, int, int>( size_t, size_t, const uint64_t*, int, uint64_t*, int*, int*, int*, cudaStream_t );
template void CSRToTileCOO<int, int, float, false>( int,
                                                    int,
                                                    const int*,
                                                    const int*,
                                                    const float*,
                                                    ...
                                                    cudaStream_t );
template void CSRToTileCOO<int, int, float, true>( int,
                                                   int,
                                                   const int*,
                                                   const int*,
                                                   const float*,
                                                   ...
                                                   DeviceTileCOOMatrix<int, int, float>&,
                                                   cudaStream_t );
template void CSRToTileCOO<int, int, double, false>( int,
                                                     int,
                                                     const int*,
                                                     const int*,
                                                     const double*,
                                                     ...
                                                     cudaStream_t );
template void CSRToTileCOO<int, int, double, true>( int,
                                                    int,
                                                    const int*,
                                                    const int*,
                                                    const double*,
                                                    ...
                                                    DeviceTileCOOMatrix<int, int, double>&,
                                                    cudaStream_t );
template void CSRToTileCOO<std::int64_t, int, float, false>( int,
                                                             int,
                                                             const std::int64_t*,
                                                             const int*,
                                                             const float*,
                                                             ...
                                                             cudaStream_t );
template void CSRToTileCOO<std::int64_t, int, float, true>( int,
                                                            int,
                                                            const std::int64_t*,
                                                            const int*,
                                                            const float*,
                                                            ...
                                                            DeviceTileCOOMatrix<std::int64_t, int, float>&,
                                                            cudaStream_t );
template void CSRToTileCOO<std::int64_t, int, double, false>( int,
                                                              int,
                                                              const std::int64_t*,
                                                              const int*,
                                                              const double*,
                                                              ...
                                                              cudaStream_t );
template void CSRToTileCOO<std::int64_t, int, double, true>( int,
                                                             int,
                                                             const std::int64_t*,
                                                             const int*,
                                                             const double*,
