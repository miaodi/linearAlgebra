...
State CudaGMRES::perform_restart_cycle( const DeviceVectorView& d_b,
                                        DeviceVectorView& d_x,
                                        double init_resid,
                                        int& iter,
                                        double& resid )
{
...
void CudaGMRES::batch_gram_schmidt( int j, double* d_q_j_plus_1 )
{
    ...
    check_cublas_error( cublasDgemv( _cublas_handle, CUBLAS_OP_T, m, k, &alpha, _d_Q.data(), m,
                                     d_q_j_plus_1, 1, &zero, _d_h_batch.data(), 1 ),
                        "Failed to compute batched dot products in Gram-Schmidt" );

    // Copy Hessenberg column from device to host
    check_cuda_error( cudaMemcpy( _h_H.data() + j * _current_restart, _d_h_batch.data(),
    ...
    double neg_one = -1.0;
    double one = 1.0;
    check_cublas_error( cublasDgemv( _cublas_handle, CUBLAS_OP_N, m, k, &neg_one, _d_Q.data(), m,
                                     _d_h_batch.data(), 1, &one, d_q_j_plus_1, 1 ),
...
void CudaGMRES::gram_schmidt( int j, double* d_q_j_plus_1 )
{
    for ( int i = 0; i <= j; ++i )
    {
        ...
        check_cublas_error( cublasDdot( _cublas_handle, _n, d_q_i, 1, d_q_j_plus_1, 1,
...
void CudaGMRES::solve_least_squares( int j )
{
    ...
    dtrsv_( &uplo, &trans, &diag, &j, _h_H.data(), &_current_restart, _h_g.data(), &incx );
}
...
void CudaGMRES::update_solution( DeviceVectorView& d_x, int j )
{
    ...
    check_cublas_error( cublasDgemv( _cublas_handle, CUBLAS_OP_N, _n, j, &one, _d_Q.data(), _n,
                                     _d_g.data(), 1, &zero, _view_d_w.data(), 1 ),
