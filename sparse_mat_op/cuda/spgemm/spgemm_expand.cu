...
template <typename ROWTYPE, typename COLTYPE, typename VALTYPE>
__device__ void expand_row_strided(
    COLTYPE row,
    int lane,
    int stride,
    const ROWTYPE* A_row_ptr,
    ...
    cudaStream_t stream)
{
    ...
    cub::DeviceScan::ExclusiveScan(
        ...
        ::cuda::std::plus<ExpandedIndex>(),
        static_cast<ExpandedIndex>(base),
    ...
    cudaStream_t stream)
{
    ...
    if (global_rows > 0)
    {
        ...
        checkCudaError(
            cudaMemsetAsync(global_tile_scan_input.data(), 0, static_cast<size_t>(global_rows + 1) * sizeof(ExpandedIndex), stream),
        ...
        compute_global_tile_counts_kernel<COLTYPE><<<count_blocks, threads_per_block, 0, stream>>>(
            ...
            symbolic.expanded_row_ptr.data(),
            static_cast<ExpandedIndex>(base),
            static_cast<ExpandedIndex>(threads_per_block),
