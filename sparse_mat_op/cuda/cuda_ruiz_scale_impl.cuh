                    ...
                    const int max_iters )
{
    ...
    {
        static_assert( detail::always_false_v<ROWTYPE, COLTYPE, VALTYPE>,
                       "Unsupported RuizScaleCuda norm" );
        return false;
    }
}

template <typename ROWTYPE, typename COLTYPE, typename VALTYPE, CudaRuizScalingNormType NORM>
bool RuizScaleCuda( DeviceTileCOOMatrix<ROWTYPE, COLTYPE, VALTYPE>& tile_mat, VALTYPE* d_dr, VALTYPE* d_dc, const int max_iters )
{
    if constexpr ( NORM == CudaRuizScalingNormType::MaxNorm )
    {
