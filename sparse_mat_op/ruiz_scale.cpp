...
bool RuizScaleSerial( const COLTYPE rows,
                      const COLTYPE cols,
                      ROWTYPE const* ai,
                      COLTYPE const* aj,
                      VALTYPE* av,
                      ...
                      const VALTYPE tol )
{
    ...
    for ( int iter = 0; iter < max_iters; ++iter )
    {
        ...
        std::cout << "Iteration " << iter << ": avg_row_norm = " << avg_row_norm
                  << ", avg_col_norm = " << avg_col_norm << ", max_dev = " << max_dev << std::endl;
...
#define INSTANTIATE_RUIZ_SCALE( ROWTYPE, COLTYPE, VALTYPE )                                 \
    template bool RuizScaleSerial<ROWTYPE, COLTYPE, VALTYPE, RuizScalingNormType::MaxNorm>( \
        const COLTYPE, const COLTYPE, ROWTYPE const*, COLTYPE const*, VALTYPE*, VALTYPE*,   \
        VALTYPE*, const int, const VALTYPE );                                               \
